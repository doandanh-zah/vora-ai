...
1. Download the runtime for your OS (extracts into `$VORA_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.vora/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$VORA_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.vora/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Vora config file (`$VORA_CONFIG_PATH`, default `~/.vora/vora.json`):
