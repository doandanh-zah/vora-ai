---
name: node-connect
description: Diagnose Vora node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Vora is advertising that route, then fix pairing/auth.
...
- whether `vora devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `vora qr --json`. It uses the same setup-code payload Android scans.

```bash
vora config get gateway.mode
vora config get gateway.bind
vora config get gateway.tailscale.mode
vora config get gateway.remote.url
vora config get gateway.auth.mode
vora config get gateway.auth.allowTailscale
vora config get plugins.entries.device-pair.config.publicUrl
vora qr --json
vora devices list
vora nodes status
```

If this Vora instance is pointed at a remote gateway, also run:
...
If `vora qr --json` says `Gateway is only bound to loopback`:
...
- `vora devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run vora qr again, rescan, then approve the pending device pairing.`
