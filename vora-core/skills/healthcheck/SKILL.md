---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Vora deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Vora cron scheduling for periodic checks, or version status checks on a machine running Vora (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Vora, then align it to a user-defined risk tolerance without breaking access. Use Vora security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Vora changes the host firewall, SSH, or OS updates; it does not.
...
5. Vora gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Vora can access sensitive data.
...
### 2) Run Vora security audits (read-only)

As part of the default read-only checks, run `vora security audit --deep`. Only offer alternatives if the user requests them:

1. `vora security audit` (faster, non-probing)
2. `vora security audit --json` (structured output)

Offer to apply Vora safe defaults (numbered):

1. `vora security audit --fix`

Be explicit that `--fix` only tightens Vora defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Vora version/update status (read-only)

As part of the default read-only checks, run `vora update status`.
...
- Credential hygiene notes (location of Vora creds, prefer disk encryption)
...
After Vora install or first hardening pass, run at least one baseline audit and version check:

- `vora security audit`
- `vora security audit --deep`
- `vora update status`

Ongoing monitoring is recommended. Use the Vora cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `vora cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `vora update status`
...
Before creating, `vora cron list` and match on exact `name`. If found, `vora cron edit <id> ...`.
If not found, `vora cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `vora update status` (preferred for source checkouts and channels)
2. `npm view vora version` (published npm version)

## Vora command accuracy

Use only supported commands and flags:

- `vora security audit [--deep] [--fix] [--json]`
- `vora status` / `vora status --deep`
- `vora health --json`
- `vora update status`
- `vora cron add|list|runs|run`

Do not invent CLI flags or imply Vora enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Vora compaction:
