...
description: Triage GitHub security advisories for Vora with high-confidence close/keep decisions, exact tag and commit verification, trust-model checks, optional hardening notes, and a final reply ready to post and copy to clipboard.
---

# Security Triage

Use when reviewing Vora security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/vora/vora/security-advisories/<GHSA>`.
3. Inspect the exact implicated code paths.
4. Verify shipped state:
   - `git tag --sort=-creatordate | head`
   - `npm view vora version --userconfig "$(mktemp)"`
...
```bash
gh api /repos/vora/vora/security-advisories/<GHSA>
gh api /repos/vora/vora/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view vora version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo vora/vora --match title,body,comments -- "<terms>"
gh search prs --repo vora/vora --match title,body,comments -- "<terms>"
```
