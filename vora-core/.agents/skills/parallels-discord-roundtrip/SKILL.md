...
```bash
export VORA_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.vora/vora.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env VORA_PARALLELS_DISCORD_TOKEN \
...
- Use the `vora` wrapper for guest `message send/read`; `node vora.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/vora-parallels-smoke.*`
...
- host inbound nonce appears in `vora message read` output
