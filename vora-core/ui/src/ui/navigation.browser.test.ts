...
function expectConfirmedGatewayChange(app: ReturnType<typeof mountApp>) {
  expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/vora");
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/vora/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/vora");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/vora/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__VORA_CONTROL_UI_BASE_PATH__ = "/vora";
    const app = mountApp("/vora/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/vora");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/vora/sessions");
  });
  ...
  it("hydrates token from query params and strips them", async () => {
    ...
    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("vora.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "vora.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/vora" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("vora.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/vora",
    });
    expect(JSON.parse(localStorage.getItem("vora.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/vora";
  ...
  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/vora#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/vora");
    expect(app.settings.token).toBe("");
