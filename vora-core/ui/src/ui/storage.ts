const SETTINGS_KEY_PREFIX = "vora.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "vora.control.settings.v1";
const LEGACY_TOKEN_SESSION_KEY = "vora.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "vora.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    typeof window.__VORA_CONTROL_UI_BASE_PATH__ === "string" &&
    window.__VORA_CONTROL_UI_BASE_PATH__.trim();
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("vora.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
