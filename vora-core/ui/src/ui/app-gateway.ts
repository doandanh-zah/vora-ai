...
export function connectGateway(host: GatewayHost, options?: ConnectGatewayOptions) {
  ...
  const client = new GatewayBrowserClient({
    ...
    onHello: (hello) => {
      ...
      void subscribeSessions(host as unknown as VoraApp);
      void loadAssistantIdentity(host as unknown as VoraApp);
      void loadAgents(host as unknown as VoraApp);
      void loadHealthState(host as unknown as VoraApp);
      void loadNodes(host as unknown as VoraApp, { quiet: true });
      void loadDevices(host as unknown as VoraApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (runId && host.refreshSessionsAfterChat.has(runId)) {
    ...
    if (state === "final") {
      void loadSessions(host as unknown as VoraApp, {
  ...
  if (hadToolEvents && state === "final") {
    void loadChatHistory(host as unknown as VoraApp);
    return true;
  }
  return false;
}
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  const state = handleChatEvent(host as unknown as VoraApp, payload);
