...
export async function handleWhatsAppStart(host: VoraApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: VoraApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: VoraApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: VoraApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: VoraApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: VoraApp): string {
...
function resolveGatewayHttpAuthHeader(host: VoraApp): string | null {
...
function buildGatewayHttpHeaders(host: VoraApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: VoraApp) {
...
export function handleNostrProfileToggleAdvanced(host: VoraApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: VoraApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: VoraApp) {
  const state = host.nostrProfileFormState;
