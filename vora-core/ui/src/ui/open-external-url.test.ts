...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://vora.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://vora.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://vora.ai/abc-123", baseHref)).toBe(
      "blob:https://vora.ai/abc-123",
    );
  });
