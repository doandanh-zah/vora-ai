...
import type { VoraConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: VoraConfig): string[] {
...
export async function noteSecurityWarnings(cfg: VoraConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("vora security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.vora/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("vora approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("vora configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("vora config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("vora doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("vora config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('vora config set session.dmScope "per-channel-peer"') +
