...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-vora-backup";
...
describe("backupVerifyCommand", () => {
  ...
  async function resetTempHome() {
    ...
    await fs.mkdir(path.join(tempHome.home, ".vora"), { recursive: true });
    delete process.env.VORA_CONFIG_PATH;
  }

  beforeAll(async () => {
    tempHome = await createTempHomeEnv("vora-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".vora");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "vora.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-vora-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.vora",
            archivePath: `${rootName}/payload/posix/tmp/.vora`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "vora-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "vora-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".vora");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "vora-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-backup-verify-out-"));
    try {
      process.env.VORA_CONFIG_PATH = configPath;
      await fs.writeFile(
        configPath,
        JSON.stringify({
          agents: {
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.vora/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "vora-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "vora-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [
