...
import * as embeddedModule from "../agents/vora-embedded-runner.js";
...
const runEmbeddedVoraAgentSpy = vi.spyOn(embeddedModule, "runEmbeddedVoraAgent");
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "vora-agent-acp-" });
}

function createAcpEnabledConfig(home: string, storePath: string): VoraConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function mockConfigWithAcpOverrides(
  home: string,
  storePath: string,
  acpOverrides: Partial<NonNullable<VoraConfig["acp"]>>,
) {
...
async function runAcpSessionWithPolicyOverrides(params: {
  ...
  await withTempHome(async (home) => {
    ...
    expect(runEmbeddedVoraAgentSpy).not.toHaveBeenCalled();
  });
}
...
describe("agentCommand ACP runtime routing", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    runEmbeddedVoraAgentSpy.mockResolvedValue({
  ...
  it.each([
    {
      ...
      acpOverrides: { enabled: false } satisfies Partial<NonNullable<VoraConfig["acp"]>>,
    },
    {
      name: "blocks ACP turns when ACP dispatch is disabled by policy",
      acpOverrides: {
        dispatch: { enabled: false },
      } satisfies Partial<NonNullable<VoraConfig["acp"]>>,
    },
