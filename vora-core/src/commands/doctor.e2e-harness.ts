...
export const resolveVoraPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadVoraPlugins: () => createEmptyPluginRegistry(),
}));
...
beforeEach(() => {
  ...
  resolveVoraPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.VORA_STATE_DIR;
  originalUpdateInProgress = process.env.VORA_UPDATE_IN_PROGRESS;
  process.env.VORA_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "vora-doctor-state-"));
