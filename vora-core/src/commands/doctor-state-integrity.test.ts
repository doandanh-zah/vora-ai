...
type EnvSnapshot = {
  HOME?: string;
  VORA_HOME?: string;
  VORA_STATE_DIR?: string;
  VORA_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    VORA_HOME: process.env.VORA_HOME,
    VORA_STATE_DIR: process.env.VORA_STATE_DIR,
    VORA_OAUTH_DIR: process.env.VORA_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: VoraConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: VoraConfig) {
...
async function runStateIntegrityText(cfg: VoraConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "vora-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.VORA_HOME = tempHome;
    process.env.VORA_STATE_DIR = path.join(tempHome, ".vora");
    delete process.env.VORA_OAUTH_DIR;
    fs.mkdirSync(process.env.VORA_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when VORA_OAUTH_DIR is explicitly configured", async () => {
    process.env.VORA_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: VoraConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints vora-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: VoraConfig = {};
    ...
    expect(text).toMatch(/vora sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/vora sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /vora sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
