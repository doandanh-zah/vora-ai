...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.VORA_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.VORA_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from vora.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<VoraConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    ...
    const baseConfig: VoraConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("vora doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.VORA_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.VORA_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });
