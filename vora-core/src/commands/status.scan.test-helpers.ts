...
export function createStatusScanSharedMocks(configPathLabel: string) {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/vora-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = VoraConfig>(
  overrides: T = {} as T,
): VoraConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): VoraConfig {
