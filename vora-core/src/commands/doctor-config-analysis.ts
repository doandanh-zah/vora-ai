...
import { VoraSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: VoraConfig): {
  config: VoraConfig;
  removed: string[];
} {
  const parsed = VoraSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: VoraConfig): void {
