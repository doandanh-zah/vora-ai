...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under VORA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("vora-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-oauth-sync-"));
    ...
    process.env.VORA_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-oauth-nosync-"));
    process.env.VORA_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside VORA_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-oauth-external-"));
    process.env.VORA_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* VORA_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to VORA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("vora-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
