...
beforeAll(() => {
  envSnapshot = captureEnv(["VORA_PROFILE"]);
  process.env.VORA_PROFILE = "isolated";
});
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.vora.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.vora.node.plist",
    }),
  }),
}));
...
vi.mock("../infra/vora-root.js", () => ({
  resolveVoraPackageRoot: vi.fn().mockResolvedValue("/tmp/vora"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/vora/pnpm-lock.yaml",
      markerPath: "/tmp/vora/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("vora status --all") ||
          line.includes("vora --profile isolated status --all"),
      ),
    ).toBe(true);
  });
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("vora --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("VORA_GATEWAY_TOKEN", "abcd1234", async () => {
