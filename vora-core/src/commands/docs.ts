...
const SEARCH_TOOL = "https://docs.vora.ai/mcp.SearchVora";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.vora.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('vora docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.vora.ai/");
      runtime.log(`Search: ${formatCliCommand('vora docs "your query"')}`);
    }
    return;
  }
