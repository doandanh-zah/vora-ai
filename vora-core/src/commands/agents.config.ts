...
function resolveAgentName(cfg: VoraConfig, agentId: string) {
...
function resolveAgentModel(cfg: VoraConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: VoraConfig): AgentSummary[] {
