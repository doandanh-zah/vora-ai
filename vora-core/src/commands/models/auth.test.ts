...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: VoraConfig;
  let lastUpdatedConfig: VoraConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/vora/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/vora/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/vora/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: VoraConfig) => VoraConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Enable it with vora configure --section web (recommended mode: cached). Docs: https://docs.vora.ai/tools/web",
    );
  });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/vora/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `vora plugins list --json`.',
    );
  });
