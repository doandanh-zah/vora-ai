...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveVoraAgentDir: vi.fn().mockReturnValue("/tmp/vora-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/vora-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/vora-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/vora-dev/vora.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveVoraAgentDir: mocks.resolveVoraAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/vora-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveVoraAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/vora-dev/vora.json");
    expect(payload.auth.storePath).toBe("/tmp/vora-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/vora-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/vora-agent-custom");
