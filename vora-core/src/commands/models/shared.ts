...
export async function loadValidConfigOrThrow(): Promise<VoraConfig> {
...
export async function updateConfig(
  mutator: (cfg: VoraConfig) => VoraConfig,
): Promise<VoraConfig> {
  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    const issues = formatConfigIssueLines(snapshot.issues, "-").join("\n");
...
export function resolveModelTarget(params: { raw: string; cfg: VoraConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: VoraConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("vora agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
