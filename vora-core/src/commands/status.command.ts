...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByVora ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByVora ? "installed · " : "";
  ...
  runtime.log(theme.heading("Vora status"));
  runtime.log("");
  ...
  if (summary.taskAudit.errors > 0) {
    runtime.log("");
    runtime.log(
      theme.muted(`Task maintenance: ${formatCliCommand("vora tasks maintenance --apply")}`),
    );
  }
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`vora devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("vora devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("vora devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("vora security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("vora security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.vora.ai/faq");
  runtime.log("Troubleshooting: https://docs.vora.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("vora status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("vora logs --follow")}`);
  if (nodeOnlyGateway) {
    runtime.log(`  Need node service?  ${formatCliCommand("vora node status")}`);
  } else if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("vora status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("vora gateway probe")}`);
  }
}
