...
function createPerplexityConfig(apiKey: string, enabled?: boolean): VoraConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: VoraConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): VoraConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: VoraConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: VoraConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
