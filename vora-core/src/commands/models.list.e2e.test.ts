...
const ensureVoraModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveVoraAgentDir = vi.fn().mockReturnValue("/tmp/vora-agent");
...
beforeEach(() => {
  ...
  ensureVoraModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureVoraModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureVoraModelsJson).toHaveBeenCalled();
    expect(ensureVoraModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
