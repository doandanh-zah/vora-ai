...
import { runEmbeddedVoraAgent } from "../agents/vora-embedded-runner.js";
...
vi.mock("../agents/workspace.js", () => {
  const resolveDefaultAgentWorkspaceDir = () => "/tmp/vora-workspace";
  return {
    DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/vora-workspace",
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "vora-agent-" });
}
...
async function loadFreshAgentCommandModulesForTest() {
  ...
  vi.doMock("../agents/vora-embedded-runner.js", () => ({
    ...
    runEmbeddedVoraAgent: runEmbeddedVoraAgentMock,
...
function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<VoraConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<VoraConfig["channels"]>["telegram"]>>,
...
async function runWithDefaultAgentConfig(params: {
  ...
  return vi.mocked(runEmbeddedVoraAgent).mock.calls.at(-1)?.[0];
}
...
async function withCrossAgentResumeFixture(
  run: (params: { sessionId: string; sessionKey: string; cfg: VoraConfig }) => Promise<void>,
): Promise<void> {
...
beforeEach(() => {
  ...
  vi.mocked(runEmbeddedVoraAgent).mockResolvedValue(createDefaultAgentResult());
  vi.mocked(loadModelCatalog).mockResolvedValue([]);
  vi.mocked(modelSelectionModule.isCliProvider).mockImplementation(() => false);
  readConfigFileSnapshotForWriteSpy.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as VoraConfig },
    writeOptions: {},
...
describe("agentCommand", () => {
  it("sets runtime snapshots from source config before embedded agent run", async () => {
    await withTempHome(async (home) => {
      ...
      runEmbeddedVoraAgentMock.mockResolvedValue(createDefaultAgentResult());
      ...
      expect(runEmbeddedVoraAgentMock.mock.calls.at(-1)?.[0]?.config).toBe(resolvedConfig);
    });
  });
  ...
  it("persists thinking and verbose overrides", async () => {
    await withTempHome(async (home) => {
      ...
      const callArgs = vi.mocked(runEmbeddedVoraAgent).mock.calls.at(-1)?.[0];
  ...
  it("honors explicit senderIsOwner for ingress runs", async () => {
    await withTempHome(async (home) => {
      ...
      const ingressCall = vi.mocked(runEmbeddedVoraAgent).mock.calls.at(-1)?.[0];
  ...
  it("does not duplicate agent events from embedded runs", async () => {
    await withTempHome(async (home) => {
      ...
      vi.mocked(runEmbeddedVoraAgent).mockImplementationOnce(async (params) => {
  ...
  it("uses default fallback list for session model overrides", async () => {
    await withTempHome(async (home) => {
      ...
      vi.mocked(runEmbeddedVoraAgent)
  ...
  it("prints JSON payload when requested", async () => {
    await withTempHome(async (home) => {
      vi.mocked(runEmbeddedVoraAgent).mockResolvedValue(
