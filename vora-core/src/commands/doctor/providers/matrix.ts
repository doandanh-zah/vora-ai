...
export function formatMatrixLegacyStatePreview(
  ...
  return [
    ...
    '- Run "vora doctor --fix" to migrate this Matrix state now.',
  ].join("\n");
}
  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "vora doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: VoraConfig): Promise<string[]> {
...
function hasConfiguredMatrixChannel(cfg: VoraConfig): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  return isRecord(channels?.matrix);
}

function hasConfiguredMatrixPluginSurface(cfg: VoraConfig): boolean {
  return Boolean(
...
function configMayNeedMatrixDoctorSequence(cfg: VoraConfig, env: NodeJS.ProcessEnv): boolean {
  return (
