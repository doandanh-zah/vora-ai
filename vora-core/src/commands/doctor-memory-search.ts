 ...
 * Runs as part of `vora doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("vora config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("vora config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("vora memory status --deep")}`,
        ]
          .filter(Boolean)
          .join("\n"),
        "Memory search",
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`vora config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("vora configure --section model")}`,
        `- To disable: ${formatCliCommand("vora config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("vora memory status --deep")}`,
      ].join("\n"),
      "Memory search",
