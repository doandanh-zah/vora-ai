...
const TEMP_MEDIA_PREFIX = "vora-media-";
...
function createGroqAudioConfig(): VoraConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): VoraConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): VoraConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: VoraConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredVoraTmpDir();
