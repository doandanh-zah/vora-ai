...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    ...
    expect(resolved.VORA_BASE_ENV).toBe("base");
    expect(resolved.VORA_TEST_ENV).toBe("ok");
    expect(resolved.VORA_TO_REMOVE).toBeUndefined();
    expect(resolved.VORA_CLI).toBe(VORA_CLI_ENV_VALUE);
  });
