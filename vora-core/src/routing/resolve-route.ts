...
function listAgents(cfg: VoraConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: VoraConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<VoraConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: VoraConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: VoraConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: VoraConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<VoraConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  VoraConfig,
  {
    bindingsRef: VoraConfig["bindings"];
    agentsRef: VoraConfig["agents"];
    sessionRef: VoraConfig["session"];
...
function resolveRouteCacheForConfig(cfg: VoraConfig): Map<string, ResolvedAgentRoute> {
