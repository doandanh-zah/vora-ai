...
import { type VoraConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: VoraConfig): string[] {
...
export function listAgentsForGateway(cfg: VoraConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: VoraConfig): string {
...
function resolveSessionStoreAgentId(cfg: VoraConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: VoraConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: VoraConfig): GatewaySessionsDefaults {
