...
const { createVoraTools } = await import("../agents/vora-tools.js");
...
type SessionSendTool = ReturnType<typeof createVoraTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createVoraTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["VORA_GATEWAY_PORT", "VORA_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.VORA_GATEWAY_PORT = String(gatewayPort);
  process.env.VORA_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("VORA_CONFIG_PATH missing in gateway test environment");
      }
