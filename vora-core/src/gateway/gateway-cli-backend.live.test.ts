...
const CLI_LIVE = isTruthyEnvValue(process.env.VORA_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.VORA_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.VORA_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("VORA_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
  ...
  it("runs the agent pipeline against the local CLI backend", async () => {
    const preservedEnv = new Set(
      parseJsonStringArray(
        "VORA_LIVE_CLI_BACKEND_PRESERVE_ENV",
        process.env.VORA_LIVE_CLI_BACKEND_PRESERVE_ENV,
      ) ?? [],
    );
    ...
    const rawModel = process.env.VORA_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `VORA_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.VORA_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
        `VORA_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
      );
    }
    const baseCliArgs =
      parseJsonStringArray(
        "VORA_LIVE_CLI_BACKEND_ARGS",
        process.env.VORA_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`VORA_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "VORA_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.VORA_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const filteredCliClearEnv = cliClearEnv.filter((name) => !preservedEnv.has(name));
    const cliImageArg = process.env.VORA_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.VORA_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "VORA_LIVE_CLI_BACKEND_IMAGE_MODE requires VORA_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-live-cli-"));
    const disableMcpConfig = process.env.VORA_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
