...
type GatewayInstallAuthMode = NonNullable<NonNullable<VoraConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: VoraConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    durableServiceEnv.VORA_GATEWAY_PASSWORD?.trim() ||
