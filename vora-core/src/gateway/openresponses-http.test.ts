...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("VORA_CONFIG_PATH is required for gateway config tests");
  }
...
describe("OpenResponses HTTP API (e2e)", () => {
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "vora", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `vora` or `vora/<agentId>`.",
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "vora/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "vora/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "vora", input: "hi" },
        { "x-vora-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-vora-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("treats HTTP callers as non-owner regardless of requested scopes", async () => {
    ...
    const adminScopeResponse = await postResponses(
      port,
      { model: "vora", input: "hi" },
      { "x-vora-scopes": "operator.admin, operator.write" },
    );
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "vora", input: "hi" },
      { "x-vora-scopes": "operator.admin, operator.write" },
    );
