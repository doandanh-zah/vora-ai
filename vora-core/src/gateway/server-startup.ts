...
import type { loadVoraPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureVoraModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: ReturnType<typeof loadConfig>;
  pluginRegistry: ReturnType<typeof loadVoraPlugins>;
  ...
  // Tests can opt out via VORA_SKIP_CHANNELS (or legacy VORA_SKIP_PROVIDERS).
  const skipChannels =
    isTruthyEnvValue(process.env.VORA_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.VORA_SKIP_PROVIDERS);
  if (!skipChannels) {
    try {
    ...
    params.logChannels.info(
      "skipping channel start (VORA_SKIP_CHANNELS=1 or VORA_SKIP_PROVIDERS=1)",
    );
  }
