...
import { resolveVoraAgentDir } from "../agents/agent-paths.js";
...
import { ensureVoraModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/config.js";
import type { ModelsConfig, VoraConfig, ModelProviderConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.VORA_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const PROVIDERS = parseFilter(process.env.VORA_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.VORA_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.VORA_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.VORA_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["VORA_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.VORA_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.VORA_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<VoraConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.VORA_CONFIG_PATH,
    token: process.env.VORA_GATEWAY_TOKEN,
    skipChannels: process.env.VORA_SKIP_CHANNELS,
    skipGmail: process.env.VORA_SKIP_GMAIL_WATCHER,
    skipCron: process.env.VORA_SKIP_CRON,
    skipCanvas: process.env.VORA_SKIP_CANVAS_HOST,
    disableBonjour: process.env.VORA_DISABLE_BONJOUR,
    logLevel: process.env.VORA_LOG_LEVEL,
    agentDir: process.env.VORA_AGENT_DIR,
    voraAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.VORA_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.VORA_SKIP_CHANNELS = "1";
  process.env.VORA_SKIP_GMAIL_WATCHER = "1";
  process.env.VORA_SKIP_CRON = "1";
  process.env.VORA_SKIP_CANVAS_HOST = "1";
  if (QUIET_LIVE_LOGS) {
    process.env.VORA_DISABLE_BONJOUR = "1";
    process.env.VORA_LOG_LEVEL = "silent";
  }

  const token = `test-${randomUUID()}`;
  process.env.VORA_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveVoraAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-live-state-"));
  process.env.VORA_STATE_DIR = tempStateDir;
  ...
  process.env.VORA_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.vora-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveVoraAgentDir();
  const sanitizedCfg: VoraConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-live-"));
  const tempConfigPath = path.join(tempDir, "vora.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.VORA_CONFIG_PATH = tempConfigPath;
  ...
  try {
        ...
        try {
          ...
          for (
            ...
            toolText = await requestGatewayAgentText({
              ...
              modelKey,
              message: strictReply
                ? "Vora live tool probe (local, safe): " +
    ...
    process.env.VORA_CONFIG_PATH = previous.configPath;
    process.env.VORA_GATEWAY_TOKEN = previous.token;
    process.env.VORA_SKIP_CHANNELS = previous.skipChannels;
    process.env.VORA_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.VORA_SKIP_CRON = previous.skipCron;
    process.env.VORA_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.VORA_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.VORA_LOG_LEVEL = previous.logLevel;
    process.env.VORA_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.voraAgentDir;
    process.env.VORA_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureVoraModelsJson(cfg);
      ...
      const rawModels = process.env.VORA_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via VORA_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.vora-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
