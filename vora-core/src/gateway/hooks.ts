...
export function resolveHooksConfig(cfg: VoraConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: VoraConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-vora-token"] === "string"
      ? req.headers["x-vora-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-vora-idempotency-key"]) ||
