...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([VORA_MODEL_ID, VORA_DEFAULT_MODEL_ID]);
  ids.add(`vora/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`vora/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== VORA_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
