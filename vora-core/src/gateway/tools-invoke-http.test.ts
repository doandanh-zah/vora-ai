...
let lastCreateVoraToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/vora-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateVoraToolsContext?.agentTo,
          agentThreadId: lastCreateVoraToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createVoraTools: (ctx: Record<string, unknown>) => {
      lastCreateVoraToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateVoraToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-vora-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-vora-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateVoraToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateVoraToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-vora-message-to": "channel:24514",
        "x-vora-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("treats shared-secret bearer auth as owner on /tools/invoke", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        authorization: "Bearer secret",
        "x-vora-scopes": "operator.approvals",
      },
      tool: "owner_only_test",
      sessionKey: "main",
    });
