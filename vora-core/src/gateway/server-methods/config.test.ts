...
describe("resolveConfigOpenCommand", () => {
  it("uses open on macOS", () => {
    expect(resolveConfigOpenCommand("/tmp/vora.json", "darwin")).toEqual({
      command: "open",
      args: ["/tmp/vora.json"],
    });
  });
  ...
  it("uses xdg-open on Linux", () => {
    expect(resolveConfigOpenCommand("/tmp/vora.json", "linux")).toEqual({
      command: "xdg-open",
      args: ["/tmp/vora.json"],
    });
  });
...
describe("config.openFile", () => {
  ...
  it("opens the configured file without shell interpolation", async () => {
    process.env.VORA_CONFIG_PATH = "/tmp/config $(touch pwned).json";
  ...
  it("returns a generic error and logs details when the opener fails", async () => {
    process.env.VORA_CONFIG_PATH = "/tmp/config.json";
    vi.mocked(execFile).mockImplementation(((...args: unknown[]) => {
