...
const ORIGINAL_STATE_DIR = process.env.VORA_STATE_DIR;
...
describe("gateway agent handler", () => {
  afterEach(() => {
    if (ORIGINAL_STATE_DIR === undefined) {
      ...
      process.env.VORA_STATE_DIR = ORIGINAL_STATE_DIR;
    }
  ...
  it("tracks async gateway agent runs in the shared task registry", async () => {
    await withTempDir({ prefix: "vora-gateway-agent-task-" }, async (root) => {
  ...
  it("handles bare /new by resetting the same session and sending reset greeting prompt", async () => {
    ...
    expect(call?.message).toContain("Execute your Session Startup sequence");
