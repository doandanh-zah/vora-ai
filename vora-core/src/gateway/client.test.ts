...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("vora doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with VORA_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with VORA_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.VORA_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://vora-gateway.ai:18789",
      onConnectError,
    });
