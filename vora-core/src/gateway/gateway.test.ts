...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".vora", "extensions", params.id);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "vora-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "vora");
      await fs.mkdir(workspaceDir, { recursive: true });

      const configDir = path.join(tempHome, ".vora");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "vora-gw-http-tools-home-"));
      process.env.HOME = tempHome;
      process.env.VORA_STATE_DIR = path.join(tempHome, ".vora");
      delete process.env.VORA_CONFIG_PATH;
      process.env.VORA_SKIP_CHANNELS = "1";
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "vora-wizard-home-"));
      process.env.HOME = tempHome;
