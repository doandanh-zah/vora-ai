...
function cfg(input: Partial<VoraConfig>): VoraConfig {
  return input as VoraConfig;
}
...
const DEFAULT_ENV = {
  VORA_GATEWAY_TOKEN: "env-token",
  VORA_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when VORA env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when VORA env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when VORA env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
