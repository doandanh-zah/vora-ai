...
const ensureVoraModelsJsonMock = vi.fn<
...
vi.mock("../agents/agent-paths.js", () => ({
  resolveVoraAgentDir: () => "/tmp/agent",
}));

vi.mock("../agents/models-config.js", () => ({
  ensureVoraModelsJson: (config: unknown, agentDir: unknown) =>
    ensureVoraModelsJsonMock(config, agentDir),
}));
...
describe("gateway startup primary model warmup", () => {
  ...
  beforeEach(() => {
    ensureVoraModelsJsonMock.mockClear();
  ...
  it("prewarms an explicit configured primary model", async () => {
    ...
    expect(ensureVoraModelsJsonMock).toHaveBeenCalledWith(cfg, "/tmp/agent");
  ...
  it("skips warmup when no explicit primary model is configured", async () => {
    ...
    expect(ensureVoraModelsJsonMock).not.toHaveBeenCalled();
