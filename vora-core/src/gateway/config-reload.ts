...
export function resolveGatewayReloadSettings(cfg: VoraConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: VoraConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: VoraConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: VoraConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: VoraConfig | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: VoraConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: VoraConfig) => {
