...
function loadGatewayConfig(): VoraConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: VoraConfig, env?: NodeJS.ProcessEnv): number {
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__VORA_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
