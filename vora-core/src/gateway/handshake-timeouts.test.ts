...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers VORA_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        VORA_HANDSHAKE_TIMEOUT_MS: "75",
        VORA_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        VORA_HANDSHAKE_TIMEOUT_MS: "",
        VORA_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
