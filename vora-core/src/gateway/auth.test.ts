...
describe("gateway auth", () => {
  ...
  it("resolves token/password from VORA gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          VORA_GATEWAY_TOKEN: "env-token",
          VORA_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides VORA_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        VORA_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
