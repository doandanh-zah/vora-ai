...
type GatewaySecretDefaults = NonNullable<VoraConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${VORA_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.VORA_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.VORA_GATEWAY_PASSWORD);
}
