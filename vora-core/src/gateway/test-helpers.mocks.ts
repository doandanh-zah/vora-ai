...
const GATEWAY_TEST_PLUGIN_REGISTRY_STATE_KEY = Symbol.for(
  "vora.gatewayTestHelpers.pluginRegistryState",
);
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("vora.gatewayTestHelpers.configRoot");

const hoisted = vi.hoisted(() => {
  const key = Symbol.for("vora.gatewayTestHelpers.hoisted");
...
const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `vora-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.VORA_CONFIG_PATH = path.join(root, "vora.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "vora.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "vora-gateway-test"),
      ...fileDefaults,
  ...
  return {
    ...
    applyConfigOverrides: (cfg: VoraConfig) =>
...
vi.mock("../agents/vora-embedded-runner.js", async () => {
  return await importEmbeddedRunMockModule<typeof import("../agents/vora-embedded-runner.js")>(
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadVoraPlugins: () => pluginRegistryState.registry,
  };
});
