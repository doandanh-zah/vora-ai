...
import { createVoraTools } from "../agents/vora-tools.js";
import { runBeforeToolCallHook } from "../agents/vora-tools.before-tool-call.js";
import { resolveToolLoopDetectionConfig } from "../agents/vora-tools.js";
import {
  resolveEffectiveToolPolicy,
  resolveGroupToolPolicy,
  resolveSubagentToolPolicy,
} from "../agents/vora-tools.policy.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-vora-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-vora-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-vora-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-vora-thread-id")?.trim() || undefined;
  ...
  const allTools = createVoraTools({
