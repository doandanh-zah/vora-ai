...
export function attachVoraTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__vora && typeof record.__vora === "object" && !Array.isArray(record.__vora)
      ? (record.__vora as Record<string, unknown>)
      : {};
  return {
    ...record,
    __vora: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachVoraTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "vora" && model === "delivery-mirror";
