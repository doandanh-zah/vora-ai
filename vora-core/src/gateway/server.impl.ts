...
import { ensureVoraCliOnPath } from "../infra/path-env.js";
import {
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("vora doctor")}" to repair, then retry.`
    : "";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.VORA_TEST_MINIMAL_GATEWAY === "1";
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `vora config set gateway.auth.mode token` and `vora config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  if (matrixInstallPathIssue) {
    const lines = formatPluginInstallPathIssue({
      ...
      pluginLabel: "Matrix",
      defaultInstallCommand: "vora plugins install @vora/matrix",
    ...
    transcriptUnsub = minimalTestGateway
      ...
      : onSessionTranscriptUpdate((update) => {
          ...
          const sessionSnapshot = sessionRow
            ...
            : {};
          const message = attachVoraTranscriptMeta(update.message, {
