import { DEFAULT_GATEWAY_PORT, resolveConfigPath, resolveGatewayPort } from "../config/paths.js";
...
type GatewayConnectionDetailResolvers = {
  loadConfig?: () => VoraConfig;
  resolveConfigPath?: (env: NodeJS.ProcessEnv) => string;
  resolveGatewayPort?: (cfg?: VoraConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.VORA_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.VORA_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        `- keep gateway.bind=loopback and use an SSH tunnel (ssh -N -L ${DEFAULT_GATEWAY_PORT}:127.0.0.1:${DEFAULT_GATEWAY_PORT} user@gateway-host)`,
        "- or use Tailscale Serve/Funnel for HTTPS remote access",
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set VORA_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: vora doctor --fix",
