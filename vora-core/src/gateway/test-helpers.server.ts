...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "VORA_STATE_DIR",
  "VORA_CONFIG_PATH",
  "VORA_SKIP_BROWSER_CONTROL_SERVER",
  "VORA_SKIP_GMAIL_WATCHER",
  "VORA_SKIP_CANVAS_HOST",
  "VORA_BUNDLED_PLUGINS_DIR",
  "VORA_SKIP_CHANNELS",
  "VORA_SKIP_PROVIDERS",
  "VORA_SKIP_CRON",
  "VORA_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.VORA_CONFIG_PATH) {
    configPaths.add(process.env.VORA_CONFIG_PATH);
  }
  if (process.env.VORA_STATE_DIR) {
    configPaths.add(path.join(process.env.VORA_STATE_DIR, "vora.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "vora-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.VORA_STATE_DIR = path.join(tempHome, ".vora");
  delete process.env.VORA_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.VORA_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.VORA_SKIP_GMAIL_WATCHER = "1";
  process.env.VORA_SKIP_CANVAS_HOST = "1";
  process.env.VORA_SKIP_CHANNELS = "1";
  process.env.VORA_SKIP_PROVIDERS = "1";
  process.env.VORA_SKIP_CRON = "1";
  process.env.VORA_TEST_MINIMAL_GATEWAY = "1";
  process.env.VORA_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "vora-test-no-bundled-extensions")
    : "vora-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  applyGatewaySkipEnv();
  const stateDir = process.env.VORA_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".vora-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".vora-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".vora-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>vora-test-control-ui</title>\n",
    "utf-8",
  );
...
const CONNECT_CHALLENGE_NONCE_KEY = "__voraTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__voraTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["VORA_GATEWAY_TOKEN"]);
  const prev = process.env.VORA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.VORA_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.VORA_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
