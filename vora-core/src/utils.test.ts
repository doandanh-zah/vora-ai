...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("vora-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("vora-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("vora-lid-a-", async (first) => {
      await withTempDir("vora-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.vora when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "vora-config-dir-"));
    try {
  ...
  it("expands VORA_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/vora-home",
      VORA_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/vora-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers VORA_HOME over HOME", () => {
    vi.stubEnv("VORA_HOME", "/srv/vora-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/vora-home"));
...
describe("shortenHomePath", () => {
  it("uses $VORA_HOME prefix when VORA_HOME is set", () => {
    vi.stubEnv("VORA_HOME", "/srv/vora-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/vora-home")}/.vora/vora.json`)).toBe(
      "$VORA_HOME/.vora/vora.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $VORA_HOME replacement when VORA_HOME is set", () => {
    vi.stubEnv("VORA_HOME", "/srv/vora-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/vora-home")}/.vora/vora.json`),
    ).toBe("config: $VORA_HOME/.vora/vora.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/vora", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "vora"),
    );
  });
  ...
  it("prefers VORA_HOME for tilde expansion", () => {
    vi.stubEnv("VORA_HOME", "/srv/vora-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/vora")).toBe(path.resolve("/srv/vora-home", "vora"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/vora", env)).toBe(path.resolve("/srv/vora-home", "vora"));
  });
