...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["vora security audit", "Run a local security audit."],
          ["vora security audit --deep", "Include best-effort live Gateway probe checks."],
          ["vora security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "vora security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["vora security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["vora security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.vora.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Vora security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("vora security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("vora security audit --fix")}`));
        if (!fixResult) {
