...
const serveVoraChannelMcp = mocks.serveVoraChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveVoraChannelMcp: mocks.serveVoraChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("vora-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("vora-cli-mcp-home-", async () => {
      ...
      expect(serveVoraChannelMcp).toHaveBeenCalledWith({
