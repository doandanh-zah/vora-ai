...
describe("tryRouteCli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.VORA_DISABLE_ROUTE_FIRST;
    delete process.env.VORA_DISABLE_ROUTE_FIRST;
    vi.resetModules();
  ...
  afterEach(() => {
    ...
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.VORA_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  });

  it("skips config guard for routed status --json commands", async () => {
    await expect(tryRouteCli(["node", "vora", "status", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "vora", "status"])).resolves.toBe(true);
  ...
  it("routes logs to stderr during plugin loading in --json mode and restores after", async () => {
    ...
    await tryRouteCli(["node", "vora", "agents", "--json"]);
  ...
  it("does not route logs to stderr during plugin loading without --json", async () => {
    ...
    await tryRouteCli(["node", "vora", "agents"]);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "vora", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );
