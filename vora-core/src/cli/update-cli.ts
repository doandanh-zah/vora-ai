...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["vora update", "Update a source checkout (git)"],
        ["vora update --channel beta", "Switch to beta channel (git + npm)"],
        ["vora update --channel dev", "Switch to dev channel (git + npm)"],
        ["vora update --tag beta", "One-off update to a dist-tag or version"],
        ["vora update --tag main", "One-off package install from GitHub main"],
        ["vora update --dry-run", "Preview actions without changing anything"],
        ["vora update --no-restart", "Update without restarting the service"],
        ["vora update --json", "Output result as JSON"],
        ["vora update --yes", "Non-interactive (accept downgrade prompts)"],
        ["vora update wizard", "Interactive update wizard"],
        ["vora --update", "Shorthand for vora update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run vora update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.vora.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["vora update status", "Show channel + version status."],
          ["vora update status --json", "JSON output."],
          ["vora update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.vora.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
