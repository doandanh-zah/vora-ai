...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("vora-daemon-install-int-");
    configPath = path.join(tempHome, "vora.json");
    process.env.HOME = tempHome;
    process.env.VORA_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.VORA_GATEWAY_TOKEN).toBeUndefined();
  });
});
