...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    VORA_STATE_DIR: "/tmp/vora-daemon",
    VORA_CONFIG_PATH: "/tmp/vora-daemon/vora.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.VORA_STATE_DIR ?? "/tmp/vora-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.VORA_CONFIG_PATH ?? `${stateDir}/vora.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/vora-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.VORA_STATE_DIR = "/tmp/vora-cli";
    process.env.VORA_CONFIG_PATH = "/tmp/vora-cli/vora.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/vora-daemon/vora.json",
      }),
    );
  });
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.VORA_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.VORA_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.VORA_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "vora-gateway" }],
        hints: [],
      },
      healthy: false,
