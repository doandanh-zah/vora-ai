...
export const loadConfig = vi.fn<() => VoraConfig>(() => ({}) as VoraConfig);
export const readConfigFileSnapshot = vi.fn();
export const writeConfigFile = vi.fn<(config: VoraConfig) => Promise<void>>(
  async () => undefined,
);
export const replaceConfigFile = vi.fn(
  async (params: { nextConfig: VoraConfig }) => await writeConfigFile(params.nextConfig),
);
export const resolveStateDir = vi.fn(() => "/tmp/vora-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: (config: VoraConfig) => writeConfigFile(config),
  replaceConfigFile: (params: { nextConfig: VoraConfig }) => replaceConfigFile(params),
}));
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as VoraConfig);
  readConfigFileSnapshot.mockImplementation(async () => {
    const config = loadConfig();
    return {
      path: "/tmp/vora-config.json5",
      exists: true,
      raw: "{}",
      parsed: config,
      resolved: config,
  ...
  resolveStateDir.mockReturnValue("/tmp/vora-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation((cfg: VoraConfig) => ({ config: cfg }));
  recordPluginInstall.mockImplementation((cfg: VoraConfig) => cfg);
  buildPluginStatusReport.mockReturnValue({
    plugins: [],
    diagnostics: [],
  });
  applyExclusiveSlotSelection.mockImplementation(({ config }: { config: VoraConfig }) => ({
    config,
    warnings: [],
  }));
  uninstallPlugin.mockResolvedValue({
    ok: true,
    config: {} as VoraConfig,
    warnings: [],
    actions: {
      entry: false,
      install: false,
  ...
  recordHookInstall.mockImplementation((cfg: VoraConfig) => cfg);
}
