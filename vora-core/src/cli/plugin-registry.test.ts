...
vi.mock("../plugins/loader.js", () => ({
  loadVoraPlugins: mocks.loadVoraPlugins,
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("reloads when escalating from configured-channels to channels", async () => {
    ...
    expect(mocks.loadVoraPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadVoraPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not treat a pre-seeded partial registry as all scope", async () => {
    ...
    expect(mocks.loadVoraPlugins).toHaveBeenCalledTimes(1);
    expect(mocks.loadVoraPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        config,
        throwOnLoadError: true,
