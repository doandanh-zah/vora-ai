...
function warnInlinePasswordFlag() {
  defaultRuntime.error(
    "Warning: --password can be exposed via process listings. Prefer --password-file or VORA_GATEWAY_PASSWORD.",
  );
}
...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile = process.env.VORA_PROFILE?.trim().toLowerCase() === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.VORA_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.VORA_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.VORA_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (process.env.VORA_SERVICE_MARKER?.trim()) {
  ...
  if (!opts.allowUnconfigured && mode !== "local") {
    if (!configExists) {
      defaultRuntime.error(
        `Missing config. Run \`${formatCliCommand("vora setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
      );
    } else {
      defaultRuntime.error(
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or VORA_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (
    ...
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or VORA_GATEWAY_TOKEN/VORA_GATEWAY_PASSWORD) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (opts.background) {
    if (process.platform !== "win32") {
      gatewayLog.info("Background mode is only supported on Windows");
      gatewayLog.info("On Unix systems, use nohup or systemd/launchd services");
      defaultRuntime.exit(1);
      return;
    }

    gatewayLog.info("Starting gateway in background mode...");

    // Build command line arguments without --background
    const args = process.argv.slice(2).filter((arg) => arg !== "--background");

    // Spawn new process with hidden window on Windows
    const child = spawn(
      "powershell",
      [
        "-Command",
        `Start-Process -WindowStyle Hidden node "${process.execPath}" gateway ${args.join(" ")}"`,
      ],
      {
        detached: true,
        stdio: "ignore",
    ...
    gatewayLog.info(`Gateway started in background (PID: ${child.pid})`);
    gatewayLog.info("Use 'vora gateway status' to check if it's running");
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("vora gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: VORA_GATEWAY_TOKEN env if set)",
    )
    .option(
      "--auth <mode>",
    ...
    .option("--background", "Run gateway in background (hidden window)", false)
