      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["vora gateway run", "Run the gateway in the foreground."],
            ["vora gateway status", "Show service status and probe reachability."],
            ["vora gateway discover", "Find local and wide-area gateway beacons."],
            ["vora gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.vora.ai/cli/gateway")}\n`,
      ),
  );
