...
export function resolveMissingBrowserCommandMessage(config?: VoraConfig): string | null {
  const allow =
  ...
  if (allow.length > 0 && !allow.includes("browser")) {
    return (
      'The `vora browser` command is unavailable because `plugins.allow` excludes "browser". ' +
      'Add "browser" to `plugins.allow` if you want the bundled browser CLI and tool.'
    );
  }
  if (config?.plugins?.entries?.browser?.enabled === false) {
    return (
      "The `vora browser` command is unavailable because `plugins.entries.browser.enabled=false`. " +
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? process.env.VORA_CONTAINER?.trim() ?? null;
  ...
  try {
    ...
    process.on("uncaughtException", (error) => {
      console.error("[vora] Uncaught exception:", formatUncaughtError(error));
      process.exit(1);
    });
