...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.vora${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.VORA_PROFILE = profile;

  const stateDir = env.VORA_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.VORA_STATE_DIR?.trim()) {
    env.VORA_STATE_DIR = stateDir;
  }

  if (!env.VORA_CONFIG_PATH?.trim()) {
    env.VORA_CONFIG_PATH = path.join(stateDir, "vora.json");
  }

  if (profile === "dev" && !env.VORA_GATEWAY_PORT?.trim()) {
    env.VORA_GATEWAY_PORT = "19001";
  }
}
