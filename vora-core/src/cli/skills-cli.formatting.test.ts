...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "vora-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "vora-bundled-skills-test-"));
    process.env.VORA_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
