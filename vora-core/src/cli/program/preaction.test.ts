...
beforeEach(() => {
  ...
  originalHideBanner = process.env.VORA_HIDE_BANNER;
  ...
  delete process.env.VORA_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.VORA_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "vora", "status", "--debug"],
    });
    ...
    expect(processTitleSetSpy).toHaveBeenCalledWith("vora-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "vora", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    ...
    await runPreAction({
      ...
      processArgv: ["node", "vora", "channels", "add"],
    });
  ...
  it("only allows invalid config for explicit Matrix reinstall requests", async () => {
    await runPreAction({
      parseArgv: ["plugins", "install", "@vora/matrix"],
      processArgv: ["node", "vora", "plugins", "install", "@vora/matrix"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "vora", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "vora", "plugins", "install", MATRIX_REPO_INSTALL_SPEC],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@vora/matrix", "--marketplace", "local/repo"],
      processArgv: [
        "node",
        "vora",
        "plugins",
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    ...
    vi.clearAllMocks();
    process.env.VORA_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "vora", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "vora", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "vora", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "vora", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "vora", "agents", "list", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "vora", "config", "set", "gateway.auth.mode", "local", "--json"],
    });
    ...
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "vora", "agents", "list"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "vora", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "vora", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for config schema", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "vora", "config", "schema"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "vora", "backup", "create", "--json"],
    });
