...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["vora status", "Show channel health + session summary."],
          ["vora status --all", "Full diagnosis (read-only)."],
          ["vora status --json", "Machine-readable output."],
          ["vora status --usage", "Show model provider usage/quota snapshots."],
          [
            "vora status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["vora status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.vora.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.vora.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["vora sessions", "List all sessions."],
          ["vora sessions --agent work", "List sessions for one agent."],
          ["vora sessions --all-agents", "Aggregate sessions across agents."],
          ["vora sessions --active 120", "Only last 2 hours."],
          ["vora sessions --json", "Machine-readable output."],
          ["vora sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.vora.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["vora sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "vora sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["vora sessions cleanup --enforce", "Apply maintenance now."],
          ["vora sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["vora sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "vora sessions cleanup --enforce --store ./tmp/sessions.json",
