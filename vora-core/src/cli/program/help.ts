...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--container <name>",
      "Run the CLI inside a running Podman/Docker container named <name> (default: env VORA_CONTAINER)",
    )
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.vora-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates VORA_STATE_DIR/VORA_CONFIG_PATH under ~/.vora-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  if (
    ...
    console.log(
      commit ? `Vora ${ctx.programVersion} (${commit})` : `Vora ${ctx.programVersion}`,
    );
    process.exit(0);
  }
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.vora.ai/cli");
