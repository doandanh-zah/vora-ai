...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      VORA_GATEWAY_PORT: String(params.port),
      ...(params.token ? { VORA_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.VORA_STATE_DIR = "/tmp/vora-cli-state";
    process.env.VORA_CONFIG_PATH = "/tmp/vora-cli-state/vora.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        VORA_PROFILE: "dev",
        VORA_STATE_DIR: "/tmp/vora-daemon-state",
        VORA_CONFIG_PATH: "/tmp/vora-daemon-state/vora.json",
        VORA_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.vora.gateway.plist",
    });
