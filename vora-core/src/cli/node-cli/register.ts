    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            `vora node run --host 127.0.0.1 --port ${DEFAULT_GATEWAY_PORT}`,
            "Run the node host in the foreground.",
          ],
          ["vora node status", "Check node host service status."],
          ["vora node install", "Install the node host service."],
          ["vora node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.vora.ai/cli/node")}\n`,
    );
