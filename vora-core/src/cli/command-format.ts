...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+vora\b|^vora\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE =
  /^(?:pnpm|npm|bunx|npx)\s+vora\b.*(?:^|\s)update(?:\s|$)|^vora\b.*(?:^|\s)update(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const container = env.VORA_CONTAINER_HINT?.trim();
  const profile = normalizeProfileName(env.VORA_PROFILE);
