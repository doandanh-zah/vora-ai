...
export function isCommandEnabled(cfg: VoraConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: VoraConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as VoraConfig),
...
export function getCommandDetection(_cfg?: VoraConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: VoraConfig) {
