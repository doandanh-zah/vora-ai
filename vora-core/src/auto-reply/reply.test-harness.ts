...
type HomeEnvSnapshot = {
  ...
  VORA_STATE_DIR: string | undefined;
  VORA_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    VORA_AGENT_DIR: process.env.VORA_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".vora", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.VORA_STATE_DIR = path.join(home, ".vora");
    process.env.VORA_AGENT_DIR = path.join(home, ".vora", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".vora", "agent");
...
export function installReplyRuntimeMocks(mocks: ReplyRuntimeMocks) {
  vi.mock("../agents/vora-embedded-runner.js", () => ({
    ...
    runEmbeddedVoraAgent: (...args: unknown[]) => mocks.runEmbeddedVoraAgent(...args),
...
export function resetReplyRuntimeMocks(mocks: ReplyRuntimeMocks) {
  mocks.runEmbeddedVoraAgent.mockClear();
