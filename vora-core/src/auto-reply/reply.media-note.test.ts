...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(
    async (home) => {
      agentMocks.runEmbeddedVoraAgent.mockClear();
      return await fn(home);
    },
    {
      env: {
        VORA_BUNDLED_SKILLS_DIR: (home) => path.join(home, "bundled-skills"),
      },
      prefix: "vora-media-note-",
    },
  );
}
...
describe("getReplyFromConfig media note plumbing", () => {
  ...
  it("includes all MediaPaths in the agent prompt", async () => {
    await withTempHome(async (home) => {
      ...
      agentMocks.runEmbeddedVoraAgent.mockImplementation(async (params) => {
