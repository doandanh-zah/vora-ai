...
const { withTempHome } = createTempHomeHarness({ prefix: "vora-rawbody-" });
...
describe("RawBody directive parsing", () => {
  ...
  it("handles directives and history in the prompt", async () => {
    await withTempHome(async (home) => {
      agentMocks.runEmbeddedVoraAgent.mockResolvedValue(makeEmbeddedTextResult("ok"));
      ...
      const res = await getReplyFromConfig(
        groupMessageCtx,
        {},
        makeReplyConfig(home) as VoraConfig,
      );

      const text = Array.isArray(res) ? res[0]?.text : res?.text;
      expect(text).toBe("ok");
      expect(agentMocks.runEmbeddedVoraAgent).toHaveBeenCalledOnce();
      const prompt =
        (agentMocks.runEmbeddedVoraAgent.mock.calls[0]?.[0] as { prompt?: string } | undefined)
          ?.prompt ?? "";
