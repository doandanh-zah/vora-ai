...
const piEmbeddedMocks = getSharedMocks("vora.trigger-handling.pi-embedded-mocks", () => ({
  abortEmbeddedPiRun: vi.fn().mockReturnValue(false),
  compactEmbeddedPiSession: vi.fn(),
  runEmbeddedVoraAgent: vi.fn(),
...
export function getRunEmbeddedVoraAgentMock(): AnyMock {
  return piEmbeddedMocks.runEmbeddedVoraAgent;
}
...
const installPiEmbeddedMock = () =>
  vi.doMock("../agents/vora-embedded-runner.js", () => ({
    ...
    runEmbeddedVoraAgent: (...args: unknown[]) => piEmbeddedMocks.runEmbeddedVoraAgent(...args),
...
const modelCatalogMocks = getSharedMocks("vora.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("vora.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("vora.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    voraHome: process.env.VORA_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("VORA_HOME", snapshot.voraHome);
  restoreKey("VORA_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.VORA_STATE_DIR = join(home, ".vora");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "vora-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".vora", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);

  try {
    // Hard reset shared mocks so non-isolated runs don't inherit prior behavior.
    piEmbeddedMocks.runEmbeddedVoraAgent.mockReset();
...
export function makeCfg(home: string): VoraConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "vora"),
...
export async function expectInlineCommandHandledAndStripped(params: {
  ...
  const runEmbeddedVoraAgentMock = mockRunEmbeddedVoraAgentOk();
  ...
  expect(runEmbeddedVoraAgentMock).toHaveBeenCalled();
  const prompt = runEmbeddedVoraAgentMock.mock.calls.at(-1)?.[0]?.prompt ?? "";
...
export async function runGreetingPromptForBareNewOrReset(params: {
  ...
  const runEmbeddedVoraAgentMock = getRunEmbeddedVoraAgentMock();
  runEmbeddedVoraAgentMock.mockClear();
  ...
  expect(runEmbeddedVoraAgentMock).toHaveBeenCalledOnce();
  const prompt = runEmbeddedVoraAgentMock.mock.calls.at(-1)?.[0]?.prompt ?? "";
  expect(prompt).toContain("A new session was started via /new or /reset");
  expect(prompt).toContain("Execute your Session Startup sequence");
}
...
export function mockRunEmbeddedVoraAgentOk(text = "ok"): AnyMock {
  const runEmbeddedVoraAgentMock = getRunEmbeddedVoraAgentMock();
  runEmbeddedVoraAgentMock.mockResolvedValue({
    payloads: [{ text }],
    meta: {
  ...
  return runEmbeddedVoraAgentMock;
}
