...
const FOLLOWUP_DEBUG = process.env.VORA_DEBUG_FOLLOWUP_RUNNER_TEST === "1";
...
beforeEach(async () => {
  ...
  runEmbeddedVoraAgentMock.mockReset();
...
function mockCompactionRun(params: {
  ...
  runEmbeddedVoraAgentMock.mockImplementationOnce(
    async (args: {
...
describe("createFollowupRunner compaction", () => {
  it("adds verbose auto-compaction notice and tracks count", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "vora-compaction-")),
      "sessions.json",
    );
  ...
  it("tracks auto-compaction from embedded result metadata even when no compaction event is emitted", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "vora-compaction-meta-")),
      "sessions.json",
    );
    const sessionEntry: SessionEntry = {
      sessionId: "session",
  ...
  it("refreshes queued followup runs to the rotated transcript", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "vora-compaction-queue-")),
      "sessions.json",
    );
    const sessionEntry: SessionEntry = {
      sessionId: "session",
  ...
  it("does not count failed compaction end events in followup runs", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "vora-compaction-failed-")),
      "sessions.json",
    );
    const sessionEntry: SessionEntry = {
      sessionId: "session",
    ...
    runEmbeddedVoraAgentMock.mockImplementationOnce(async (args) => {
      args.onAgentEvent?.({
        stream: "compaction",
  ...
  it("injects the post-compaction refresh prompt before followup runs after preflight compaction", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(tmpdir(), "vora-preflight-followup-"));
...
describe("createFollowupRunner bootstrap warning dedupe", () => {
  it("passes stored warning signature history to embedded followup runs", async () => {
    ...
    const call = runEmbeddedVoraAgentMock.mock.calls.at(-1)?.[0] as
      | {
...
describe("createFollowupRunner messaging tool dedupe", () => {
  ...
  it("persists usage even when replies are suppressed", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "vora-followup-usage-")),
      "sessions.json",
    );
  ...
  it("passes queued config into usage persistence during drained followups", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "vora-followup-usage-cfg-")),
      "sessions.json",
    );
    const sessionKey = "main";
    const sessionEntry: SessionEntry = { sessionId: "session", updatedAt: Date.now() };
...
describe("createFollowupRunner typing cleanup", () => {
  ...
  it("calls both markRunComplete and markDispatchIdle on agent error", async () => {
    const typing = createMockTypingController();
    runEmbeddedVoraAgentMock.mockRejectedValueOnce(new Error("agent exploded"));
...
describe("createFollowupRunner agentDir forwarding", () => {
  it("passes queued run agentDir to runEmbeddedVoraAgent", async () => {
    ...
    const call = runEmbeddedVoraAgentMock.mock.calls.at(-1)?.[0] as { agentDir?: string };
