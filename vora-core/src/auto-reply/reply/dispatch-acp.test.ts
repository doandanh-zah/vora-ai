...
const policyMocks = vi.hoisted(() => ({
  resolveAcpDispatchPolicyError: vi.fn<(cfg: VoraConfig) => AcpRuntimeError | null>(() => null),
  resolveAcpAgentPolicyError: vi.fn<(cfg: VoraConfig, agent: string) => AcpRuntimeError | null>(
    () => null,
  ),
}));
...
const sessionMetaMocks = vi.hoisted(() => ({
  readAcpSessionEntry: vi.fn<
    (params: { sessionKey: string; cfg?: VoraConfig }) => AcpSessionStoreEntry | null
  >(() => null),
}));
...
function createAcpConfigWithVisibleToolTags(): VoraConfig {
...
describe("tryDispatchAcpReply", () => {
  beforeEach(async () => {
    ...
    vi.doMock("../../acp/policy.js", () => ({
      resolveAcpDispatchPolicyError: (cfg: VoraConfig) =>
        policyMocks.resolveAcpDispatchPolicyError(cfg),
      resolveAcpAgentPolicyError: (cfg: VoraConfig, agent: string) =>
    ...
    vi.doMock("../../acp/runtime/session-meta.js", () => ({
      readAcpSessionEntry: (params: { sessionKey: string; cfg?: VoraConfig }) =>
  ...
  it("uses canonical session keys for bound-session identity notices", async () => {
    ...
    sessionMetaMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: VoraConfig }) =>
        params.sessionKey === canonicalSessionKey
          ? {
              cfg: params.cfg ?? createAcpTestConfig(),
              storePath: "/tmp/vora-session-store.json",
              sessionKey: canonicalSessionKey,
