...
const { createVoraCodingToolsMock } = vi.hoisted(() => ({
  createVoraCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(async () => {
    vi.restoreAllMocks();
    vi.resetModules();
    createVoraCodingToolsMock.mockClear();
    createVoraCodingToolsMock.mockReturnValue([]);
    const piTools = await import("../../agents/vora-tools.js");
    vi.spyOn(piTools, "createVoraCodingTools").mockImplementation(
      createVoraCodingToolsMock,
    );
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createVoraCodingToolsMock).toHaveBeenCalledWith(
