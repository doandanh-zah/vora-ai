...
const runEmbeddedVoraAgentMock = vi.fn();
...
vi.mock("../../agents/vora-embedded-runner.js", async () => {
  const actual = await vi.importActual<typeof import("../../agents/vora-embedded-runner.js")>(
    "../../agents/vora-embedded-runner.js",
  );
  return {
    ...actual,
    queueEmbeddedPiMessage: vi.fn().mockReturnValue(false),
    runEmbeddedVoraAgent: (params: unknown) => runEmbeddedVoraAgentMock(params),
  };
});
...
beforeEach(() => {
  runEmbeddedVoraAgentMock.mockClear();
...
describe("runReplyAgent authProfileId fallback scoping", () => {
  it("drops authProfileId when provider changes during fallback", async () => {
    ...
    runEmbeddedVoraAgentMock.mockResolvedValue({ payloads: [{ text: "ok" }], meta: {} });
    ...
    expect(runEmbeddedVoraAgentMock).toHaveBeenCalledTimes(1);
    const call = runEmbeddedVoraAgentMock.mock.calls[0]?.[0] as {
...
describe("runReplyAgent auto-compaction token update", () => {
  ...
  it("updates totalTokens after auto-compaction using lastCallUsage", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "vora-compact-tokens-"));
    ...
    runEmbeddedVoraAgentMock.mockImplementation(async (params: EmbeddedRunParams) => {
  ...
  it("tracks auto-compaction from embedded result metadata even when no compaction event is emitted", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "vora-compact-meta-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
    ...
    runEmbeddedVoraAgentMock.mockResolvedValue({
      payloads: [{ text: "done" }],
      meta: {
        agentMeta: {
  ...
  it("accumulates compactions across fallback attempts without double-counting a single attempt", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "vora-compact-fallback-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("does not count failed compaction end events from earlier fallback attempts", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "vora-compact-fallback-failed-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("updates totalTokens from lastCallUsage even without compaction", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "vora-usage-last-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("does not enqueue legacy post-compaction audit warnings", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "vora-no-audit-warning-"));
...
describe("runReplyAgent block streaming", () => {
  it("coalesces duplicate text_end block replies", async () => {
    ...
    runEmbeddedVoraAgentMock.mockImplementationOnce(async (params) => {
...
describe("runReplyAgent claude-cli routing", () => {
  ...
  it("uses claude-cli runner for claude-cli provider", async () => {
    ...
    expect(runEmbeddedVoraAgentMock).not.toHaveBeenCalled();
...
describe("runReplyAgent messaging tool suppression", () => {
  ...
  it("drops replies when a messaging tool sent via the same provider + target", async () => {
    runEmbeddedVoraAgentMock.mockResolvedValueOnce({
  ...
  it("persists usage fields even when replies are suppressed", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(os.tmpdir(), "vora-session-store-")),
      "sessions.json",
    );
    const sessionKey = "main";
...
describe("runReplyAgent fallback reasoning tags", () => {
  ...
  it("enforces <final> when the fallback provider requires reasoning tags", async () => {
    ...
    const call = runEmbeddedVoraAgentMock.mock.calls[0]?.[0] as EmbeddedVoraAgentParams | undefined;
    expect(call?.enforceFinalTag).toBe(true);
  });

  it("enforces <final> during memory flush on fallback providers", async () => {
    runEmbeddedVoraAgentMock.mockImplementation(async (params: EmbeddedVoraAgentParams) => {
    ...
    const flushCall = runEmbeddedVoraAgentMock.mock.calls.find(([params]) =>
      (params as EmbeddedVoraAgentParams | undefined)?.prompt?.includes(
        "Pre-compaction memory flush.",
      ),
    )?.[0] as EmbeddedVoraAgentParams | undefined;
...
describe("runReplyAgent transient HTTP retry", () => {
  it("retries once after transient 521 HTML failure and then succeeds", async () => {
    ...
    expect(runEmbeddedVoraAgentMock).toHaveBeenCalledTimes(2);
...
describe("runReplyAgent billing error classification", () => {
  ...
  it("returns billing message for mixed-signal error (billing text + overflow patterns)", async () => {
    runEmbeddedVoraAgentMock.mockRejectedValueOnce(
