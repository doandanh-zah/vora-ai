...
function getDiscordExecApprovalApproversForTests(params: { cfg: VoraConfig }): string[] {
...
const { abortEmbeddedPiRun, compactEmbeddedPiSession } =
  await import("../../agents/vora-embedded-runner.js");
...
function listConfiguredTelegramAccountIds(cfg: VoraConfig): string[] {
...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-commands-"));
...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-commands-config-"));
  const configPath = path.join(dir, "vora.json");
  const previous = process.env.VORA_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.VORA_CONFIG_PATH = previous;
    }
    await fs.rm(dir, {
      recursive: true,
      force: true,
...
function buildParams(commandBody: string, cfg: VoraConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/vora-session-store.json" },
    } as VoraConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/vora-agent-compact";
      ...
      },
      {
        ...
        run: async () => {
          await withTempConfigPath({ messages: { ackReaction: ":)" } }, async (configPath) => {
            ...
            const written = await readJsonFile<VoraConfig>(configPath);
