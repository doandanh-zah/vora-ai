...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "vora-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop vora!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@vora_bot", { botUsername: "vora_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@vora_bot", { botUsername: "vora_bot" })).toBe(true);
