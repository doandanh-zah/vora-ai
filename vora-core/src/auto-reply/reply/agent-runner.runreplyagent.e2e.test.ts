...
const state = vi.hoisted(() => ({
  ...
  runEmbeddedVoraAgentMock: vi.fn(),
...
vi.mock("../../agents/vora-embedded-runner.js", () => ({
  ...
  runEmbeddedVoraAgent: (params: unknown) => state.runEmbeddedVoraAgentMock(params),
}));
...
describe("runReplyAgent heartbeat followup guard", () => {
  it("drops heartbeat runs when another run is active", async () => {
    ...
    expect(state.runEmbeddedVoraAgentMock).not.toHaveBeenCalled();
  ...
  it("drains followup queue when an unexpected exception escapes the run path", async () => {
    ...
    state.runEmbeddedVoraAgentMock.mockResolvedValueOnce({
      payloads: [{ text: "ok" }],
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  it("signals typing for normal runs", async () => {
    ...
    state.runEmbeddedVoraAgentMock.mockImplementationOnce(async (params: AgentRunParams) => {
  ...
  it("retries transient HTTP failures once with timer-driven backoff", async () => {
    vi.useFakeTimers();
    let calls = 0;
    state.runEmbeddedVoraAgentMock.mockImplementation(async () => {
      calls += 1;
      if (calls === 1) {
  ...
  it("announces model fallback only once per active fallback state", async () => {
    ...
    state.runEmbeddedVoraAgentMock.mockResolvedValue({
      payloads: [{ text: "final" }],
      meta: {},
    });
  ...
  it("retries after compaction failure by resetting the session", async () => {
    await withTempStateDir(async (stateDir) => {
      ...
      state.runEmbeddedVoraAgentMock.mockImplementationOnce(async () => {
        throw new Error(
      ...
      expect(state.runEmbeddedVoraAgentMock).toHaveBeenCalledTimes(1);
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "vora-memory-flush-"));
  });
  ...
  it("runs preflight compaction when transcript-estimated tokens cross the threshold", async () => {
    await withTempStore(async (storePath) => {
      ...
      state.runEmbeddedVoraAgentMock.mockImplementation(async (params: EmbeddedRunParams) => {
        calls.push({
  ...
  it("skips memory flush when disabled in config", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(state.runEmbeddedVoraAgentMock).toHaveBeenCalledTimes(1);
      const call = state.runEmbeddedVoraAgentMock.mock.calls[0]?.[0] as
        | { prompt?: string }
        | undefined;
      expect(call?.prompt).toBe("hello");
