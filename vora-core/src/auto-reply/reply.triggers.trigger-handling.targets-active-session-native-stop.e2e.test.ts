...
import {
  ...
  getRunEmbeddedVoraAgentMock,
  installTriggerHandlingReplyHarness,
  MAIN_SESSION_KEY,
  makeCfg,
  mockRunEmbeddedVoraAgentOk,
...
vi.mock("./reply/agent-runner.runtime.js", () => ({
  runReplyAgent: async (params: {
    ...
    try {
      const result = await runEmbeddedVoraAgentMock({
...
function mockEmbeddedOkPayload() {
  return mockRunEmbeddedVoraAgentOk("ok");
}
...
describe("trigger handling", () => {
  ...
  for (const testCase of [
    ...
    it(`surfaces agent error: ${testCase.error}`, async () => {
      await withTempHome(async (home) => {
        ...
        runEmbeddedVoraAgentMock.mockImplementation(async () => {
        ...
        expect(runEmbeddedVoraAgentMock, testCase.error).toHaveBeenCalledOnce();
      });
    });
  }
  ...
  it("sanitizes thinking directives before the agent run", async () => {
    await withTempHome(async (home) => {
      ...
      for (const testCase of thinkCases) {
        ...
        expect(runEmbeddedVoraAgentMock, testCase.label).toHaveBeenCalledOnce();
        if (testCase.assertPrompt) {
          const prompt = runEmbeddedVoraAgentMock.mock.calls[0]?.[0]?.prompt ?? "";
  ...
  it("resolves heartbeat model selection from overrides", async () => {
    await withTempHome(async (home) => {
      ...
      for (const testCase of modelCases) {
        ...
        const call = runEmbeddedVoraAgentMock.mock.calls[0]?.[0];
  ...
  it("applies native model changes to the target session", async () => {
    await withTempHome(async (home) => {
      ...
      expect(runEmbeddedVoraAgentMock).toHaveBeenCalledOnce();
      expect(runEmbeddedVoraAgentMock.mock.calls[0]?.[0]).toEqual(
  ...
  it("handles bare session reset, inline commands, and unauthorized inline status", async () => {
    await withTempHome(async (home) => {
      ...
      const inlineRunEmbeddedVoraAgentMock = mockEmbeddedOk();
      ...
      const text = Array.isArray(res) ? res[0]?.text : res?.text;
      expect(text).toBe("ok");
      expect(inlineRunEmbeddedVoraAgentMock).toHaveBeenCalled();
      const prompt = inlineRunEmbeddedVoraAgentMock.mock.calls.at(-1)?.[0]?.prompt ?? "";
