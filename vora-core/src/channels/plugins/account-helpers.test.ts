...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): VoraConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as VoraConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedDefaultAccountCase(input: VoraConfig, expected: string) {
