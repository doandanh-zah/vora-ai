...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: VoraConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: VoraConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: VoraConfig, policy: DmPolicy, accountId?: string) => VoraConfig;
