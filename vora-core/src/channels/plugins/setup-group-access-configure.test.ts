...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: VoraConfig, policy: ChannelAccessPolicy) => VoraConfig;
  resolveAllowlist: (params: { cfg: VoraConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: VoraConfig; resolved: TResolved }) => VoraConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: VoraConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: VoraConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: VoraConfig, policy: ChannelAccessPolicy): VoraConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: VoraConfig, policy: ChannelAccessPolicy): VoraConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: VoraConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: VoraConfig; resolved: string[] }) => {
      calls.push("apply");
