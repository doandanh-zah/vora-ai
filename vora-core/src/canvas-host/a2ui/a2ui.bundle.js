...
var VoraA2UIHost = class extends i$6 {
	static properties = {
		surfaces: { state: true },
    ...
    :host {
      ...
      box-sizing: border-box;
      padding:
        var(--vora-a2ui-inset-top, 0px)
        var(--vora-a2ui-inset-right, 0px)
        var(--vora-a2ui-inset-bottom, 0px)
        var(--vora-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--vora-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--vora-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--vora-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--vora-a2ui-empty-top, var(--vora-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  `;
	connectedCallback() {
		...
		for (const eventName of ["vora:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["vora:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		globalThis.__voraLastA2UIAction = userAction;
		const handler = globalThis.webkit?.messageHandlers?.voraCanvasA2UIAction ?? globalThis.voraCanvasA2UIAction;
		if (handler?.postMessage) try {
			if (handler === globalThis.voraCanvasA2UIAction) handler.postMessage(JSON.stringify({ userAction }));
...
if (!customElements.get("vora-a2ui-host")) customElements.define("vora-a2ui-host", VoraA2UIHost);
