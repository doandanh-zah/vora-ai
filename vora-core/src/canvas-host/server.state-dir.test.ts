...
describe("canvas host state dir defaults", () => {
  ...
  it("uses VORA_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("vora-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Vora Canvas");
      } finally {
        await handler.close();
      }
    });
