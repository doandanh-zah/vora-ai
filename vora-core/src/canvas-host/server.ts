...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.voraCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.voraCanvasA2UIAction &&
        typeof window.voraCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.voraSendUserAction === "function";
  ...
  window.addEventListener("vora:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Vora node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.voraSendUserAction === "function"
        ? window.voraSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.VORA_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.VORA_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Vora Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
