...
export type PromptDefaultModelResult = { model?: string; config?: VoraConfig };
...
function resolveConfiguredModelRaw(cfg: VoraConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: VoraConfig): string[] {
...
export function applyModelAllowlist(cfg: VoraConfig, models: string[]): VoraConfig {
