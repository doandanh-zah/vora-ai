...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<VoraConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<VoraConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: VoraConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: VoraConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: VoraConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: VoraConfig, result: VoraConfig): VoraConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: VoraConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as VoraConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with vora configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Vora will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.vora.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Vora will store a reference instead of the API key.",
