...
import {
  ...
  VORA_NODE_EXTRA_CA_CERTS_READY,
  VORA_NODE_OPTIONS_READY,
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(plan?.env[VORA_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(plan?.env[VORA_NODE_OPTIONS_READY]).toBe("1");
  });
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "vora", "gateway", "run"],
        env: {
          [VORA_NODE_EXTRA_CA_CERTS_READY]: "1",
          [VORA_NODE_OPTIONS_READY]: "1",
        },
