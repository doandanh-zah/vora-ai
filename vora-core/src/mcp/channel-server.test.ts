...
import { z } from "zod";
import { createVoraChannelMcpServer, VoraChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createVoraChannelMcpServer({
...
describe("vora channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new VoraChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });
