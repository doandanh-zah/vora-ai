...
export class VoraChannelBridge {
  ...
  constructor(
    private readonly cfg: VoraConfig,
  ...
  async start(): Promise<void> {
    ...
    const gatewayUrlOverrideSource =
      connection.urlSource === "cli --url"
        ? "cli"
        : connection.urlSource === "env VORA_GATEWAY_URL"
          ? "env"
          : undefined;
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`vora mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `vora mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
