...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.VORA_GATEWAY_PORT).toBe("18789");
    expect(env.VORA_GATEWAY_TOKEN).toBeUndefined();
    expect(env.VORA_SERVICE_MARKER).toBe("vora");
    expect(env.VORA_SERVICE_KIND).toBe("gateway");
    expect(typeof env.VORA_SERVICE_VERSION).toBe("string");
    expect(env.VORA_SYSTEMD_UNIT).toBe("vora-gateway.service");
    expect(env.VORA_WINDOWS_TASK_NAME).toBe("Vora Gateway");
    if (process.platform === "darwin") {
      expect(env.VORA_LAUNCHD_LABEL).toBe("ai.vora.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", VORA_PROFILE: "work" },
      port: 18789,
    });
    expect(env.VORA_SYSTEMD_UNIT).toBe("vora-gateway-work.service");
    expect(env.VORA_WINDOWS_TASK_NAME).toBe("Vora Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.VORA_LAUNCHD_LABEL).toBe("ai.vora.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through VORA_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", VORA_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.VORA_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits VORA_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        VORA_GATEWAY_TOKEN: "   ",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".vora"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", VORA_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".vora-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", VORA_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".vora"));
  });

  it("uses VORA_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", VORA_STATE_DIR: "/var/lib/vora" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/vora"));
  });

  it("expands ~ in VORA_STATE_DIR", () => {
    const env = { HOME: "/Users/test", VORA_STATE_DIR: "~/vora-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/vora-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { VORA_STATE_DIR: "C:\\State\\vora" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\vora");
  });
});
