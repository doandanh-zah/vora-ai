...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds VORA_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `vora gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service VORA_GATEWAY_TOKEN does not match gateway.auth.token in vora.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.VORA_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.VORA_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `vora gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
