...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.VORA_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    VORA_PROFILE: profile,
    VORA_GATEWAY_PORT: String(port),
    VORA_LAUNCHD_LABEL: resolvedLaunchdLabel,
    VORA_SYSTEMD_UNIT: systemdUnit,
    VORA_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    VORA_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    VORA_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = env.VORA_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    VORA_GATEWAY_TOKEN: gatewayToken,
    VORA_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    VORA_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    VORA_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    VORA_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    VORA_LOG_PREFIX: "node",
    VORA_SERVICE_MARKER: NODE_SERVICE_MARKER,
    VORA_SERVICE_KIND: NODE_SERVICE_KIND,
    VORA_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    VORA_STATE_DIR: sharedEnv.stateDir,
    VORA_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.VORA_STATE_DIR;
  const configPath = env.VORA_CONFIG_PATH;
