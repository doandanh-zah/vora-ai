...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `vora-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      VORA_LAUNCHD_LABEL: `ai.vora.launchd-int-${testId}`,
      VORA_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
