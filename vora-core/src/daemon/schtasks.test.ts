...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when VORA_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".vora", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when VORA_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", VORA_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".vora-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers VORA_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        VORA_PROFILE: "rescue",
        VORA_STATE_DIR: "C:\\State\\vora",
      },
      expected: path.join("C:\\State\\vora", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", VORA_PROFILE: "default" },
      expected: path.join("/home/test", ".vora", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\vora",
          environment: {
            NODE_ENV: "production",
            VORA_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\vora\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\vora\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Vora Share\\node.exe" "\\\\fileserver\\Vora Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Vora Share\\node.exe",
            "\\\\fileserver\\Vora Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from VORA_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ VORA_STATE_DIR: path.join(tmpDir, "custom-state") }),
