...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["VORA_CONTAINER_HINT", "VORA_PROFILE"]);
    delete process.env.VORA_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:vora|vora) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
