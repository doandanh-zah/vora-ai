...
let originalVoraLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.VORA_TEST_FILE_LOG;
  originalVoraLogLevel = process.env.VORA_LOG_LEVEL;
  delete process.env.VORA_TEST_FILE_LOG;
  delete process.env.VORA_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.VORA_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalVoraLogLevel === undefined) {
    delete process.env.VORA_LOG_LEVEL;
  } else {
    process.env.VORA_LOG_LEVEL = originalVoraLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
