...
type FetchConfig = NonNullable<VoraConfig["tools"]>["web"] extends infer Web
...
function hasCustomWebSearchPluginRisk(config: VoraConfig): boolean {
...
function setResolvedXSearchApiKey(params: { resolvedConfig: VoraConfig; value: string }): void {
  const tools = ensureObject(params.resolvedConfig as Record<string, unknown>, "tools");
  const web = ensureObject(tools, "web");
