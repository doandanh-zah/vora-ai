...
function asConfig(value: unknown): VoraConfig {
...
function createOpenAIFileRuntimeConfig(secretFile: string): VoraConfig {
  return asConfig({
    secrets: {
      providers: {
        default: {
...
describe("secrets runtime snapshot integration", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "VORA_DISABLE_PLUGIN_DISCOVERY_CACHE",
      "VORA_VERSION",
    ]);
    delete process.env.VORA_BUNDLED_PLUGINS_DIR;
    process.env.VORA_DISABLE_PLUGIN_DISCOVERY_CACHE = "1";
    delete process.env.VORA_VERSION;
  });
  ...
  it("activates runtime snapshots for loadConfig and ensureAuthProfileStore", async () => {
    await withEnvAsync(
      {
        VORA_BUNDLED_PLUGINS_DIR: undefined,
        VORA_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
        VORA_VERSION: undefined,
      },
      async () => {
        ...
        expect(
          ensureAuthProfileStore("/tmp/vora-agent-main").profiles["openai:default"],
        ).toMatchObject({
          type: "api_key",
          key: "sk-runtime",
        });
  ...
  it("keeps active secrets runtime snapshots resolved after config writes", async () => {
    ...
    await withTempHome("vora-secrets-runtime-write-", async (home) => {
  ...
  it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
    ...
    await withTempHome("vora-secrets-runtime-refresh-fail-", async (home) => {
      const { secretFile, agentDir } = await createOpenAIFileRuntimeFixture(home);
  ...
  it(
    ...
    async () => {
      await withTempHome("vora-secrets-runtime-gateway-auth-reload-lkg-", async (home) => {
        ...
        const persistedConfig = JSON.parse(
          await fs.readFile(path.join(home, ".vora", "vora.json"), "utf8"),
        ) as VoraConfig;
  ...
  it(
    ...
    async () => {
      await withTempHome("vora-secrets-runtime-web-reload-lkg-", async (home) => {
        const prepared = await prepareSecretsRuntimeSnapshot({
          config: asConfig({
            tools: {
              web: {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("vora-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".vora", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".vora", "agents", "ops", "agent");
