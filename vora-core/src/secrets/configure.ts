...
function getSecretProviders(config: VoraConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: VoraConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: VoraConfig): Array<{ value: SecretRefSource; label: string }> {
...
function configureCandidateKey(candidate: {
  configFile: "vora.json" | "auth-profiles.json";
  path: string;
  agentId?: string;
}): string {
...
function resolveConfigureAgentId(config: VoraConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: VoraConfig): Promise<void> {
  while (true) {
  ...
  if (!params.providersOnly) {
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "vora.json",
