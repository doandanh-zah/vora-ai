...
`;
    ...
    expect(parsed.vora.emoji).toBe("💾");
    expect(parsed.vora.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.vora.emoji).toBe("📝");
    expect(parsed.vora.events).toEqual(["command"]);
    expect(parsed.vora.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.vora.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"vora": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"vora": {"events": ["test"]}}');
  });
...
describe("resolveVoraMetadata", () => {
  it("extracts vora metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveVoraMetadata(frontmatter);
  ...
  it("returns undefined when vora key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        vora: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Vora" },
            { id: "npm", kind: "npm", package: "@vora/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@vora/hook");
  });
    ...
    "vora":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Vora" }],
      },
  }
---
...
`;
    ...
    const vora = resolveVoraMetadata(frontmatter);
    expect(vora).toBeDefined();
    expect(vora?.emoji).toBe("💾");
    expect(vora?.events).toEqual(["command:new", "command:reset"]);
    expect(vora?.requires?.config).toEqual(["workspace.dir"]);
    expect(vora?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const vora = resolveVoraMetadata(frontmatter);
    expect(vora?.emoji).toBe("disk");
    expect(vora?.events).toEqual(["command:new"]);
  });
});
