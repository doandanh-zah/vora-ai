...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vora-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["VORA_BUNDLED_HOOKS_DIR"]);
  ...
  describe("loadInternalHooks", () => {
    ...
    const expectNoCommandHookRegistration = async (cfg: VoraConfig) => {
    ...
    it("should treat missing hooks.internal.enabled as enabled (default-on)", async () => {
      ...
      for (const cfg of [
        {} satisfies VoraConfig,
        { hooks: {} } satisfies VoraConfig,
        { hooks: { internal: {} } } satisfies VoraConfig,
      ]) {
        const count = await loadInternalHooks(cfg, tmpDir);
        expect(count).toBe(0);
      }
