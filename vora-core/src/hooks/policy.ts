import type { VoraConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "vora-bundled": {
    ...
    canOverride: ["vora-bundled"],
    canBeOverriddenBy: ["vora-managed", "vora-plugin"],
  },
  "vora-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["vora-bundled", "vora-plugin"],
    canBeOverriddenBy: ["vora-managed"],
  },
  "vora-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["vora-bundled", "vora-managed", "vora-plugin"],
    canBeOverriddenBy: ["vora-managed"],
  },
  "vora-workspace": {
    precedence: 40,
    ...
    canOverride: ["vora-workspace"],
    canBeOverriddenBy: ["vora-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "vora-plugin") {
    return { enabled: true };
  }
