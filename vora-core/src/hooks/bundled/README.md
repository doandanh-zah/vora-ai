...
This directory contains hooks that ship with Vora. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
- **Managed hooks**: `~/.vora/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.vora/vora.json`:
...
2. Restart gateway: `pkill -9 -f 'vora.*gateway' && pnpm vora gateway`
3. Enable the hook: `vora hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.vora.ai/automation/hooks
