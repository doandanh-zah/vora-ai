...
homepage: https://docs.vora.ai/automation/hooks#command-logger
metadata:
  {
    "vora":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.vora/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.vora/logs/commands.log ~/.vora/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/vora`:
   ```
   /home/username/.vora/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
