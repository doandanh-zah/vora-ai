...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["VORA_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["VORA_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["VORA_SERVICE_VERSION"],
...
// Single source of truth for the current Vora version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __VORA_VERSION__ === "string" ? __VORA_VERSION__ : undefined,
  bundledVersion: process.env.VORA_BUNDLED_VERSION,
});
