...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async (importOriginal) => {
    ...
    return {
      ...mod,
      ensureVoraModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};

let runEmbeddedVoraAgent: typeof import("./pi-embedded-runner/run.js").runEmbeddedVoraAgent;
...
beforeAll(async () => {
  ...
  ({ runEmbeddedVoraAgent } = await import("./pi-embedded-runner/run.js"));
  ({
...
async function runEmbeddedVoraAgentInline(
  params: Parameters<typeof runEmbeddedVoraAgent>[0],
): Promise<Awaited<ReturnType<typeof runEmbeddedVoraAgent>>> {
  return await runEmbeddedVoraAgent({
    ...params,
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): VoraConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): VoraConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function runAutoPinnedOpenAiTurn(params: {
  ...
  await runEmbeddedVoraAgentInline({
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedVoraAgent auth profile rotation", () => {
  ...
  it("does not reschedule copilot refresh after shutdown", async () => {
    ...
    try {
      ...
      const runPromise = runEmbeddedVoraAgentInline({
        sessionId: "session:test",
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `vora-auth-rotation-${Date.now()}.log`),
    });
  ...
  it("does not rotate for compaction timeouts", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
      ...
      const result = await runEmbeddedVoraAgentInline({
        sessionId: "session:test",
