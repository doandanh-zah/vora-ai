...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["VORA_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-subagent-"));
  ...
  it("uses isolated temp state when VORA_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "vora-test-state"));
  });
});
