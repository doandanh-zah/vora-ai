...
let createVoraTools: typeof import("./vora-tools.js").createVoraTools;

async function loadFreshVoraToolsModuleForTest() {
  vi.resetModules();
  vi.doMock("./tools/gateway.js", () => createGatewayToolModuleMocks());
  ({ createVoraTools } = await import("./vora-tools.js"));
}
...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshVoraToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-test-"));

    try {
      await withEnvAsync(
        { VORA_STATE_DIR: stateDir, VORA_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: vora --profile isolated doctor --non-interactive",
          );
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/vora" } } }',
      }),
    ).rejects.toThrow(
