    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "vora.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "vora.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "vora.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("vora-");
  const acpxRoot = await tempDirs.make("vora-acpx-plugin-");
  const helperRoot = await tempDirs.make("vora-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("vora-");
  const pluginRoot = await tempDirs.make("vora-plugin-");
  const outsideDir = await tempDirs.make("vora-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("vora-");
    const pluginRoot = await tempDirs.make("vora-claude-bundle-");
