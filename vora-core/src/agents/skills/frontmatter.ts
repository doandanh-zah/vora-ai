...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseVoraManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyVoraManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveVoraMetadata(
  frontmatter: ParsedSkillFrontmatter,
): VoraSkillMetadata | undefined {
  const metadataObj = resolveVoraManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveVoraManifestRequires(metadataObj);
  const install = resolveVoraManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveVoraManifestOs(metadataObj);
  return {
