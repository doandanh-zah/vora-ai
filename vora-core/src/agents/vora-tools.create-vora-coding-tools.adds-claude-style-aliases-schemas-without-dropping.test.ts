...
import { __testing, createVoraCodingTools } from "./vora-tools.js";
import { createVoraReadTool, createSandboxedReadTool } from "./vora-tools.read.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";

const defaultTools = createVoraCodingTools();
...
describe("createVoraCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createVoraCodingTools({ senderIsOwner: true }).find(
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createVoraCodingTools({
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-depth-policy-"));
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createVoraCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createVoraCodingTools({
      modelProvider: "xai",
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "vora-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createVoraReadTool(
      baseRead as unknown as Parameters<typeof createVoraReadTool>[0],
    );
