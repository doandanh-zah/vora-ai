...
describe("before_tool_call requireApproval handling", () => {
  let runBeforeToolCallHook: (typeof import("./vora-tools.before-tool-call.js"))["runBeforeToolCallHook"];
  let hookRunner: {
    hasHooks: ReturnType<typeof vi.fn>;
    runBeforeToolCall: ReturnType<typeof vi.fn>;
  };
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ runBeforeToolCallHook } = await import("./vora-tools.before-tool-call.js"));
    ...
    const hookRunnerGlobalStateKey = Symbol.for("vora.plugins.hook-runner-global-state");
