...
const GOOGLE_LIVE_SINGLE_KEY = "VORA_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "VORA_LIVE_ANTHROPIC_KEY",
    listVar: "VORA_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "VORA_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `VORA_LIVE_${base}_KEY`;
