...
import type { EmbeddedContextFile } from "./vora-embedded-helpers.js";
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Vora handles all routing internally.",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Vora process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Vora agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Vora agent ids allowed for sessions_spawn",
  ...
  const lines = [
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control Vora's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    ...safetySection,
    "## Vora CLI Quick Reference",
    "Vora is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- vora gateway status",
    "- vora gateway start",
    "- vora gateway stop",
    "- vora gateway restart",
    "If unsure, ask the user to run `vora help` (or `vora gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Vora Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Vora pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by Vora and included below in Project Context.",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'Vora treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
