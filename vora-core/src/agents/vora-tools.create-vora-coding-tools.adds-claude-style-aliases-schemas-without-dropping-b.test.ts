...
const defaultTools = createVoraCodingTools({ senderIsOwner: true });
...
describe("createVoraCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const disabledOpenAiTools = createVoraCodingTools({
    ...
    const anthropicTools = createVoraCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: VoraConfig = {
      tools: {
        exec: {
    ...
    const allowed = createVoraCodingTools({
    ...
    const denied = createVoraCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createVoraCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
