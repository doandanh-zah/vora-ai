...
type ModelsConfig = NonNullable<VoraConfig["models"]>;
...
function resolveConfigBackedProviderAuth(params: { provider: string; config?: VoraConfig }):
  | {
      apiKey: string;
      discoveryApiKey?: string;
      mode: "api_key";
...
function resolveXaiConfigFallbackAuth(params: { provider: string; config?: VoraConfig }):
  | {
      apiKey: string;
      source: string;
      mode: "api-key";
