...
describe("ensureAuthProfileStore", () => {
  ...
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "vora-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "vora-auth-merge-"));
    const previousAgentDir = process.env.VORA_AGENT_DIR;
    const previousVoraAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      ...
      process.env.VORA_AGENT_DIR = mainDir;
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.VORA_AGENT_DIR = previousAgentDir;
      }
      if (previousVoraAgentDir === undefined) {
        delete process.env.PI_CODING_AGENT_DIR;
      } else {
        process.env.PI_CODING_AGENT_DIR = previousVoraAgentDir;
      }
  ...
  it.each([
    ...
    ({ name, profile, expected }) => {
      withTempAgentDir("vora-auth-alias-", (agentDir) => {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    withTempAgentDir("vora-auth-legacy-alias-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("merges legacy oauth.json into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "vora-oauth-migrate-"));
    ...
    try {
      ...
      process.env.VORA_STATE_DIR = root;
      process.env.VORA_AGENT_DIR = agentDir;
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.VORA_STATE_DIR = previousStateDir;
      }
      if (previousAgentDir === undefined) {
        delete process.env.VORA_AGENT_DIR;
      } else {
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    ...
    try {
      withTempAgentDir("vora-auth-invalid-", (agentDir) => {
        const invalidStore = {
          version: AUTH_STORE_VERSION,
          profiles: {
