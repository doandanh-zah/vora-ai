...
function resolveExecConfig(params: { cfg?: VoraConfig; agentId?: string }) {
...
export function createVoraCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createVoraReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
  ...
  // Always normalize tool JSON Schemas before handing them to vora-agent/pi-ai.
