...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "vora-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.VORA_BUNDLED_SKILLS_DIR = bundledDir;
