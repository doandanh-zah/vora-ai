...
import { markVoraExecEnv } from "../../infra/vora-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "vora.sandbox=1");
  args.push("--label", `vora.sessionKey=${params.scopeKey}`);
  args.push("--label", `vora.createdAtMs=${createdAtMs}`);
  args.push("--label", `vora.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `vora.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markVoraExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "vora.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`vora sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`vora sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("vora sandbox recreate --all");
}
