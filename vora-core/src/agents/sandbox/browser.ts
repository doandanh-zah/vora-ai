...
export async function ensureSandboxBrowser(params: {
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `vora sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `vora sandbox recreate --browser --agent ${agentId}`;
          }
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_VORA_BROWSER_PROFILE_NAME)
    : null;
