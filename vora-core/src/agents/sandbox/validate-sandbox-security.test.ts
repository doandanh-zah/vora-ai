...
describe("getBlockedBindReason", () => {
  ...
  it("blocks sensitive home subdirectories", () => {
    ...
    expect(getBlockedBindReason("/home/tester/.vora:/mnt/state:ro")).toEqual(
      expect.objectContaining({
        kind: "targets",
        blockedPath: "/home/tester/.vora",
      }),
    );
  ...
  it("blocks sensitive home subdirectories under VORA_HOME", () => {
    vi.stubEnv("HOME", "/home/tester");
    vi.stubEnv("VORA_HOME", "/srv/vora-home");
    expect(getBlockedBindReason("/srv/vora-home/.ssh:/mnt/ssh:ro")).toEqual(
      expect.objectContaining({
        kind: "targets",
        blockedPath: "/srv/vora-home/.ssh",
      }),
    );
  });

  it("still blocks OS-home sensitive paths when VORA_HOME is overridden", () => {
    vi.stubEnv("HOME", "/home/tester");
    vi.stubEnv("VORA_HOME", "/srv/vora-home");
  ...
  it("blocks the resolved Vora state directory override", () => {
    vi.stubEnv("VORA_STATE_DIR", "/srv/vora-state");
    expect(getBlockedBindReason("/srv/vora-state/credentials:/mnt/state:ro")).toEqual(
      expect.objectContaining({
        kind: "targets",
        blockedPath: "/srv/vora-state",
      }),
    );
  });
});
...
describe("validateBindMounts", () => {
  ...
  it("blocks symlink escapes into blocked directories", () => {
    if (process.platform === "win32") {
      ...
      const dir = mkdtempSync(join(tmpdir(), "vora-sbx-"));
  ...
  it("blocks canonicalized sensitive paths derived from VORA_HOME", () => {
    if (process.platform === "win32") {
      return;
    }

    const dir = mkdtempSync(join(tmpdir(), "vora-home-"));
    ...
    vi.stubEnv("VORA_HOME", linkedHome);
  ...
  it("refreshes canonical blocked aliases when VORA_HOME symlinks retarget", () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("blocks OS-home sensitive paths when VORA_HOME points elsewhere", () => {
    vi.stubEnv("HOME", "/home/tester");
    vi.stubEnv("VORA_HOME", "/srv/vora-home");
  ...
  it("blocks symlink-parent escapes into blocked paths when leaf does not exist", () => {
    ...
    const missingLeaf = join(link, "vora-not-created");
    expect(() =>
...
describe("validateApparmorProfile", () => {
  it("allows named profile/undefined", () => {
    expect(() => validateApparmorProfile("vora-sandbox")).not.toThrow();
