...
// subdirectories and the resolved Vora state directory.
...
const BLOCKED_HOME_SUBPATHS = [".aws", ".config", ".kube", ".vora", ".ssh"] as const;
  ...
  if (params.reason.kind === "reserved_target") {
    return new Error(
      `Sandbox security: bind mount "${params.bind}" targets reserved container path "${params.reason.reservedPath}" ` +
        `(resolved target: "${params.reason.targetPath}"). This can shadow Vora sandbox mounts. ` +
