...
function resolveAttachmentLimits(config: VoraConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".vora", "attachments");
  const relDir = path.posix.join(".vora", "attachments", attachmentId);
