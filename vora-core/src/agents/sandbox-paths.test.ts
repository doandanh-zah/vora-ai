...
async function withOutsideHardlinkInVoraTmp<T>(
  params: {
    voraTmpDir: string;
  ...
  const hardlinkPath = path.join(params.voraTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.voraTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.voraTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.voraTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const voraTmpDir = resolvePreferredVoraTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Vora tmp root",
      media: path.join(voraTmpDir, "image.png"),
      expected: path.join(voraTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Vora tmp root",
      media: pathToFileURL(path.join(voraTmpDir, "photo.png")).href,
      expected: path.join(voraTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Vora tmp root",
      media: path.join(voraTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(voraTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(voraTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside vora tmp root",
      media: path.join(os.tmpdir(), "outside-vora", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Vora tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(voraTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(voraTmpDir, { recursive: true });
      const symlinkPath = path.join(voraTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Vora tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInVoraTmp(
      {
        voraTmpDir,
  ...
  it("rejects symlinked Vora tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInVoraTmp(
