...
type AgentEntry = NonNullable<NonNullable<VoraConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: VoraConfig): AgentEntry[] {
...
export function listAgentIds(cfg: VoraConfig): string[] {
...
export function resolveDefaultAgentId(cfg: VoraConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: VoraConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: VoraConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: VoraConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
