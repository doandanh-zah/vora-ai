...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const voraTmpDir = path.resolve(resolvePreferredVoraTmpDir());
  if (!isPathInside(voraTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: voraTmpDir });
  return resolved;
}
