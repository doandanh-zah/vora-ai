...
import { createPiToolsSandboxContext } from "./vora-tools-sandbox-context.js";
...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("vora-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("vora-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("vora-sbx-");
