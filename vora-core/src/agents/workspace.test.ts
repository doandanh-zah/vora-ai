...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses VORA_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      VORA_HOME: "/srv/vora-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/vora-home"), ".vora", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".vora", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("vora-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("vora-workspace-");
    await fs.mkdir(path.join(tempDir, ".vora"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-workspace-hardlink-"));
    try {
