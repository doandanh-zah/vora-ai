...
let configuredConfig: VoraConfig | undefined;
...
function isLikelyVoraCliProcess(argv: string[] = process.argv): boolean {
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Vora CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureVoraModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyVoraCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): VoraConfig | undefined {
  if (configuredConfig) {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureVoraModelsJson(cfg);
    } catch {
