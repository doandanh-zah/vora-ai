...
vi.mock("./vora-embedded-runner.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./vora-embedded-runner.js")>();
  return {
    ...actual,
...
describe("vora-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnAnnounceFlowOverride();
    resetSessionsSpawnHookRunnerOverride();
    resetSessionsSpawnConfigOverride();
  ...
  afterAll(() => {
    ...
    process.env.VORA_TEST_FAST = fastModeEnv.previous;
  });
