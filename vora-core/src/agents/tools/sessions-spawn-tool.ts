...
// Keep the schema local to avoid a circular import through acp-spawn/vora-tools.
...
export function createSessionsSpawnTool(
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      // ── VORA_V2_DISABLED ──────────────────────────────────────────────
      // Subagent orchestration is disabled in VORA V1.
      // To re-enable for V2, remove this block and uncomment the code below.
      return jsonResult({
        status: "error",
        error: "sessions_spawn is disabled in VORA V1. This feature will be available in V2.",
      });
      // ── END VORA_V2_DISABLED ──────────────────────────────────────────
