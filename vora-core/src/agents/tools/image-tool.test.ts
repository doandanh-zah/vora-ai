...
type PiToolsModule = typeof import("../vora-tools.js");
type CreateVoraCodingToolsArgs = Parameters<PiToolsModule["createVoraCodingTools"]>[0];
type MockVoraToolsOptions = {
  config?: VoraConfig;
  agentDir?: string;
  workspaceDir?: string;
  sandboxRoot?: string;
...
vi.mock("../vora-tools.before-tool-call.js", () => ({
  wrapToolWithBeforeToolCallHook: vi.fn((tool) => tool),
}));

vi.mock("../vora-tools.abort.js", () => ({
  wrapToolWithAbortSignal: vi.fn((tool) => tool),
}));

vi.mock("../vora-tools.js", async () => {
  const { createImageTool } = await import("./image-tool.js");
  return {
    createVoraTools: vi.fn((options?: MockVoraToolsOptions) => {
...
async function createVoraCodingToolsWithFreshModules(options?: CreateVoraCodingToolsArgs) {
  vi.resetModules();
  const { createVoraCodingTools } = await import("../vora-tools.js");
  return createVoraCodingTools(options);
}

async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-image-"));
  try {
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(parentDir, "vora-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): VoraConfig {
  return {
    agents: {
      defaults: {
...
async function withTempSandboxState(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-image-sandbox-"));
...
describe("image tool implicit imageModel config", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-outside-"));
  ...
  it("allows non-workspace local image paths when workspaceOnly is disabled", async () => {
    ...
    await withTempAgentDir(async (agentDir) => {
      const cfg = createMinimaxImageConfig();
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-image-outside-"));
      const outsideImage = path.join(outsideDir, "secret.png");
      await fs.writeFile(outsideImage, Buffer.from(ONE_PIXEL_PNG_B64, "base64"));
      try {
        const tool = createRequiredImageTool({
  ...
  it("allows workspace images via createVoraCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createVoraCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.vora/media/inbound/photo.png",
      });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg = createMinimaxImageConfig();
