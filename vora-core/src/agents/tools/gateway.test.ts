...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    vora: process.env.VORA_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.vora === undefined) {
      delete process.env.VORA_GATEWAY_TOKEN;
    } else {
      process.env.VORA_GATEWAY_TOKEN = envSnapshot.vora;
    }
  });
  ...
  it("uses VORA_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.VORA_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
