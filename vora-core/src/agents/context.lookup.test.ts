...
function mockContextDeps(params: {
  ...
  const ensureVoraModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveVoraAgentDir: () => "/tmp/vora-agent",
  }));
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real vora startup commands that need model metadata", async () => {
    ...
    try {
      for (const scenario of [
        ...
        {
          argv: ["node", "vora", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "vora", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureVoraModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureVoraModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
