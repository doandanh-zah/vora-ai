...
import { planVoraModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  config: VoraConfig;
  sourceConfigForSecrets: VoraConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: VoraConfig): {
  config: VoraConfig;
  sourceConfigForSecrets: VoraConfig;
} {
...
export async function ensureVoraModelsJson(
  config?: VoraConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveVoraAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planVoraModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
