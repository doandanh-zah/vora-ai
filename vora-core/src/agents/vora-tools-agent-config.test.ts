...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: VoraConfig) {
    return createVoraCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<VoraConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<VoraConfig["agents"]>["list"]>[number]["tools"];
  }): VoraConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createVoraCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createVoraCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createVoraCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createVoraCodingTools({
      config: cfg,
    ...
    const bobTools = createVoraCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createVoraCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createVoraCodingTools({
      config: cfg,
