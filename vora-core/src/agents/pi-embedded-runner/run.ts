...
import { ensureVoraModelsJson } from "../models-config.js";
...
import type { RunEmbeddedVoraAgentParams } from "./run/params.js";
...
import type { EmbeddedVoraAgentMeta, EmbeddedPiRunResult } from "./types.js";
import { createUsageAccumulator, mergeUsageIntoAccumulator } from "./usage-accumulator.js";
import { describeUnknownError } from "./utils.js";

type ApiKeyInfo = ResolvedProviderAuth;

export async function runEmbeddedVoraAgent(
  params: RunEmbeddedVoraAgentParams,
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      if (workspaceResolution.usedFallback) {
        log.warn(
          `[workspace-fallback] caller=runEmbeddedVoraAgent reason=${workspaceResolution.fallbackReason} run=${params.runId} session=${redactedSessionId} sessionKey=${redactedSessionKey} agent=${workspaceResolution.agentId} workspace=${redactedWorkspace}`,
        );
      }
      ...
      const agentDir = params.agentDir ?? resolveVoraAgentDir();
      const fallbackConfigured = hasConfiguredModelFallbacks({
        cfg: params.config,
        agentId: params.agentId,
        sessionKey: params.sessionKey,
      });
      await ensureVoraModelsJson(params.config, agentDir);
      ...
      const maybeMarkAuthProfileFailure = async (failure: {
        ...
        config?: RunEmbeddedVoraAgentParams["config"];
        agentDir?: RunEmbeddedVoraAgentParams["agentDir"];
        modelId?: string;
      }) => {
      ...
      try {
        ...
        while (true) {
          ...
          if (shouldRotate) {
            ...
            // See: https://github.com/vora/vora/issues/58348
          ...
          const agentMeta: EmbeddedVoraAgentMeta = {
            sessionId: sessionIdUsed,
          ...
          // vora-agent-core's auto-retry timing issue: when a mid-turn 429/overload
