...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("vora-bundled-");
  const workspaceDir = await createTempDir("vora-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: VoraConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.VORA_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
