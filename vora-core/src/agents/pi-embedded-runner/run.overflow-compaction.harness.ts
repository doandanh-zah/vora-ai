...
export async function loadRunOverflowCompactionHarness(): Promise<{
  runEmbeddedVoraAgent: typeof import("./run.js").runEmbeddedVoraAgent;
}> {
  ...
  vi.doMock("../agent-paths.js", () => ({
    resolveVoraAgentDir: vi.fn(() => "/tmp/agent-dir"),
  }));
  ...
  const { runEmbeddedVoraAgent } = await import("./run.js");
  return { runEmbeddedVoraAgent };
}
