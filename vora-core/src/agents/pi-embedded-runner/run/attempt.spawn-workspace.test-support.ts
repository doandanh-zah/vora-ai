...
vi.mock("../../vora-embedded-subscribe.js", () => ({
...
vi.mock("../../docs-path.js", () => ({
  resolveVoraDocsPath: async () => undefined,
}));
...
vi.mock("../../vora-tools.js", () => ({
  createVoraCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) => [
    {
      name: "sessions_spawn",
      execute: async (
        _callId: string,
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("vora.cache-ttl", data),
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-ctx-engine-agent-"));
