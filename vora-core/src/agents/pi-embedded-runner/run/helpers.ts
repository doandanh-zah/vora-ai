...
import type { EmbeddedVoraAgentMeta } from "../types.js";
...
export function resolveOverloadFailoverBackoffMs(cfg?: VoraConfig): number {
  return cfg?.auth?.cooldowns?.overloadedBackoffMs ?? DEFAULT_OVERLOAD_FAILOVER_BACKOFF_MS;
}

export function resolveOverloadProfileRotationLimit(cfg?: VoraConfig): number {
...
}): Pick<EmbeddedVoraAgentMeta, "usage" | "lastCallUsage" | "promptTokens"> {
