...
let runEmbeddedVoraAgent: typeof import("./run.js").runEmbeddedVoraAgent;

describe("runEmbeddedVoraAgent overflow compaction trigger routing", () => {
  beforeAll(async () => {
    ({ runEmbeddedVoraAgent } = await loadRunOverflowCompactionHarness());
  });
  ...
  it("passes trigger=overflow when retrying compaction after context overflow", async () => {
    ...
    await runEmbeddedVoraAgent(overflowBaseRunParams);
  ...
  it("passes observed overflow token counts into compaction when providers report them", async () => {
    ...
    const result = await runEmbeddedVoraAgent(overflowBaseRunParams);
