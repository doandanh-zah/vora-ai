...
import * as piEmbedded from "./vora-embedded-runner.js";
...
function setConfigOverride(next: VoraConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/vora/vora/issues/31298
    previousFastTestEnv = process.env.VORA_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.VORA_TEST_FAST;
      return;
    }
    process.env.VORA_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // VORA_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Vora runtime context (internal):");
