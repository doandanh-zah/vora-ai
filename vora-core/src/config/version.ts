...
export function parseVoraVersion(raw: string | null | undefined): VoraVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeVoraVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseVoraVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameVoraStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseVoraVersion(a);
  const parsedB = parseVoraVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareVoraVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseVoraVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseVoraVersion(current);
  const parsedTouched = parseVoraVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameVoraStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareVoraVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: VoraVersion): number {
