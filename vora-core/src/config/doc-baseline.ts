...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.VORA_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveVoraPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    VORA_STATE_DIR: path.join(os.tmpdir(), "vora-config-doc-baseline-state"),
    VORA_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
