...
import {
  compareVoraVersions,
  isSameVoraStableFamily,
  parseVoraVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseVoraVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseVoraVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseVoraVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseVoraVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseVoraVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseVoraVersion("2026.3")).toBeNull();
    expect(parseVoraVersion("latest")).toBeNull();
  });
});

describe("compareVoraVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareVoraVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareVoraVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareVoraVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareVoraVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareVoraVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareVoraVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameVoraStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameVoraStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameVoraStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameVoraStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameVoraStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
