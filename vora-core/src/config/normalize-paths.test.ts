  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.vora/logs/vora.log" },
        hooks: {
          path: "~/.vora/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".vora", "logs", "vora.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".vora", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".vora", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".vora", "agents", "main"));
