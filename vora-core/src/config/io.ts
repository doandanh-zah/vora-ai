...
import type { VoraConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  vora config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  vora config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: VoraConfig,
  pathSegments: string[],
): { changed: boolean; next: VoraConfig } {
...
function coerceConfig(value: unknown): VoraConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as VoraConfig;
}
...
function stampConfigVersion(cfg: VoraConfig): VoraConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: VoraConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer Vora (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as VoraConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): VoraConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(effectiveConfigRaw as VoraConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(
    ...
    let cfgToWrite = persistCandidate as VoraConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as VoraConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.VORA_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.VORA_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      argv: process.argv.slice(0, 8),
      execArgv: process.execArgv.slice(0, 8),
      watchMode: deps.env.VORA_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.VORA_WATCH_SESSION === "string" &&
        deps.env.VORA_WATCH_SESSION.trim().length > 0
          ? deps.env.VORA_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.VORA_WATCH_COMMAND === "string" &&
        deps.env.VORA_WATCH_COMMAND.trim().length > 0
          ? deps.env.VORA_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `VORA_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: VoraConfig | null = null;
let runtimeConfigSourceSnapshot: VoraConfig | null = null;
...
export function getRuntimeConfigSourceSnapshot(): VoraConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: VoraConfig;
  candidate: VoraConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: VoraConfig): VoraConfig {
...
export function loadConfig(): VoraConfig {
  ...
  // reparsing vora.json on hot code paths.
  setRuntimeConfigSnapshot(config);
  return runtimeConfigSnapshot ?? config;
}

export function getRuntimeConfig(): VoraConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<VoraConfig> {
