...
import { buildWebSearchProviderConfig, withTempHome, writeVoraConfig } from "./test-helpers.js";
import { VoraSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = VoraSchema.safeParse({
      $schema: "https://vora.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://vora.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = VoraSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = VoraSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeVoraConfig(home, {
        gateway: { bind: "${VORA_BIND}" },
      });

      const prev = process.env.VORA_BIND;
      process.env.VORA_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        ...
        if (prev === undefined) {
          delete process.env.VORA_BIND;
        } else {
          process.env.VORA_BIND = prev;
        }
      }
    });
  });
