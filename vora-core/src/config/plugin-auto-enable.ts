...
import type { VoraConfig } from "./config.js";
...
const ENV_CATALOG_PATHS = ["VORA_PLUGIN_CATALOG_PATHS", "VORA_MPM_CATALOG_PATHS"];
...
function collectModelRefs(cfg: VoraConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: VoraConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: VoraConfig, pluginId: string): boolean {
...
function hasPluginOwnedToolConfig(cfg: VoraConfig, pluginId: string): boolean {
  if (pluginId === "xai") {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.vora) || !isRecord(entry.vora.channel)) {
      continue;
    }
    const channel = entry.vora.channel;
...
function collectCandidateChannelIds(cfg: VoraConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: VoraConfig): boolean {
...
function configMayNeedPluginManifestRegistry(cfg: VoraConfig): boolean {
...
function configMayNeedPluginAutoEnable(cfg: VoraConfig, env: NodeJS.ProcessEnv): boolean {
...
function hasBrowserToolReference(cfg: VoraConfig): boolean {
...
function hasExplicitBrowserPluginEntry(cfg: VoraConfig): boolean {
  return Boolean(
    cfg.plugins?.entries && Object.prototype.hasOwnProperty.call(cfg.plugins.entries, "browser"),
  );
}

function resolveBrowserAutoEnableReason(cfg: VoraConfig): string | null {
...
function isPluginExplicitlyDisabled(cfg: VoraConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: VoraConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: VoraConfig, pluginId: string): VoraConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
