...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vora-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ VORA_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-vora",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-vora#general");
  });
  ...
  it("derives session transcripts dir from VORA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { VORA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when VORA_STATE_DIR differs from stored paths", () => {
