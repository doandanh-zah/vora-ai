...
import type { ConfigFileSnapshot, VoraConfig } from "./types.js";

const mockLoadConfig = vi.hoisted(() => vi.fn<() => VoraConfig>());
...
function makeSnapshot(params: { valid: boolean; config?: VoraConfig }): ConfigFileSnapshot {
  return {
    path: "/tmp/vora.json",
    exists: true,
    raw: "{}",
