...
export type VoraConfig = {
  meta?: {
    /** Last Vora version that wrote this config. */
  ...
  ui?: {
    /** Accent color for Vora UI chrome (hex). */
    seamColor?: string;
    assistant?: {
...
declare const voraConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = VoraConfig & {
  readonly [voraConfigStateBrand]?: TState;
};
