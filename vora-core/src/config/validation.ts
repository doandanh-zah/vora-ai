...
import type { VoraConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.vora.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: VoraConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: VoraConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: VoraConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = VoraSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as VoraConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: VoraConfig | null | undefined;

  const ensureCompatConfig = (): VoraConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Vora config schema.
