...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("vora-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "vora-sandbox-browser-global" },
      agentBrowser: { network: "vora-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("vora-sandbox-browser-agent");
  });
