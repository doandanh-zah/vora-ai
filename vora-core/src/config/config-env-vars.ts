...
function collectConfigEnvVarsByTarget(cfg?: VoraConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: VoraConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: VoraConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: VoraConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.VORA_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
