...
import { parseVoraVersion } from "./version.js";

async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "vora-config-"));
  try {
    await run(home);
  } finally {
...
async function expectNoNewerVersionWarning(touchedVersion: string) {
  await withTempHome(async (home) => {
    ...
    const configPath = path.join(configDir, "vora.json");
    await fs.writeFile(
      configPath,
    ...
    expect(logger.warn).not.toHaveBeenCalledWith(
      expect.stringContaining("Config was last written by a newer Vora"),
    );
...
describe("config io paths", () => {
  it("uses ~/.vora/vora.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".vora", 19001);
  ...
  it("defaults to ~/.vora/vora.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".vora", "vora.json"));
    });
  });

  it("uses VORA_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { VORA_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".vora", "vora.json"));
    });
  });

  it("honors explicit VORA_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".vora", 20002, "custom.json");
      const io = createIoForHome(home, { VORA_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("does not warn when config was last touched by a same-base correction publish", async () => {
    const parsedVersion = parseVoraVersion(VERSION);
    if (!parsedVersion) {
