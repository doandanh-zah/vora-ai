...
import type { ConfigFileSnapshot } from "./types.vora.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/vora/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/vora/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/vora/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/vora/nickserv-password.txt");
