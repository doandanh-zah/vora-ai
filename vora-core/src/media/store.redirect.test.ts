...
const HOME = path.join(os.tmpdir(), "vora-home-redirect");
...
describe("media store redirects", () => {
  ...
  beforeAll(async () => {
    envSnapshot = captureEnv(["VORA_STATE_DIR"]);
    await fs.rm(HOME, { recursive: true, force: true });
    process.env.VORA_STATE_DIR = HOME;
  });
  ...
  it("strips sensitive headers when a redirect crosses origins", async () => {
    ...
    expect(secondHeaders.get("user-agent")).toBe("Vora-Test/1.0");
  });
