...
import { resolveVoraPackageRootSync } from "../infra/vora-root.js";
...
function hasTrustedVoraRootIndicator(params: {
  packageRoot: string;
  ...
  const hasVoraBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("vora")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.vora === "string");
  const hasVoraEntrypoint = fs.existsSync(path.join(params.packageRoot, "vora.mjs"));
  return hasCliEntryExport || hasVoraBin || hasVoraEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedVoraRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedVoraRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveVoraPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedVoraRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveVoraPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedVoraRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (fs.existsSync(candidate)) {
        aliasMap[`vora/plugin-sdk/${subpath}`] = candidate;
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias ? { "vora/extension-api": extensionApiAlias } : {}),
    ...(pluginSdkAlias ? { "vora/plugin-sdk": pluginSdkAlias } : {}),
