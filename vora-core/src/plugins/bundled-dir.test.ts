...
const originalBundledDir = process.env.VORA_BUNDLED_PLUGINS_DIR;
...
function createVoraRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "vora" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.VORA_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.VORA_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "vora-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "vora-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    const repoRoot = createVoraRoot(layout);
    expectResolvedBundledDirFromRoot({
      repoRoot,
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createVoraRoot({
          prefix: "vora-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        const cwdRepoRoot = createVoraRoot({
          prefix: "vora-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "vora.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createVoraRoot({
          prefix: "vora-bundled-dir-override-",
          hasDistExtensions: true,
        });
        return {
          installedRoot,
