...
import type { VoraPluginCliCommandDescriptor } from "./types.js";
...
function resolvePluginCliLoadContext(cfg?: VoraConfig, env?: NodeJS.ProcessEnv) {
...
async function loadPluginCliMetadataRegistry(
  ...
  return {
    ...context,
    registry: await loadVoraPluginCliRegistry({
...
async function loadPluginCliCommandRegistry(
  ...
  const context = resolvePluginCliLoadContext(cfg, env);
  const runtimeRegistry = loadVoraPlugins({
    config: context.config,
    workspaceDir: context.workspaceDir,
    env,
    logger: context.logger,
  ...
  try {
    const metadataRegistry = await loadVoraPluginCliRegistry({
      config: context.config,
      workspaceDir: context.workspaceDir,
      env,
      logger: context.logger,
...
export async function getPluginCliCommandDescriptors(
  cfg?: VoraConfig,
  env?: NodeJS.ProcessEnv,
): Promise<VoraPluginCliCommandDescriptor[]> {
  try {
    const { registry } = await loadPluginCliMetadataRegistry(cfg, env);
    const seen = new Set<string>();
    const descriptors: VoraPluginCliCommandDescriptor[] = [];
