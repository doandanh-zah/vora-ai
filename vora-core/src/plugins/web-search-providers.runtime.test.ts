...
let loadVoraPluginsMock: ReturnType<typeof vi.fn>;
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/vora.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/vora.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
function expectRuntimeProviderResolution(
  ...
  expect(loadVoraPluginsMock).not.toHaveBeenCalled();
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadVoraPluginsMock = vi
      .spyOn(loaderModule, "loadVoraPlugins")
  ...
  it.each([
    ...
    },
    {
      ...
      mutate: (_config: { plugins?: Record<string, unknown> }, env: NodeJS.ProcessEnv) => {
        env.VORA_HOME = "/tmp/vora-home-b";
      },
    },
  ] as const)("$name", ({ mutate }) => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({ VORA_HOME: "/tmp/vora-home-a" });
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        VORA_PLUGIN_DISCOVERY_CACHE_MS: "0",
      },
    },
  ])("$title", ({ env }) => {
    expectSnapshotMemoization({
  ...
  it("does not leak host Vitest env into an explicit non-Vitest cache key", () => {
    ...
    expect(loadVoraPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      VORA_PLUGIN_DISCOVERY_CACHE_MS: "5",
      VORA_PLUGIN_MANIFEST_CACHE_MS: "20",
    });
    ...
    expect(loadVoraPluginsMock).toHaveBeenCalledTimes(2);
  });

  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      VORA_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    });
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.VORA_PLUGIN_DISCOVERY_CACHE_MS = "5";
      },
      expectedLoaderCalls: 2,
    });
  });
