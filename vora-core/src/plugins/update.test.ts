...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-vora",
    targetDir: params?.targetDir ?? "/tmp/opik-vora",
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@vora/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["vora-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/vora-codex-app-server",
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "vora-codex-app-server",
        spec: "vora-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/vora-codex-app-server",
      }),
      ...
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "vora-codex-app-server",
        targetDir: "/tmp/vora-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "vora-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @vora/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@vora/missing",
      ...
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @vora/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        ...
        resolvedSpec: "vora-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "vora-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "vora-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "vora-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
      ...
      expectedSpec: "vora-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "vora-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "vora-codex-app-server",
      });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@vora/voice-call",
      targetDir: "/tmp/vora-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@vora/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@vora/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@vora/voice-call");
    expect(result.config.plugins?.entries?.["@vora/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@vora/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@vora/voice-call",
      installPath: "/tmp/vora-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { VORA_HOME: "/srv/vora-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/vora-home";
    mockBundledSources(
      createBundledSource({
