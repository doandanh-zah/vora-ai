...
const mocks = vi.hoisted(() => ({
  ...
  loadVoraPluginCliRegistry: vi.fn(),
  loadVoraPlugins: vi.fn(),
  applyPluginAutoEnable: vi.fn(),
}));

vi.mock("./loader.js", () => ({
  loadVoraPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadVoraPluginCliRegistry(...args),
  loadVoraPlugins: (...args: unknown[]) => mocks.loadVoraPlugins(...args),
}));
...
function expectPluginLoaderConfig(config: VoraConfig) {
  expect(mocks.loadVoraPlugins).toHaveBeenCalledWith(
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: VoraConfig;
  autoEnabledConfig: VoraConfig;
}) {
...
describe("registerPluginCliCommands", () => {
  beforeEach(() => {
    ...
    mocks.loadVoraPluginCliRegistry.mockReset();
    mocks.loadVoraPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadVoraPlugins.mockReset();
    mocks.loadVoraPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as VoraConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as VoraConfig, env);
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    const { rawConfig, autoEnabledConfig } = createAutoEnabledCliFixture();
    mocks.applyPluginAutoEnable.mockReturnValue({ config: autoEnabledConfig, changes: [] });
    mocks.loadVoraPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    expect(mocks.loadVoraPluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
      }),
    );
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadVoraPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to awaited CLI metadata collection when runtime loading ignored async registration", async () => {
    ...
    await registerPluginCliCommands(program, {} as VoraConfig, undefined, undefined, {
      mode: "lazy",
    });

    expect(mocks.loadVoraPluginCliRegistry).toHaveBeenCalledTimes(1);
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as VoraConfig, undefined, undefined, {
      mode: "lazy",
    });
