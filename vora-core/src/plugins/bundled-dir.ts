...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.VORA_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveVoraPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveVoraPackageRootSync({ argv1: process.argv[1] }),
