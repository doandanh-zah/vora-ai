...
const loadVoraPluginsMock = vi.fn();
...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadVoraPluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
