...
type PluginLoadConfig = NonNullable<Parameters<typeof loadVoraPlugins>[0]>["config"];
type PluginRegistry = ReturnType<typeof loadVoraPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "vora-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.VORA_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "vora.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function writeBundledPlugin(params: {
  ...
  process.env.VORA_BUNDLED_PLUGINS_DIR = bundledDir;
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".vora", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ VORA_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.VORA_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadVoraPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          vora: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.VORA_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadVoraPlugins>) {
...
function useNoBundledPlugins() {
  process.env.VORA_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadVoraPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadVoraPlugins>[0], "cache" | "config">,
) {
  return loadVoraPlugins({
    cache: false,
    ...options,
...
function loadBundleFixture(params: {
  ...
  const stateDir = makeTempDir();
  const bundleRoot = path.join(workspaceDir, ".vora", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ VORA_STATE_DIR: stateDir, ...params.env }, () =>
    loadVoraPlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadVoraPlugins>,
  pluginId: string,
) {
  expect(
    registry.diagnostics.some(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadVoraPlugins>;
  loadSecond: () => ReturnType<typeof loadVoraPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadVoraPlugins>;
  loadVariant: () => ReturnType<typeof loadVoraPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const voraHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(voraHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...
    VORA_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadVoraPlugins({
    cache: false,
    config: {
      plugins: {
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.VORA_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.VORA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".vora", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ VORA_STATE_DIR: stateDir }, () =>
      loadVoraPlugins({
        workspaceDir,
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadVoraPlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadVoraPlugins(options);
        const scoped = loadVoraPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadVoraPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadVoraPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadVoraPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadVoraPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadVoraPlugins(options);
        ...
        return {
          ...
          loadFirst: () =>
            loadVoraPlugins({
              ...
              env: {
                ...process.env,
                VORA_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadVoraPlugins({
              ...
              env: {
                ...process.env,
                VORA_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadVoraPlugins({
              ...
              env: {
                ...
                VORA_HOME: undefined,
                VORA_STATE_DIR: stateDir,
                VORA_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadVoraPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(voraHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadVoraPlugins(options),
          loadVariant: () =>
            loadVoraPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadVoraPlugins({
      env: {
        ...
        HOME: homeDir,
        VORA_HOME: undefined,
        VORA_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers VORA_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const voraHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "vora-home-demo",
      dir: path.join(voraHome, "plugins", "vora-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "vora-home-demo", register() {} };`,
    });
    ...
    const registry = loadVoraPlugins({
      ...
      config: {
        plugins: {
          allow: ["vora-home-demo"],
          entries: {
            "vora-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/vora-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "vora-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadVoraPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadVoraPlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "vora.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const setupRegistry = loadVoraPlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@vora/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@vora/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@vora/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@vora/setup-runtime-not-preferred-test",
  ...
  register(api) {
    ...
    const registry = await loadVoraPluginCliRegistry({
      config: {
        plugins: {
          load: { paths: [plugin.file] },
          allow: ["config-cli"],
  ...
  it("uses the real channel entry in cli-metadata mode for CLI metadata capture", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@vora/cli-metadata-channel",
          vora: { extensions: ["./index.cjs"], setupEntry: "./setup-entry.cjs" },
        },
        null,
        2,
      ),
    ...
    fs.writeFileSync(
      path.join(pluginDir, "index.cjs"),
      `const { defineChannelPluginEntry } = require("vora/plugin-sdk/core");
require("node:fs").writeFileSync(${JSON.stringify(fullMarker)}, "loaded", "utf-8");
module.exports = {
  ...
  it("collects channel CLI metadata during full plugin loads", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@vora/full-cli-metadata-channel",
          vora: { extensions: ["./index.cjs"] },
        },
        null,
        2,
      ),
...
module.exports = {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "vora.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "vora.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ VORA_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("vora/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ VORA_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadVoraPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__voraLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("vora/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("vora/plugin-sdk");
