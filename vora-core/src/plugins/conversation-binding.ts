...
const APPROVALS_PATH = "~/.vora/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "vora-app-server:thread:",
  "vora-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("vora.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("vora.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Vora instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
