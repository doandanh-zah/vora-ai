 ...
 */
export type VoraPluginConfigSchema = {
...
export type VoraPluginToolContext = {
  config?: VoraConfig;
  /** Active runtime-resolved config snapshot when one is available. */
  runtimeConfig?: VoraConfig;
  workspaceDir?: string;
  agentDir?: string;
  agentId?: string;
  sessionKey?: string;
...
export type VoraPluginToolFactory = (
  ctx: VoraPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type VoraPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type VoraPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<VoraConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Vora uses this to expose
 ...
 *
 * Runs after Vora resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Vora auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Vora wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Vora builds its generic
...
/**
 * Provider-owned stream wrapper hook after Vora applies its generic
 ...
 *
 * Return `true` or `false` to override Vora's built-in provider cache TTL
 ...
 *
 * Runs only after Vora exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Vora should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Vora loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Vora falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Vora calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic Vora wrappers.
   *
   ...
   *
   * Called after Vora resolves the raw configured credential but before the
   ...
   * hint than Vora's generic auth-store guidance.
   */
   ...
   * Vora's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Vora should surface a provider-specific hint for
   ...
   * merging. Vora deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Vora uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `vora doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `vora doctor` should offer.
   *
   ...
   *
   * Vora calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Vora's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
   ...
   * when Vora has no config-aware runtime context yet.
   *
  ...
  getConfiguredCredentialValue?: (config?: VoraConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: VoraConfig, value: unknown) => void;
  applySelectionConfig?: (config: VoraConfig) => VoraConfig;
  runSetup?: (ctx: WebSearchProviderSetupContext) => VoraConfig | Promise<VoraConfig>;
...
export type VoraPluginGatewayMethod = {
  method: string;
 ...
 */
export type VoraPluginCommandDefinition = {
...
export type VoraPluginHttpRouteAuth = "gateway" | "plugin";
export type VoraPluginHttpRouteMatch = "exact" | "prefix";

export type VoraPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type VoraPluginHttpRouteParams = {
  path: string;
  handler: VoraPluginHttpRouteHandler;
  auth: VoraPluginHttpRouteAuth;
  match?: VoraPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type VoraPluginCliContext = {
  program: Command;
  config: VoraConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type VoraPluginCliRegistrar = (ctx: VoraPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Vora to keep a plugin command lazy-loaded while still
 ...
 */
export type VoraPluginCliCommandDescriptor = {
  name: string;
  description: string;
...
export type VoraPluginServiceContext = {
  config: VoraConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type VoraPluginService = {
  id: string;
  start: (ctx: VoraPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: VoraPluginServiceContext) => void | Promise<void>;
};
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Vora should inject bundle MCP config for this backend.
   *
...
export type VoraPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type VoraPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: VoraPluginApi) => void | Promise<void>;
  activate?: (api: VoraPluginApi) => void | Promise<void>;
};

export type VoraPluginModule =
  | VoraPluginDefinition
  | ((api: VoraPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

/** Main registration API injected into native plugin entry files. */
export type VoraPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | VoraPluginToolFactory,
    opts?: VoraPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: VoraPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: VoraPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: VoraPluginChannelRegistration | ChannelPlugin) => void;
  ...
  registerCli: (
    registrar: VoraPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, Vora can keep
       * the plugin registrar lazy in the normal root CLI path. Command-only
       * registrations stay on the eager compatibility path.
       */
      descriptors?: VoraPluginCliCommandDescriptor[];
    },
  ) => void;
  registerService: (service: VoraPluginService) => void;
   ...
   */
  registerCommand: (command: VoraPluginCommandDefinition) => void;
...
export type PluginFormat = "vora" | "bundle";
...
export type PluginHookBeforeToolCallResult = {
  ...
  requireApproval?: {
     ...
     * Vora does not await this callback before allowing or denying the tool call.
     */
