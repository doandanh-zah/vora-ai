...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "vora", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".vora", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".vora", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "vora-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".vora", "extensions"),
      },
    });
  });
});
