...
import type { VoraPluginService, VoraPluginServiceContext } from "./types.js";
...
function createRegistry(services: VoraPluginService[]) {
...
function expectServiceLogger(ctx: VoraPluginServiceContext) {
...
function expectServiceContexts(
  contexts: VoraPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: VoraPluginServiceContext[] = [];
