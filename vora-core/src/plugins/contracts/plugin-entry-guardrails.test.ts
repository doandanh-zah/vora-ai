...
const CORE_PLUGIN_ENTRY_IMPORT_RE =
  /import\s*\{[^}]*\bdefinePluginEntry\b[^}]*\}\s*from\s*"vora\/plugin-sdk\/core"/;
    ...
    for (const entry of readdirSync(EXTENSIONS_DIR, { withFileTypes: true })) {
      ...
      try {
        const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
          vora?: { extensions?: unknown };
        };
        const extensions = Array.isArray(pkg.vora?.extensions) ? pkg.vora.extensions : [];
        if (
          extensions.some(
