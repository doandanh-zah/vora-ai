...
describe("provider discovery contract", () => {
  beforeEach(async () => {
    ...
    vi.resetModules();
    vi.doMock("vora/plugin-sdk/agent-runtime", async () => {
    ...
    vi.doMock("vora/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<object>("vora/plugin-sdk/provider-auth");
      return {
        ...actual,
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
