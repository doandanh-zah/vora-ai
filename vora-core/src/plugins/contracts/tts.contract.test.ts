...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): VoraConfig {
  return {
    messages: {
      tts: {
        provider: "openai",
...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.VORA_TTS_PREFS;
      process.env.VORA_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.VORA_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: VoraConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
