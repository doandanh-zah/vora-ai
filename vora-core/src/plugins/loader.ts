...
import { discoverVoraPlugins } from "./discovery.js";
...
export function resolveRuntimePluginRegistry(
  ...
  return getCompatibleActivePluginRegistry(options) ?? loadVoraPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: VoraPluginDefinition;
  register?: VoraPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as VoraPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as VoraPluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.VORA_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverVoraPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverVoraPlugins>["candidates"][number];
  right: ReturnType<typeof discoverVoraPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadVoraPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadVoraPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  const discovery = discoverVoraPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into Vora yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: VoraPluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as VoraPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
...
export async function loadVoraPluginCliRegistry(
  options: PluginLoadOptions = {},
