...
import type {
  ...
  VoraPluginApi,
  VoraPluginChannelRegistration,
  VoraPluginCliCommandDescriptor,
  VoraPluginCliRegistrar,
  VoraPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  VoraPluginHttpRouteAuth,
  VoraPluginHttpRouteMatch,
  VoraPluginHttpRouteHandler,
  VoraPluginHttpRouteParams,
  VoraPluginHookOptions,
...
export type PluginCliRegistration = {
  pluginId: string;
  pluginName?: string;
  register: VoraPluginCliRegistrar;
  commands: string[];
...
export type PluginCommandRegistration = {
  pluginId: string;
  pluginName?: string;
  command: VoraPluginCommandDefinition;
  source: string;
  rootDir?: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: VoraPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: VoraPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: VoraPluginHookOptions | undefined,
    config: VoraPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: VoraPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: VoraPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as VoraPluginChannelRegistration).plugin === "object"
        ? (registration as VoraPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    opts?: { commands?: string[]; descriptors?: VoraPluginCliCommandDescriptor[] },
  ) => {
  ...
  const registerService = (record: PluginRecord, service: VoraPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: VoraPluginCommandDefinition) => {
