...
type LoadVoraPlugins = typeof import("./loader.js").loadVoraPlugins;
...
const loadVoraPluginsMock = vi.fn<LoadVoraPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/vora.plugin.json`,
  };
}
...
function getLastLoadPluginsCall(): Record<string, unknown> {
  const call = loadVoraPluginsMock.mock.calls.at(-1)?.[0];
...
function createAutoEnabledProviderConfig() {
  const rawConfig: VoraConfig = {
    plugins: {},
  };
  const autoEnabledConfig: VoraConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    vi.resetModules();
    vi.doMock("./loader.js", () => ({
      loadVoraPlugins: (...args: Parameters<LoadVoraPlugins>) =>
        loadVoraPluginsMock(...args),
    }));
  ...
  beforeEach(() => {
    ...
    loadVoraPluginsMock.mockReturnValue(registry);
