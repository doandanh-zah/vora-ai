...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "VORA_ENV_TEST_A";
    const keyB = "VORA_ENV_TEST_B";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "VORA_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "VORA_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "VORA_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "VORA_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "vora-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousVoraHome = process.env.VORA_HOME;
    const previousStateDir = process.env.VORA_STATE_DIR;
    const previousBundledDir = process.env.VORA_BUNDLED_PLUGINS_DIR;
    process.env.VORA_HOME = "/srv/vora-home";
    process.env.VORA_STATE_DIR = "/srv/vora-state";
    process.env.VORA_BUNDLED_PLUGINS_DIR = "/srv/vora-bundled";
    ...
    try {
      ...
      expect(env.VORA_HOME).toBeUndefined();
      expect(env.VORA_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.VORA_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("VORA_HOME", previousVoraHome);
      restoreEnvKey("VORA_STATE_DIR", previousStateDir);
      restoreEnvKey("VORA_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { VORA_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processVoraHome: process.env.VORA_HOME,
          processBundledDir: process.env.VORA_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.VORA_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processVoraHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.VORA_HOME).toBe("/srv/vora-home");
    } finally {
      restoreEnvKey("VORA_HOME", previousVoraHome);
    }
  });
