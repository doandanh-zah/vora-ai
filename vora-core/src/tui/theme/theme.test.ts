...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      VORA_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when VORA_THEME=light", async () => {
    const mod = await importThemeWithEnv({ VORA_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when VORA_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ VORA_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats VORA_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ VORA_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("VORA_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      VORA_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ VORA_THEME: "light" });
    const darkMod = await importThemeWithEnv({ VORA_THEME: "dark" });
