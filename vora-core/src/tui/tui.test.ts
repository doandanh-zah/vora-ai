...
describe("resolveInitialTuiAgentId", () => {
  const cfg: VoraConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/vora" },
        { id: "ops", workspace: "/tmp/vora/projects/ops" },
      ],
    },
  };
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run vora devices list");
    expect(state.pairingHint).toContain("vora devices list");
  });
