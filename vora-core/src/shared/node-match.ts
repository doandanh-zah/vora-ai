...
function isCurrentVoraClient(clientId: string | undefined): boolean {
  const normalized = clientId?.trim().toLowerCase() ?? "";
  return normalized.startsWith("vora-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentVoraClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentVoraClient(node.clientId)) {
    score += 10;
