...
describe("shared/node-match", () => {
  ...
  it("prefers a unique current Vora client over a legacy clawdbot client", () => {
    expect(
      resolveNodeIdFromCandidates(
        [
          {
  ...
  it("prints client ids in ambiguous-node errors when available", () => {
    expect(() =>
      ...
      /ambiguous node: Peter's Mac Studio.*node=other-mac.*client=vora-macos.*node=third-mac.*client=vora-macos/,
    );
  });
