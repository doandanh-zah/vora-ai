...
export async function finalizeSetupWizard(
    ...
    if (gatewayProbe.ok) {
      try {
        ...
        await prompter.note(
          [
            "Docs:",
            "https://docs.vora.ai/gateway/health",
            "https://docs.vora.ai/gateway/troubleshooting",
          ].join("\n"),
          "Health check help",
        );
      }
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("vora gateway run")}`,
          `Or rerun with: ${formatCliCommand("vora onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("vora onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.vora.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: $VORA_CONFIG_PATH (default: ~/.vora/vora.json) under gateway.auth.token, or in VORA_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("vora config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("vora doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("vora dashboard --no-open")}`,
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.vora.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.vora.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("vora configure --section web")}`,
          "",
          "Docs: https://docs.vora.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("vora configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.vora.ai/tools/web"}`,
          "Docs: https://docs.vora.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("vora configure --section web")}`,
          "",
          "Docs: https://docs.vora.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://vora.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Vora."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Vora.",
  );
