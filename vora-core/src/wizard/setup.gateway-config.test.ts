...
describe("configureGatewayForSetup", () => {
  ...
  it("prefers VORA_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.VORA_GATEWAY_TOKEN;
    process.env.VORA_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    const previous = process.env.VORA_GATEWAY_PASSWORD;
    process.env.VORA_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "VORA_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.VORA_GATEWAY_PASSWORD = previous;
      }
    }
  });

  it("stores gateway token as SecretRef when secretInputMode=ref", async () => {
    const previous = process.env.VORA_GATEWAY_TOKEN;
    process.env.VORA_GATEWAY_TOKEN = "token-from-env";
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "token", "off", "env"],
        textQueue: ["18789", "VORA_GATEWAY_TOKEN"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.VORA_GATEWAY_TOKEN = previous;
      }
    }
  });
