...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`vora pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`vora pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends VoraConfig = VoraConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends VoraConfig = VoraConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends VoraConfig = VoraConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: VoraConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends VoraConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends VoraConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends VoraConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
