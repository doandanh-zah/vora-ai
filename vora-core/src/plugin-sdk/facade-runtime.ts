...
function resolveSourceFirstPublicSurfacePath(params: {
  ...
  const sourceRoot = params.bundledPluginsDir ?? path.resolve(VORA_PACKAGE_ROOT, "extensions");
...
export function loadBundledPluginPublicSurfaceModuleSync<T extends object>(params: {
  ...
  if (!location) {
    // VORA V1: gracefully handle missing extensions by returning a no-op proxy.
    // Deleted extensions (google, anthropic, whatsapp, etc.) will get this stub
    // instead of crashing the entire CLI.
    const missingStub = new Proxy({} as T, {
      get(_target, property) {
        if (property === Symbol.toPrimitive) return () => `[missing:${params.dirName}]`;
        if (property === Symbol.toStringTag) return `MissingPlugin(${params.dirName})`;
        if (typeof property === "symbol") return undefined;
        return (..._args: unknown[]) => {
          // Return undefined for most calls, allowing graceful degradation
          return undefined;
        };
      },
      has() { return false; },
    ...
    loadedFacadeModules.set(`missing:${params.dirName}/${params.artifactBasename}`, missingStub);
    return missingStub;
  }
  ...
  const opened = openBoundaryFileSync({
    ...
    boundaryLabel:
      location.boundaryRoot === VORA_PACKAGE_ROOT
        ? "Vora package root"
