...
function candidateBinDirs(opts: EnsureVoraPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `vora` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "vora");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.VORA_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "vora"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `vora`
...
/**
 * Best-effort PATH bootstrap so skills that require the `vora` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureVoraCliOnPath(opts: EnsureVoraPathOpts = {}) {
  if (isTruthyEnvValue(process.env.VORA_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.VORA_PATH_BOOTSTRAPPED = "1";
