...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@vora/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@vora/matrix", id: "matrix", expected: true },
    { packageName: "@vora/matrix", id: "@vora/matrix", expected: true },
    { packageName: "@vora/matrix", id: "signal", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@vora/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@vora__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-install-safe-"));
    try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-install-safe-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-install-safe-outside-"));
      try {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked base directories", async () => {
    const parentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-install-safe-"));
