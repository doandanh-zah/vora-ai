...
describe("clawhub helpers", () => {
  ...
  afterEach(() => {
    delete process.env.VORA_CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_AUTH_TOKEN;
    delete process.env.VORA_CLAWHUB_CONFIG_PATH;
  ...
  it("checks min gateway versions with loose host labels", () => {
    ...
    expect(satisfiesGatewayMinimum("Vora 2026.3.22", "2026.3.0")).toBe(true);
  ...
  it("resolves ClawHub auth token from config.json", async () => {
    const configRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vora-clawhub-config-"));
    const configPath = path.join(configRoot, "clawhub", "config.json");
    process.env.VORA_CLAWHUB_CONFIG_PATH = configPath;
  ...
  it("resolves ClawHub auth token from the legacy config path override", async () => {
    const configRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vora-clawdhub-config-"));
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "vora-clawhub-home-"));
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "vora-clawhub-home-"));
      const xdgRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vora-clawhub-xdg-"));
  ...
  it("injects resolved auth token into ClawHub requests", async () => {
    process.env.VORA_CLAWHUB_TOKEN = "env-token-123";
