...
const DEFAULT_PACKAGE_NAME = "vora";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["vora", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vora-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "vora.mjs");
    ...
    const doctorStep = await runStep(
      step("vora doctor", doctorArgv, gitRoot, { VORA_UPDATE_IN_PROGRESS: "1" }),
    );
