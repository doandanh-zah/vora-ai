...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__vora_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "vora-root");
...
function setPackageRoot(root: string, name = "vora") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./vora-root.js").resolveVoraPackageRootSync,
  asyncResolver: typeof import("./vora-root.js").resolveVoraPackageRoot,
  opts: Parameters<typeof import("./vora-root.js").resolveVoraPackageRootSync>[0],
...
describe("resolveVoraPackageRoot", () => {
  let resolveVoraPackageRoot: typeof import("./vora-root.js").resolveVoraPackageRoot;
  let resolveVoraPackageRootSync: typeof import("./vora-root.js").resolveVoraPackageRootSync;

  beforeEach(async () => {
    vi.resetModules();
    ({ resolveVoraPackageRoot, resolveVoraPackageRootSync } =
      await import("./vora-root.js"));
    state.entries.clear();
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "vora");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "vora.mjs")));
    ...
    {
      name: "falls through from a non-vora moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-vora");
    ...
    {
      name: "returns null for non-vora package roots",
      setup: () => {
        const pkgRoot = fx("not-vora");
        setPackageRoot(pkgRoot, "not-vora");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "vora.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "vora");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveVoraPackageRootSync,
      resolveVoraPackageRoot,
      opts,
      expected,
    );
  });
