...
import { POSIX_VORA_TMP_DIR, resolvePreferredVoraTmpDir } from "./tmp-vora-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredVoraTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `vora-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredVoraTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_VORA_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredVoraTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredVoraTmpDir", () => {
  it("prefers /tmp/vora when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_VORA_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/vora when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_VORA_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it.each([
    {
      name: "falls back to os.tmpdir()/vora when /tmp/vora is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/vora when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/vora exists but is not writable",
    ...
    {
      name: "falls back when /tmp/vora is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/vora is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/vora is group/other writable",
  ...
  it("repairs existing /tmp/vora permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_VORA_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_VORA_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_VORA_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/vora after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_VORA_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_VORA_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_VORA_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_VORA_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Vora temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "vora");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredVoraTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_VORA_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback Vora temp dir/);
  });
});
