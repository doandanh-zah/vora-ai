...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers VORA_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for VORA_HOME",
      env: { VORA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when VORA_HOME is tilde-only and no fallback home exists",
      env: { VORA_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores VORA_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          VORA_HOME: "/srv/vora-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { VORA_HOME: "/srv/vora-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/vora-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/vora-home " },
      expected: "/srv/vora-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/vora-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of VORA_HOME", () => {
    expect(
