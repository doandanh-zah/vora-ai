...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.VORA_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Vora";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Vora\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.VORA_MDNS_HOSTNAME?.trim() || "vora";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
