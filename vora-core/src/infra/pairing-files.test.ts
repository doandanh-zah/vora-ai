...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/vora-state", "devices")).toEqual({
      dir: path.join("/tmp/vora-state", "devices"),
      pendingPath: path.join("/tmp/vora-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/vora-state", "devices", "paired.json"),
    });
  });
