 ...
 * returning control to the caller (typically `triggerVoraRestart` →
...
/**
 * Parse vora gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("vora")) {
 ...
 * Returns only PIDs that belong to vora gateway processes (not the current process).
 */
