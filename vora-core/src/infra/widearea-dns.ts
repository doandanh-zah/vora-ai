...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.VORA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*vora-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "vora";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "vora");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "vora-gw");
  ...
  records.push(`_vora-gw._tcp IN PTR ${instanceLabel}._vora-gw._tcp`);
  records.push(`${instanceLabel}._vora-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._vora-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; vora-content-hash: ${contentHash}\n${contentBody}`;
}
