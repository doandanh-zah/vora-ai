...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: VoraConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: VoraConfig) {
...
function resolveHeartbeatAgents(cfg: VoraConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: VoraConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: VoraConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: VoraConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
