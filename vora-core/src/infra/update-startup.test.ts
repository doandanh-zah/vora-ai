...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vora-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["VORA_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.VORA_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveVoraPackageRoot } = await import("./vora-root.js"));
    ...
    vi.mocked(resolveVoraPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveVoraPackageRoot).mockResolvedValue("/opt/vora");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/vora/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
