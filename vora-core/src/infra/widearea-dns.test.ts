...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "vora.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (Vora)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "vora.internal", expected: "vora.internal." },
    { value: "vora.internal.", expected: "vora.internal." },
    { value: "  vora.internal  ", expected: "vora.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { VORA_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { VORA_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("vora.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "vora.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderZoneText({
      ...
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/vora",
    });

    expectZoneRecords(txt, [
      `$ORIGIN vora.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_vora-gw._tcp IN PTR studio-london._vora-gw._tcp`,
      `studio-london._vora-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Vora)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/vora`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN vora.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._vora-gw._tcp IN TXT`,
        `displayName=Mac Studio (Vora)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("vora.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("vora.internal."),
