...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __voraDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__voraDiagnosticEventsState) {
    globalStore.__voraDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__voraDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: VoraConfig): boolean {
