...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.VORA_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-current-bindings-"));
    process.env.VORA_STATE_DIR = testStateDir;
