...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: VoraConfig | null = null;
  ...
  get(key: string, cfg: VoraConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: VoraConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: VoraConfig): void {
