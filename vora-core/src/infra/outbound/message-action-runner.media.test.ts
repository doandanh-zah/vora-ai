...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Vora tmp root", async () => {
      const tmpRoot = resolvePreferredVoraTmpDir();
      ...
      try {
        ...
        const hostTmpOutsideVora = path.join(os.tmpdir(), "outside-vora", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
