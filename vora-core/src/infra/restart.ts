...
export function triggerVoraRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.VORA_SYSTEMD_UNIT,
      process.env.VORA_PROFILE,
    );
  ...
  const label =
    process.env.VORA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.VORA_PROFILE);
