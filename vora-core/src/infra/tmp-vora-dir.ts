...
export const POSIX_VORA_TMP_DIR = "/tmp/vora";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredVoraTmpDirOptions = {
...
export function resolvePreferredVoraTmpDir(
  options: ResolvePreferredVoraTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "vora" : `vora-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[vora] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Vora temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Vora temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_VORA_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_VORA_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_VORA_TMP_DIR)) {
      return POSIX_VORA_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_VORA_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_VORA_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_VORA_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_VORA_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_VORA_TMP_DIR;
