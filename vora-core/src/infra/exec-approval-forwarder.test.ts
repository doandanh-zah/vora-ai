...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): VoraConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): VoraConfig {
  return {
