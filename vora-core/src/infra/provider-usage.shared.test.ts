...
import {
  clampPercent,
  resolveLegacyVoraAgentAccessToken,
...
async function withLegacyPiAuthFile(
  ...
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "vora-provider-usage-"));
...
describe("provider-usage.shared", () => {
  ...
  it.each([
    ...
    await withLegacyPiAuthFile(contents, async (home) => {
      expect(resolveLegacyVoraAgentAccessToken({ HOME: home }, ["z-ai", "zai"])).toBe(expected);
    });
  });
});
