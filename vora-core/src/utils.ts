...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".vora");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.VORA_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$VORA_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via VORA_STATE_DIR.
