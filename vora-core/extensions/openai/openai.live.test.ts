...
import { encodePngRgba, fillPixel } from "vora/plugin-sdk/media-runtime";
import type { ResolvedTtsConfig } from "vora/plugin-sdk/speech-runtime";
...
const LIVE_MODEL_ID = process.env.VORA_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.4-nano";
const LIVE_IMAGE_MODEL = process.env.VORA_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-1";
const LIVE_VISION_MODEL = process.env.VORA_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.VORA_LIVE_TEST === "1";
...
function createLiveConfig(): VoraConfig {
