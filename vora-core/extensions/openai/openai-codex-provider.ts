...
} from "vora/plugin-sdk/plugin-entry";
import {
  CODEX_CLI_PROFILE_ID,
  ensureAuthProfileStore,
  listProfilesForProvider,
  type OAuthCredential,
} from "vora/plugin-sdk/provider-auth";
import { buildOauthProviderAuthResult } from "vora/plugin-sdk/provider-auth";
import { loginOpenAICodexOAuth } from "vora/plugin-sdk/provider-auth-login";
import {
  DEFAULT_CONTEXT_TOKENS,
  normalizeModelCompat,
  normalizeProviderId,
  type ProviderPlugin,
} from "vora/plugin-sdk/provider-model-shared";
import { createOpenAIAttributionHeadersWrapper } from "vora/plugin-sdk/provider-stream";
import { fetchCodexUsage } from "vora/plugin-sdk/provider-usage";
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `vora models auth login --provider openai-codex` or `vora configure`.";
}
