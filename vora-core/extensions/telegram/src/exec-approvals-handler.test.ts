...
import type { VoraConfig } from "../../../src/config/config.js";
...
function createHandler(cfg: VoraConfig, accountId = "default") {
...
describe("TelegramExecApprovalHandler", () => {
  ...
  it("falls back to the session-bound Telegram account when turn source account is missing", async () => {
    const sessionStoreDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-tg-approvals-"));
