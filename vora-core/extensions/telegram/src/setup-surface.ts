import {
  ...
  splitSetupEntries,
} from "vora/plugin-sdk/setup";
import type { ChannelSetupWizard } from "vora/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "vora/plugin-sdk/setup-tools";
...
function shouldShowTelegramDmAccessWarning(cfg: VoraConfig, accountId: string): boolean {
...
function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`vora config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`vora config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
