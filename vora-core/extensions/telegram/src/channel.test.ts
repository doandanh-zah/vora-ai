...
import type { PluginRuntime } from "vora/plugin-sdk/testing";
...
function resolveAccount(cfg: VoraConfig, accountId: string): ResolvedTelegramAccount {
  return telegramPlugin.config.resolveAccount(cfg, accountId) as ResolvedTelegramAccount;
}

function createStartTelegramContext(cfg: VoraConfig, accountId: string) {
...
function startTelegramAccount(cfg: VoraConfig, accountId: string) {
...
function configureOpsProxyNetwork(cfg: VoraConfig) {
...
describe("telegramPlugin duplicate token guard", () => {
  ...
  // Regression: https://github.com/vora/vora/issues/53876
