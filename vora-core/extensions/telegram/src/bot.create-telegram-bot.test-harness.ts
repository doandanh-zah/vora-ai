import { resolveDefaultModelForAgent } from "vora/plugin-sdk/agent-runtime";
import type { VoraConfig } from "vora/plugin-sdk/config-runtime";
import { resetInboundDedupe } from "vora/plugin-sdk/reply-runtime";
import type { MsgContext } from "vora/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "vora/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "vora/plugin-sdk/reply-runtime";
import type { MockFn } from "vora/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("vora/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("vora/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("vora/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("vora/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/vora-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("vora/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("vora/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("vora/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("vora/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
...
vi.doMock("vora/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("vora/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.doMock("vora/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("vora/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: VoraConfig): {
...
vi.doMock("vora/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("vora/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("vora/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("vora/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("vora/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("vora/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("vora/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("vora/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("vora/plugin-sdk/channel-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("vora/plugin-sdk/channel-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: VoraConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: VoraConfig) => {
