...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): VoraConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as VoraConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): VoraConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as VoraConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: VoraConfig, accountId = "media") {
    await handleTelegramAction(
