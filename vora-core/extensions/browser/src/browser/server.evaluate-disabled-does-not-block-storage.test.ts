  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "vora",
        profiles: {
          vora: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  ...
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.VORA_GATEWAY_PORT;
    process.env.VORA_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.VORA_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.VORA_GATEWAY_PASSWORD;
