...
vi.mock("./chrome.js", () => ({
  resolveVoraUserDataDir: vi.fn(() => "/tmp/vora-test/vora/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "vora-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveVoraUserDataDir).mockReturnValue(userDataDir);
