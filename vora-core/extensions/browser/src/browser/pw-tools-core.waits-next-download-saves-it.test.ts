...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredVoraTmpDir: vi.fn(() => "/tmp/vora"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-vora-dir.js");
...
describe("pw-tools-core", () => {
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-vora-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredVoraTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredVoraTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredVoraTmpDir.mockReturnValue("/tmp/vora");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vora-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".vora-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredVoraTmpDir.mockReturnValue("/tmp/vora-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "vora-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "vora-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredVoraTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "vora-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "vora-preferred", "downloads")}${path.sep}`),
    );
  });
