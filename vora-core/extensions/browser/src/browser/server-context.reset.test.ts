...
function localVoraProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "vora",
...
function createLocalVoraResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localVoraProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveVoraUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vora-reset-"));
    const profileDir = path.join(tempRoot, "vora");
    ...
    const ops = createLocalVoraResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveVoraUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vora-reset-no-own-"));
    const profileDir = path.join(tempRoot, "vora");
    fs.mkdirSync(profileDir, { recursive: true });
