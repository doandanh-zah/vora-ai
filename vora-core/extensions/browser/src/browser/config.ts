import type { BrowserConfig, BrowserProfileConfig, VoraConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_VORA_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "vora" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_VORA_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_VORA_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: VoraConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_VORA_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "vora";
