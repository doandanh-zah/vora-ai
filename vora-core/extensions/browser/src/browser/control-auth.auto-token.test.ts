...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => VoraConfig>(),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<VoraConfig["gateway"]>["auth"]> | undefined;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: VoraConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
