...
import {
  DEFAULT_VORA_BROWSER_COLOR,
  DEFAULT_VORA_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
export function resolveVoraUserDataDir(profileName = DEFAULT_VORA_BROWSER_PROFILE_NAME) {
...
export function buildVoraChromeLaunchArgs(params: {
...
export async function launchVoraChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveVoraUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_VORA_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildVoraChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateVoraProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 vora browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`vora browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`vora browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 vora browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopVoraChrome(
