...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".vora-profile-decorated");
}
 ...
 */
export function decorateVoraProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_VORA_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_VORA_BROWSER_COLOR).toUpperCase();
