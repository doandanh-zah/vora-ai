...
import type { VoraConfig } from "../../../../src/plugin-sdk/discord.js";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: VoraConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): VoraConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("vora/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("vora/plugin-sdk/acp-runtime")>(
    "vora/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("vora/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("vora/plugin-sdk/command-auth")>(
...
vi.mock("vora/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("vora/plugin-sdk/reply-runtime")>(
    "vora/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("vora/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("vora/plugin-sdk/config-runtime")>(
...
vi.mock("vora/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("vora/plugin-sdk/runtime-env")>(
    "vora/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("vora/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("vora/plugin-sdk/infra-runtime")>(
