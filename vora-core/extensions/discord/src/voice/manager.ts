...
import { resolveAgentDir } from "vora/plugin-sdk/agent-runtime";
import { agentCommandFromIngress } from "vora/plugin-sdk/agent-runtime";
import { resolveTtsConfig, type ResolvedTtsConfig } from "vora/plugin-sdk/agent-runtime";
import type { VoraConfig } from "vora/plugin-sdk/config-runtime";
import type { DiscordAccountConfig, TtsConfig } from "vora/plugin-sdk/config-runtime";
import { transcribeAudioFile } from "vora/plugin-sdk/media-understanding-runtime";
import { resolveAgentRoute } from "vora/plugin-sdk/routing";
import { logVerbose, shouldLogVerbose } from "vora/plugin-sdk/runtime-env";
import { createSubsystemLogger } from "vora/plugin-sdk/runtime-env";
import type { RuntimeEnv } from "vora/plugin-sdk/runtime-env";
import { parseTtsDirectives } from "vora/plugin-sdk/speech";
import { textToSpeech } from "vora/plugin-sdk/speech-runtime";
import { formatErrorMessage } from "vora/plugin-sdk/ssrf-runtime";
import { resolvePreferredVoraTmpDir } from "vora/plugin-sdk/temp-path";
...
function resolveVoiceTtsConfig(params: { cfg: VoraConfig; override?: TtsConfig }): {
  cfg: VoraConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredVoraTmpDir(), "discord-voice-"));
