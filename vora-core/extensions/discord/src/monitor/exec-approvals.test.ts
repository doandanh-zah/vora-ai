...
const { STORE_PATH, mockSessionStoreEntries } = vi.hoisted(() => ({
  STORE_PATH: "/tmp/vora-exec-approvals-test.json",
...
beforeEach(() => {
  ...
  mockResolveGatewayConnectionAuth.mockReset().mockImplementation(
    async (params: {
      ...
      const envToken = params.env.VORA_GATEWAY_TOKEN;
      const envPassword = params.env.VORA_GATEWAY_PASSWORD;
...
vi.mock("../../../../src/gateway/operator-approvals-client.js", () => ({
  createOperatorApprovalsGatewayClient: async (params: {
    ...
    const envUrl = process.env.VORA_GATEWAY_URL?.trim();
...
vi.mock("vora/plugin-sdk/text-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("vora/plugin-sdk/text-runtime")>();
  return {
    ...actual,
    logDebug: vi.fn(),
    logError: vi.fn(),
...
describe("DiscordExecApprovalHandler gateway auth", () => {
  ...
  it("prefers VORA_GATEWAY_TOKEN when config token is missing", async () => {
    vi.stubEnv("VORA_GATEWAY_TOKEN", "env-gateway-token");
    const handler = new DiscordExecApprovalHandler({
      token: "discord-bot-token",
      accountId: "default",
      config: { enabled: true, approvers: ["123"] },
...
describe("DiscordExecApprovalHandler gateway auth resolution", () => {
  ...
  it("passes env URL overrides to shared gateway auth resolver", async () => {
    const previousGatewayUrl = process.env.VORA_GATEWAY_URL;
    try {
      process.env.VORA_GATEWAY_URL = "wss://gateway-from-env.example/ws";
      const handler = new DiscordExecApprovalHandler({
        token: "test-token",
        accountId: "default",
        config: { enabled: true, approvers: ["123"] },
      ...
      if (typeof previousGatewayUrl === "string") {
        process.env.VORA_GATEWAY_URL = previousGatewayUrl;
      } else {
