import { DEFAULT_EMOJIS } from "vora/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/vora-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("vora/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("vora/plugin-sdk/conversation-runtime");
...
const configRuntimeModule = await import("vora/plugin-sdk/config-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/vora-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/vora-discord-process-test-sessions.json" },
      },
    });
