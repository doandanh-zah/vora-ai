...
import { AcpRuntimeError } from "vora/plugin-sdk/acp-runtime";
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): VoraConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
vi.mock("vora/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("vora/plugin-sdk/plugin-runtime")>(
...
describe("monitorDiscordProvider", () => {
  ...
  it("registers plugin commands from the real registry as native Discord commands", async () => {
    ...
    } = await vi.importActual<typeof import("vora/plugin-sdk/plugin-runtime")>(
      "vora/plugin-sdk/plugin-runtime",
    );
    clearPluginCommands();
    listNativeCommandSpecsForConfigMock.mockReturnValue([
