import { DEFAULT_ACCOUNT_ID } from "vora/plugin-sdk/account-id";
import type { DiscordGuildEntry } from "vora/plugin-sdk/config-runtime";
import type { VoraConfig } from "vora/plugin-sdk/config-runtime";
import { createEnvPatchedAccountSetupAdapter } from "vora/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "vora/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "vora/plugin-sdk/setup-runtime";
import { formatDocsLink } from "vora/plugin-sdk/setup-tools";
import {
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: VoraConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: VoraConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: VoraConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: VoraConfig; accountId: string }) =>
    ...
    disable: (cfg: VoraConfig) => setSetupChannelEnabled(cfg, channel, false),
