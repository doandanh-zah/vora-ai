...
import type { VoraPluginApi } from "vora/plugin-sdk/plugin-runtime";
...
function fakeApi(): VoraPluginApi {
  return {
    config: {},
  } as VoraPluginApi;
}
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.vora.ai", "", "vora.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.vora.ai", "vora.ai"],
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as VoraConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
