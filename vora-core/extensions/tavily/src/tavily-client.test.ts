...
vi.mock("vora/plugin-sdk/provider-web-search", () => ({
...
vi.mock("vora/plugin-sdk/security-runtime", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: vora", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "vora" });
  });

  it("runTavilyExtract sends X-Client-Source: vora", async () => {
