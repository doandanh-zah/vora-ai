...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.vora) ? packageJson.vora : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.vora?.channel?.id) || left.name;
    const rightId = trimString(right.vora?.channel?.id) || right.name;
