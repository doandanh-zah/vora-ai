...
function parsePluginSdkSubpath(specifier) {
  ...
  const subpath = specifier.slice("vora/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced vora/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced vora/plugin-sdk/<subpath> must exist in the public package exports.",
  );
