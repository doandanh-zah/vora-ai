...
const warningFilterKey = Symbol.for("vora.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadVoraPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadVoraPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vora-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "vora.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'vora/plugin-sdk';",
...
const registry = loadVoraPlugins({
  ...
  env: {
    ...process.env,
    VORA_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
