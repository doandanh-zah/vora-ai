...
import { parseReleaseVersion, resolveNpmPublishPlan } from "../vora-npm-release-check.ts";
...
export function collectPublishablePluginPackageErrors(
  ...
  const extensions = packageJson.vora?.extensions ?? [];

  if (!packageName.startsWith("@vora/")) {
    errors.push(
      `package name must start with "@vora/"; found "${packageName || "<missing>"}".`,
    );
  }
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("vora.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("vora.extensions must contain only non-empty strings.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const dir of dirs) {
    ...
    if (packageJson.vora?.release?.publishToNpm !== true) {
      continue;
    }
    ...
    publishable.push({
      ...
      installNpmSpec: packageJson.vora?.install?.npmSpec?.trim() || undefined,
    });
  }
...
export function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "vora-plugin-npm-view-"));
