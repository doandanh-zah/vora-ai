 ...
 * `vora update`. Use it to verify the completion installation flow
...
\`vora update\`, \`vora doctor\`, and \`vora onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Vora Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`vora completion --install\` later to enable.`));
    return;
  }
