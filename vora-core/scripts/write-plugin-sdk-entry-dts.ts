...
function rewriteFacadeTypeMapSpecifier(specifier: string): string {
  if (!specifier.startsWith("@vora/")) {
    return specifier;
  }
  return `${GENERATED_FACADE_TYPE_MAP_DIST_PREFIX}${specifier.slice("@vora/".length)}`;
}
...
function rewriteGeneratedFacadeTypeMapDts(): void {
  ...
  const rewritten = source.replace(/@vora\/([a-z0-9-]+\/[^")\s]+)/g, (_match, suffix: string) =>
    rewriteFacadeTypeMapSpecifier(`@vora/${suffix}`),
  );
  if (rewritten !== source) {
