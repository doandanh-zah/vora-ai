...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "vora.plugin.json");
    ...
    if (packageJson.vora && "extensions" in packageJson.vora) {
      packageJson.vora = {
        ...packageJson.vora,
        extensions: rewritePackageExtensions(packageJson.vora.extensions),
        ...(typeof packageJson.vora.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.vora.setupEntry) }
          : {}),
      };
    }
