...
const extensionRestartMetadataFiles = new Set(["vora.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.VORA_FORCE_BUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by VORA_FORCE_BUILD",
...
const logRunner = (message, deps) => {
  if (deps.env.VORA_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[vora] ${message}\n`);
};

const runVora = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["vora.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
