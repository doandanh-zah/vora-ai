...
async function buildMissingPackages(params = {}) {
  ...
  for (const relativePackagePath of workspacePackagePaths.toSorted(compareStrings)) {
    ...
    const missing = Object.keys(pkg.dependencies ?? {})
      .filter((dep) => dep !== "vora" && !rootDeps.has(dep))
    ...
    output.push({
      ...
      packagePath: relativePackagePath,
      npmSpec: redactNpmSpec(pkg.vora?.install?.npmSpec),
...
function describeCronSeamKinds(relativePath, source) {
  ...
  if (
    importsAgentRunner &&
    /\brunCliAgent\b|\brunEmbeddedVoraAgent\b|\brunWithModelFallback\b|\bregisterAgentRunContext\b/.test(
      source,
    )
  ) {
