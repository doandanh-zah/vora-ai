...
function shouldStageRuntimeDeps(packageJson) {
  return packageJson.vora?.bundle?.stageRuntimeDependencies === true;
}
...
function sanitizeBundledManifestForRuntimeInstall(pluginDir) {
  ...
  if (packageJson.peerDependencies?.vora) {
    const nextPeerDependencies = { ...packageJson.peerDependencies };
    delete nextPeerDependencies.vora;
  ...
  if (packageJson.peerDependenciesMeta?.vora) {
    const nextPeerDependenciesMeta = { ...packageJson.peerDependenciesMeta };
    delete nextPeerDependenciesMeta.vora;
  ...
  if (packageJson.devDependencies?.vora) {
    const nextDevDependencies = { ...packageJson.devDependencies };
    delete nextDevDependencies.vora;
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".vora-runtime-deps-stamp.json");
}
  ...
  if (platform === "win32") {
    ...
    throw new Error(
      `failed to resolve a toolchain-local npm next to ${execPath}. ` +
        ...
        "Vora refuses to shell out to bare npm on Windows; install a Node.js toolchain that bundles npm or run with a matching Node installation.",
    );
  }
...
function installPluginRuntimeDeps(params) {
  ...
  const tempInstallDir = makeTempDir(
    os.tmpdir(),
    `vora-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}-`,
  );
