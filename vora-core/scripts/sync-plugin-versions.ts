...
const VORA_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncVoraDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.vora;
  if (!current || current === "workspace:*" || !VORA_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncMinHostVersion(pkg: PackageJson, targetVersion: string): boolean {
  const installConfig = pkg.vora?.install;
  const current = installConfig?.minHostVersion;
  if (!current || !VORA_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core Vora release numbers.\n\n`;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncVoraDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncVoraDependencyRange(pkg.peerDependencies, targetVersion);
