...
GUM_VERSION="${VORA_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$VORA_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
cleanup_npm_vora_paths() {
    ...
    rm -rf "$npm_root"/.vora-* "$npm_root"/vora 2>/dev/null || true
}

extract_vora_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_vora_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/vora" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/vora"|"/usr/local/bin/vora")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/vora/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale vora symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing vora binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Vora package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_vora_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Vora npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*vora" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_vora_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_vora_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Vora npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an vora binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${VORA_TAGLINE_INDEX:-}" ]]; then
        if [[ "${VORA_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((VORA_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${VORA_NO_ONBOARD:-0}
NO_PROMPT=${VORA_NO_PROMPT:-0}
DRY_RUN=${VORA_DRY_RUN:-0}
INSTALL_METHOD=${VORA_INSTALL_METHOD:-}
VORA_VERSION=${VORA_VERSION:-latest}
USE_BETA=${VORA_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/vora"
GIT_DIR=${VORA_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${VORA_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${VORA_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${VORA_VERBOSE:-0}"
VERIFY_INSTALL="${VORA_VERIFY_INSTALL:-0}"
VORA_BIN=""
PNPM_CMD=()
HELP=0
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/vora)
...
Environment variables:
  VORA_INSTALL_METHOD=git|npm
  VORA_VERSION=latest|next|main|<semver>|<spec>
  VORA_BETA=0|1
  VORA_GIT_DIR=...
  VORA_GIT_UPDATE=0|1
  VORA_NO_PROMPT=1
  VORA_VERIFY_INSTALL=1
  VORA_DRY_RUN=1
  VORA_NO_ONBOARD=1
  VORA_VERBOSE=1
  VORA_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Vora checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Vora source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_vora_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"vora"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
ensure_vora_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/vora" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/vora" ]]; then
        ln -sf "$npm_root/vora/dist/entry.js" "${npm_bin}/vora"
        ui_info "Created vora bin link at ${npm_bin}/vora"
    fi
    return 0
}

# Check for existing Vora installation
check_existing_vora() {
    if [[ -n "$(type -P vora 2>/dev/null || true)" ]]; then
        ui_info "Existing Vora installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make vora show as \"command not found\" in new terminals."
...
warn_vora_not_found() {
    ui_warn "Installed, but vora is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t vora 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named vora; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_vora_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P vora 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/vora" ]]; then
        echo "${npm_bin}/vora"
        return 0
    fi
...
install_vora_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/vora/vora.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Vora from git checkout: ${repo_dir}"
    else
        ui_info "Installing Vora from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Vora" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Vora" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/vora" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/vora"
    ui_success "Vora wrapper installed to \$HOME/.local/bin/vora"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view vora dist-tags.beta 2>/dev/null || true)"
...
install_vora() {
    ...
    if can_resolve_registry_package_version "${VORA_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${VORA_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Vora v${resolved_version}"
    else
        ui_info "Installing Vora (${VORA_VERSION})"
    fi
    local install_spec=""
    ...
    ensure_vora_bin_link || true

    ui_success "Vora installed"
}
...
resolve_workspace_dir() {
    local profile="${VORA_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.vora/workspace-${profile}"
    else
        echo "${HOME}/.vora/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run vora onboard to finish setup"
        return
    fi
...
if ! declare -F extract_vora_semver >/dev/null 2>&1; then
...
resolve_vora_version() {
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/vora/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/vora/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_vora_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Vora checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set VORA_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    installed_version=$(resolve_vora_version)
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "No TTY; run vora doctor and vora plugins update --all manually"
        fi
    else
...
if [[ "${VORA_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
