...
BASE_IMAGE="${BASE_IMAGE:-vora-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-vora-sandbox-common:bookworm-slim}"
...
VORA_DOCKER_BUILD_USE_BUILDX="${VORA_DOCKER_BUILD_USE_BUILDX:-0}"
VORA_DOCKER_BUILD_CACHE_FROM="${VORA_DOCKER_BUILD_CACHE_FROM:-}"
VORA_DOCKER_BUILD_CACHE_TO="${VORA_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${VORA_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${VORA_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${VORA_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${VORA_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${VORA_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=vora.sandbox=1)
NOTE
