...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const voraMeta =
    ...
    "vora" in source.packageJson
      ? (source.packageJson.vora as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    voraMeta &&
    typeof voraMeta.channel === "object" &&
    voraMeta.channel &&
    !Array.isArray(voraMeta.channel)
      ? (voraMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
