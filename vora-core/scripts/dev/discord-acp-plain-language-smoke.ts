...
type DriverMode = "token" | "webhook" | "vora";
...
function resolveStateDir(): string {
  const override = process.env.VORA_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.VORA_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".vora");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver vora] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Vora spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|vora> Driver transport mode (default: token)\n" +
    ...
    "  --vora-bin <path>        Vora CLI binary for driver=vora (default: vora)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  VORA_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  VORA_DISCORD_SMOKE_DRIVER\n" +
    "  VORA_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  VORA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  VORA_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  VORA_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  VORA_DISCORD_SMOKE_AGENT\n" +
    "  VORA_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  VORA_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  VORA_DISCORD_SMOKE_POLL_MS\n" +
    "  VORA_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  VORA_DISCORD_SMOKE_VORA_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.VORA_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.VORA_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "vora"
        ? "vora"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.VORA_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.VORA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.VORA_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.VORA_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.VORA_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.VORA_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.VORA_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.VORA_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.VORA_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.VORA_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const voraBin =
    resolveArg("--vora-bin") || process.env.VORA_DISCORD_SMOKE_VORA_BIN || "vora";
...
async function voraCliJson<T>(params: { voraBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.voraBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`vora ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithVora(params: {
  voraBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await voraCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    voraBin: params.voraBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "vora") {
    return await readMessagesWithVora({
      voraBin: params.args.voraBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `vora-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("vora message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "vora"
            ? await readMessagesWithVora({
                voraBin: args.voraBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Vora.`,
        diagnostics: {
          bindingCandidates: [
            {
