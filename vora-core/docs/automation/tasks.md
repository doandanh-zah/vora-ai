...
- `vora tasks list` shows all tasks; `vora tasks audit` surfaces issues.
...
vora tasks list --runtime acp
vora tasks list --status running

# Show details for a specific task (by ID, run ID, or session key)
vora tasks show <lookup>

# Cancel a running task (kills the child session)
vora tasks cancel <lookup>

# Change notification policy for a task
vora tasks notify <lookup> state_changes
...
| CLI operations         | `cli`        | `vora agent` commands that run through the gateway | `done_only`           |
...
| `cancelled` | Stopped by the operator via `vora tasks cancel`                        |
...
When a task reaches a terminal state, Vora notifies you. There are two delivery paths:
...
```bash
vora tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
```
...
```bash
vora tasks notify <lookup> <done_only|state_changes|silent>
```

### `tasks audit`

```bash
vora tasks audit [--json]
```

Surfaces operational issues. Findings also appear in `vora status` when issues are detected.
...
`vora status` includes an at-a-glance task summary:
...
```
$VORA_STATE_DIR/tasks/runs.sqlite
```
...
A cron job **definition** lives in `~/.vora/cron/jobs.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
