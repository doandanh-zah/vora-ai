...
- `sessionTarget: "isolated"` or `sessionTarget: "session:..."` creates a visible task that shows up in `vora tasks` with delivery notifications.
...
- Jobs persist under `~/.vora/cron/` so restarts don’t lose schedules.
...
- For upgrades, `vora doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
vora cron list
vora cron run <job-id>
vora cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.vora/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `vora cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Vora applies a
...
Main-session cron jobs create [background task](/automation/tasks) records with `silent` notify policy (no notifications by default). They appear in `vora tasks list` but do not generate delivery messages.
...
These detached runs create [background task](/automation/tasks) records visible in `vora tasks` and subject to task audit and maintenance.
...
If `delivery` is omitted for isolated jobs, Vora defaults to `announce`.
...
- CLI equivalents: `vora cron add --light-context ...` and `vora cron edit --light-context`.
...
- `schedule.at` accepts ISO 8601. Tool/API values without a timezone are treated as UTC; the CLI also accepts `vora cron add|edit --at "<offset-less-iso>" --tz <iana>` for local wall-clock one-shots.
...
- Job store: `~/.vora/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.vora/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Vora classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Vora archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `vora cron runs` and adjust retention before logs become large
...
vora cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
vora cron edit <jobId> --agent ops
vora cron edit <jobId> --clear-agent
```
...
```bash
vora system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`vora system event`](/cli/system).

## Troubleshooting

### "Nothing runs"

- Check cron is enabled: `cron.enabled` and `VORA_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Vora applies exponential retry backoff for recurring jobs after consecutive errors:
