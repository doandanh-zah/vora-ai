...
read_when:
  ...
  - Wiring external systems into Vora
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Vora config directory (typically `~/.vora/hooks/transforms`).
...
- `vora webhooks gmail setup` writes `hooks.gmail` config for `vora webhooks gmail run`.
