...
Vora has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Pool note:
  ...
  - Opt back into Vitest file isolation with `VORA_TEST_ISOLATE=1 pnpm test`.
  - `VORA_TEST_NO_ISOLATE=0` or `VORA_TEST_NO_ISOLATE=false` also force isolated runs.
- Fast-local iteration note:
  ...
  - The wrapper keeps `VORA_VITEST_FS_MODULE_CACHE` enabled on supported hosts, but assigns a lane-local `VORA_VITEST_FS_MODULE_CACHE_PATH` so concurrent Vitest processes do not race on one shared experimental cache directory.
  - Set `VORA_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want one explicit cache location for direct single-run profiling.
- Perf-debug note:
...
- Useful overrides:
  - `VORA_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `VORA_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises Vora's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `VORA_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `VORA_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `VORA_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.vora`.
- Set `VORA_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` now defaults to a quieter mode: it keeps `[live] ...` progress output, but suppresses the extra `~/.profile` notice and mutes gateway bootstrap logs/Bonjour chatter. Set `VORA_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `VORA_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `VORA_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `VORA_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Optional target overrides:
  - `VORA_ANDROID_NODE_ID` or `VORA_ANDROID_NODE_NAME`.
  - `VORA_ANDROID_GATEWAY_URL` / `VORA_ANDROID_GATEWAY_TOKEN` / `VORA_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `VORA_LIVE_TEST=1` if invoking Vitest directly)
- Set `VORA_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `VORA_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet 4.6+, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.7, Grok 4)
  - `VORA_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `VORA_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,..."` (comma allowlist)
- How to select providers:
  - `VORA_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `VORA_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
  - Separates “provider API is broken / key is invalid” from “gateway agent pipeline is broken”
  - Contains small, isolated regressions (example: OpenAI Responses/Codex Responses reasoning replay + tool-call flows)

### Layer 2: Gateway + dev agent smoke (what "@vora" actually does)
...
- How to select models:
  ...
  - `VORA_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `VORA_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
- How to select providers (avoid “OpenRouter everything”):
  - `VORA_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Enable:
  - `pnpm test:live` (or `VORA_LIVE_TEST=1` if invoking Vitest directly)
  - `VORA_LIVE_SETUP_TOKEN=1`
- Token sources (pick one):
  - Profile: `VORA_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test`
  - Raw token: `VORA_LIVE_SETUP_TOKEN_VALUE=sk-ant-oat01-...`
- Model override (optional):
  - `VORA_LIVE_SETUP_TOKEN_MODEL=anthropic/claude-opus-4-6`

Setup example:

```bash
vora models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
VORA_LIVE_SETUP_TOKEN=1 VORA_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test pnpm test:live src/agents/anthropic.setup-token.live.test.ts
```
...
- Enable:
  - `pnpm test:live` (or `VORA_LIVE_TEST=1` if invoking Vitest directly)
  - `VORA_LIVE_CLI_BACKEND=1`
- Defaults:
  - Model: `claude-cli/claude-sonnet-4-6`
  - Command: `claude`
  - Args: `["-p","--output-format","json","--permission-mode","bypassPermissions"]`
- Overrides (optional):
  - `VORA_LIVE_CLI_BACKEND_MODEL="claude-cli/claude-opus-4-6"`
  - `VORA_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.4"`
  - `VORA_LIVE_CLI_BACKEND_COMMAND="/full/path/to/claude"`
  - `VORA_LIVE_CLI_BACKEND_ARGS='["-p","--output-format","json","--permission-mode","bypassPermissions"]'`
  - `VORA_LIVE_CLI_BACKEND_CLEAR_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'`
  - `VORA_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt).
  - `VORA_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `VORA_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `VORA_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
- `VORA_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` to keep Claude Code CLI MCP config enabled (default disables MCP config with a temporary empty file).

Example:

```bash
VORA_LIVE_CLI_BACKEND=1 \
  VORA_LIVE_CLI_BACKEND_MODEL="claude-cli/claude-sonnet-4-6" \
...
- For `claude-cli`, it installs the Linux `@anthropic-ai/claude-code` package into a cached writable prefix at `VORA_DOCKER_CLI_TOOLS_DIR` (default: `~/.cache/vora/docker-cli-tools`).
- It copies `~/.claude` into the container when available, but on machines where Claude auth is backed by `ANTHROPIC_API_KEY`, it also preserves `ANTHROPIC_API_KEY` / `ANTHROPIC_API_KEY_OLD` for the child Claude CLI via `VORA_LIVE_CLI_BACKEND_PRESERVE_ENV`.
...
- Enable:
  ...
  - `VORA_LIVE_ACP_BIND=1`
- Defaults:
  - ACP agent: `claude`
  - Synthetic channel: Slack DM-style conversation context
  - ACP backend: `acpx`
- Overrides:
  - `VORA_LIVE_ACP_BIND_AGENT=claude`
  - `VORA_LIVE_ACP_BIND_AGENT=codex`
  - `VORA_LIVE_ACP_BIND_ACPX_COMMAND=/full/path/to/acpx`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `VORA_LIVE_ACP_BIND_ACPX_COMMAND` is unset, the test uses the configured/bundled acpx command. If your harness auth depends on env vars from `~/.profile`, prefer a custom `acpx` command that preserves provider env.
...
- Inside Docker, the runner sets `VORA_LIVE_ACP_BIND_ACPX_COMMAND=$HOME/.npm-global/bin/acpx` so acpx keeps provider env vars from the sourced profile available to the child harness CLI.
...
- Single model, direct (no gateway):
  - `VORA_LIVE_MODELS="openai/gpt-5.2" pnpm test:live src/agents/models.profiles.live.test.ts`

- Single model, gateway smoke:
  - `VORA_LIVE_GATEWAY_MODELS="openai/gpt-5.2" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `VORA_LIVE_GATEWAY_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `VORA_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
  - Antigravity (OAuth): `VORA_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Gemini API vs Gemini CLI:
  - API: Vora calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: Vora shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
`VORA_LIVE_GATEWAY_MODELS="openai/gpt-5.2,openai-codex/gpt-5.4,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
Include at least one image-capable model in `VORA_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.

### Aggregators / alternate gateways

If you have keys enabled, we also support testing via:

- OpenRouter: `openrouter/...` (hundreds of models; use `vora models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `vora models list` / model selection.

- Profile store: `~/.vora/credentials/` (preferred; what “profile keys” means in the tests)
- Config: `~/.vora/vora.json` (or `VORA_CONFIG_PATH`)
...
- Current bundled providers covered:
  ...
  - `google`
- Optional narrowing:
  - `VORA_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google"`
  - `VORA_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-1,google/gemini-3.1-flash-image-preview"`
  - `VORA_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit"`
- Optional auth behavior:
  - `VORA_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
They also set `VORA_SKIP_CHANNELS=1` so gateway live probes do not start
...
`VORA_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.
`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
Vora gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `vora/default`, then sends a
...
This lane expects a usable live model key, and `VORA_PROFILE_FILE`
...
container, starts a second container that spawns `vora mcp serve`, then
...
- `VORA_CONFIG_DIR=...` (default: `~/.vora`) mounted to `/home/node/.vora`
- `VORA_WORKSPACE_DIR=...` (default: `~/.vora/workspace`) mounted to `/home/node/.vora/workspace`
- `VORA_PROFILE_FILE=...` (default: `~/.profile`) mounted to `/home/node/.profile` and sourced before running tests
- `VORA_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/vora/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs under `$HOME` are mounted read-only under `/host-auth/...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs inferred from `VORA_LIVE_PROVIDERS` / `VORA_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `VORA_DOCKER_AUTH_DIRS=all`, `VORA_DOCKER_AUTH_DIRS=none`, or a comma list like `VORA_DOCKER_AUTH_DIRS=.claude,.codex`
- `VORA_LIVE_GATEWAY_MODELS=...` / `VORA_LIVE_MODELS=...` to narrow the run
- `VORA_LIVE_GATEWAY_PROVIDERS=...` / `VORA_LIVE_PROVIDERS=...` to filter providers in-container
- `VORA_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `VORA_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `VORA_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
