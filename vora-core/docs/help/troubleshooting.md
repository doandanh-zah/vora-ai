---
summary: "Symptom first troubleshooting hub for Vora"
read_when:
  - Vora is not working and you need the fastest path to a fix
...
- `vora status` → shows configured channels and no obvious auth errors.
- `vora status --all` → full report is present and shareable.
- `vora gateway probe` → expected gateway target is reachable (`Reachable: yes`). `RPC: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `vora gateway status` → `Runtime: running` and `RPC probe: ok`.
- `vora doctor` → no blocking config/service errors.
- `vora channels status --probe` → channels report `connected` or `ready`.
- `vora logs --follow` → steady activity, no repeating fatal errors.
...
## Plugin install fails with missing vora extensions

If install fails with `package.json missing vora.extensions`, the plugin package
is using an old shape that Vora no longer accepts.

Fix in the plugin package:

1. Add `vora.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `vora plugins install <package>` again.
...
```mermaid
flowchart TD
  A[Vora is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  <Accordion title="No replies">
    ...
    vora gateway status
    vora channels status --probe
    vora pairing list --channel <channel> [--account <id>]
    vora logs --follow
    ```
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `vora gateway status`
    - `RPC probe: ok`
  ...
  <Accordion title="Cron or heartbeat did not fire or did not deliver">
    ...
    vora cron status
    vora cron list
    vora cron runs --id <jobId> --limit 20
    vora logs --follow
    ```
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    vora config get tools.exec.host
    vora config get tools.exec.security
    vora config get tools.exec.ask
    ...
    ```bash
    vora config set tools.exec.host gateway
    vora config set tools.exec.security full
    vora config set tools.exec.ask off
    vora gateway restart
    ```
  ...
  <Accordion title="Browser tool fails">
    ...
    - `vora` starts, or `user` can see local Chrome tabs.
