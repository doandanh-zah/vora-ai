---
summary: "Use OpenAI via API keys or Codex subscription in Vora"
read_when:
  - You want to use OpenAI models in Vora
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Vora.
...
```bash
vora onboard --auth-choice openai-api-key
# or non-interactive
vora onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Vora forwards both through the `openai/*` Responses path.
Vora intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

Vora does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in Vora.
...
vora onboard --auth-choice openai-codex

# Or run OAuth directly
vora models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Vora
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, Vora also supports:

- `openai-codex/gpt-5.3-codex-spark`

Vora treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

Vora also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
Vora uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Vora also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Vora enables it by default for
...
Vora, set `agents.defaults.models["<provider>/<model>"].params.serviceTier`
...
Vora forwards `params.serviceTier` to both direct `openai/*` Responses
...
- if you route either provider through another base URL or proxy, Vora leaves `service_tier` untouched

### OpenAI fast mode

Vora exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, Vora maps it to OpenAI priority processing:
...
`baseUrl` on `api.openai.com`), Vora now auto-enables OpenAI server-side
