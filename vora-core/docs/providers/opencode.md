---
summary: "Use OpenCode Zen and Go catalogs with Vora"
read_when:
...
OpenCode exposes two hosted catalogs in Vora:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Vora keeps the runtime provider ids
...
vora onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
vora onboard --auth-choice opencode-go
vora onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
