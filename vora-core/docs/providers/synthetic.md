---
summary: "Use Synthetic's Anthropic-compatible API in Vora"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Vora registers it as the
...
```bash
vora onboard --auth-choice synthetic-api-key
```
...
Note: Vora's Anthropic client appends `/v1` to the base URL, so use
