---
summary: "Use Venice AI privacy-focused models in Vora"
read_when:
  - You want privacy-focused inference in Vora
...
```bash
vora onboard --auth-choice venice-api-key
```
...
```bash
vora agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Vora shows all available Venice models. Pick based on your needs:
...
```bash
vora models set venice/kimi-k2-5
vora models set venice/claude-opus-4-6
```

List all available models:

```bash
vora models list | grep venice
```

## Configure via `vora configure`

1. Run `vora configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Vora automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
vora agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
vora agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
vora agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
vora agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
vora agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `vora models list` to see currently available models. Some models may be temporarily offline.
