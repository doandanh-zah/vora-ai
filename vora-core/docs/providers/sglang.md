---
summary: "Run Vora with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Vora against a local SGLang server
...
Vora can connect to SGLang using the `openai-completions` API.

Vora can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Vora will query:
