---
summary: "Use MiniMax models in Vora"
read_when:
  - You want MiniMax models in Vora
...
Vora's MiniMax provider defaults to **MiniMax M2.7**.
...
```bash
vora plugins enable minimax  # skip if already loaded.
vora gateway restart  # restart if gateway is already running
vora onboard --auth-choice minimax-portal
```
...
See the MiniMax plugin package README in the Vora repo for details.
...
- Use `vora models list` and `vora models set minimax/MiniMax-M2.7` to switch.
...
- Running `vora configure` and selecting a **MiniMax** auth option, or
