---
summary: "Use Amazon Bedrock (Converse API) models with Vora"
read_when:
  - You want to use Amazon Bedrock models with Vora
...
Vora can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
If AWS credentials are detected, Vora can automatically discover Bedrock
...
When running Vora on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, Vora's credential detection currently only checks for environment
...
- Vora surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
