...
read_when:
  - You want to use Groq with Vora
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Vora connects
...
media-understanding provider, Vora uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.vora/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `vora models list | grep groq`
