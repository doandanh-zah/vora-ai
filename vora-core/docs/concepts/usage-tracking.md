...
- `/usage cost` in chats: local cost summary aggregated from Vora session logs.
- CLI: `vora status --usage` prints a full per-provider breakdown.
- CLI: `vora channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
