...
- CLI helpers: `vora onboard`, `vora models list`, `vora models set <provider/model>`.
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  Vora merges that output into `models.providers` before writing
  `models.json`.
...
Provider plugins can now own most provider-specific logic while Vora keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `vora onboard`, `vora models auth`, and headless setup
...
That covers providers that still fit Vora's normal transports. A provider
...
- Configure multiple keys via:
  - `VORA_LIVE_<PROVIDER>_KEY` (single live override, highest priority)
...
Vora ships with the pi‑ai catalog. These providers require **no**
...
- Provider: `openai`
- Auth: `OPENAI_API_KEY`
- Optional rotation: `OPENAI_API_KEYS`, `OPENAI_API_KEY_1`, `OPENAI_API_KEY_2`, plus `VORA_LIVE_OPENAI_KEY` (single override)
- Example models: `openai/gpt-5.4`, `openai/gpt-5.4-pro`
- CLI: `vora onboard --auth-choice openai-api-key`
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in Vora because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- Optional rotation: `ANTHROPIC_API_KEYS`, `ANTHROPIC_API_KEY_1`, `ANTHROPIC_API_KEY_2`, plus `VORA_LIVE_ANTHROPIC_KEY` (single override)
- Example model: `anthropic/claude-opus-4-6`
- CLI: `vora onboard --auth-choice token` (paste setup-token) or `vora models auth paste-token --provider anthropic`
- Direct public Anthropic requests support the shared `/fast` toggle and `params.fastMode`, including API-key and OAuth-authenticated traffic sent to `api.anthropic.com`; Vora maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
- CLI: `vora onboard --auth-choice openai-codex` or `vora models auth login --provider openai-codex`
- Default transport is `auto` (WebSocket-first, SSE fallback)
- Override per model via `agents.defaults.models["openai-codex/<model>"].params.transport` (`"sse"`, `"websocket"`, or `"auto"`)
- `params.serviceTier` is also forwarded on native Codex Responses requests (`chatgpt.com/backend-api`)
- Shares the same `/fast` toggle and `params.fastMode` config as direct `openai/*`; Vora maps that to `service_tier=priority`
- `openai-codex/gpt-5.3-codex-spark` remains available when the Codex OAuth catalog exposes it; entitlement-dependent
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like Vora.
...
- CLI: `vora onboard --auth-choice opencode-zen` or `vora onboard --auth-choice opencode-go`
...
- Provider: `google`
- Auth: `GEMINI_API_KEY`
- Optional rotation: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY` fallback, and `VORA_LIVE_GEMINI_KEY` (single override)
- Example models: `google/gemini-3.1-pro-preview`, `google/gemini-3-flash-preview`
- Compatibility: legacy Vora config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- CLI: `vora onboard --auth-choice gemini-api-key`

### Google Vertex and Gemini CLI

- Providers: `google-vertex`, `google-gemini-cli`
- Auth: Vertex uses gcloud ADC; Gemini CLI uses its OAuth flow
- Caution: Gemini CLI OAuth in Vora is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Enable: `vora plugins enable google`
  - Login: `vora models auth login --provider google-gemini-cli --set-default`
  - Note: you do **not** paste a client id or secret into `vora.json`. The CLI login flow stores
...
- CLI: `vora onboard --auth-choice zai-api-key`
...
- CLI: `vora onboard --auth-choice ai-gateway-api-key`
...
- CLI: `vora onboard --kilocode-api-key <key>`
...
- CLI: `vora onboard --auth-choice mistral-api-key`
- Groq: `groq` (`GROQ_API_KEY`)
- Cerebras: `cerebras` (`CEREBRAS_API_KEY`)
  - GLM models on Cerebras use ids `zai-glm-4.7` and `zai-glm-4.6`.
  - OpenAI-compatible base URL: `https://api.cerebras.ai/v1`.
- GitHub Copilot: `github-copilot` (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`)
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `vora onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
- CLI: `vora onboard --auth-choice volcengine-api-key`
...
- CLI: `vora onboard --auth-choice byteplus-api-key`
...
- CLI: `vora onboard --auth-choice synthetic-api-key`
...
`vora onboard` and the model picker. See [/providers/ollama](/providers/ollama)
...
- For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), Vora forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
- If `baseUrl` is empty/omitted, Vora keeps the default OpenAI behavior (which resolves to `api.openai.com`).
- For safety, an explicit `compat.supportsDeveloperRole: true` is still overridden on non-native `openai-completions` endpoints.

## CLI examples

```bash
vora onboard --auth-choice opencode-zen
vora models set opencode/claude-opus-4-6
vora models list
```
