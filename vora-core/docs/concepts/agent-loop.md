...
In Vora, a loop is a single, serialized run per session that emits lifecycle and stream events
...
2. `agentCommand` runs the agent:
   ...
   - calls `runEmbeddedVoraAgent` (vora-agent-core runtime)
   - emits **lifecycle end/error** if the embedded loop does not emit one
3. `runEmbeddedVoraAgent`:
   - serializes runs via per-session + global queues
   - resolves model + auth profile and builds the pi session
   - subscribes to pi events and streams assistant/tool deltas
   - enforces timeout -> aborts run if exceeded
   - returns payloads + usage metadata
4. `subscribeEmbeddedPiSession` bridges vora-agent-core events to Vora `agent` stream:
...
- System prompt is built from Vora’s base prompt, skills prompt, bootstrap context, and per-run overrides.
- Model-specific limits and compaction reserve tokens are enforced.
- See [System prompt](/concepts/system-prompt) for what the model sees.

## Hook points (where you can intercept)

Vora has two hook systems:
...
- Assistant deltas are streamed from vora-agent-core and emitted as `assistant` events.
...
- `assistant`: streamed deltas from vora-agent-core
- `tool`: streamed tool events from vora-agent-core
...
- Agent runtime: `agents.defaults.timeoutSeconds` default 172800s (48 hours); enforced in `runEmbeddedVoraAgent` abort timer.
