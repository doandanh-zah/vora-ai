...
read_when:
  - You want to understand how Vora assembles model context
...
A **context engine** controls how Vora builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Vora ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.vora/vora.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Vora plugin. Install
...
vora plugins install -l ./my-context-engine
```
...
Every time Vora runs a model prompt, the context engine participates at
...
Vora currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Vora
...
The built-in `legacy` engine preserves Vora's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Vora uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Vora disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Vora automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `vora/plugin-sdk/core` to use
  Vora's built-in compaction behavior.
...
Vora resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Vora's built-in summarization. Plugin engines can implement
...
- Use `vora doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Vora
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `vora plugins install -l ./my-engine` to link a
