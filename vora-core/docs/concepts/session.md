---
summary: "How Vora manages conversation sessions"
read_when:
...
Vora organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `vora security audit`.
...
Vora automatically bounds session storage over time. By default, it runs
...
Preview with `vora sessions cleanup --dry-run`.

## Inspecting sessions

- `vora status` -- session store path and recent activity.
- `vora sessions --json` -- all sessions (filter with `--active <minutes>`).
