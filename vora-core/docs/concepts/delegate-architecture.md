---
summary: "Delegate architecture: running Vora as a named agent on behalf of an organization"
...
Goal: run Vora as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Vora agent that:
...
Vora's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Vora's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.vora/workspace-delegate",
  tools: {
...
- Cron run history: `~/.vora/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.vora/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Vora's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
vora agents add delegate
```

This creates:

- Workspace: `~/.vora/workspace-delegate`
- State: `~/.vora/agents/delegate/agent`
- Sessions: `~/.vora/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.vora/workspace" },
      {
        id: "delegate",
        workspace: "~/.vora/workspace-delegate",
        tools: {
...
~/.vora/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.vora/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.vora/workspace-org",
        agentDir: "~/.vora/agents/org-assistant/agent",
