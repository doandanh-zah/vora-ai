...
Vora selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Vora can use (plus aliases).
...
- If you omit the provider, Vora treats the input as an alias or a model for the **default provider** (only works when there is no `/` in the model ID).
...
vora models list
vora models status
vora models set <provider/model>
vora models set-image <provider/model>

vora models aliases list
vora models aliases add <alias> <provider/model>
vora models aliases remove <alias>

vora models fallbacks list
vora models fallbacks add <provider/model>
vora models fallbacks remove <provider/model>
vora models fallbacks clear

vora models image-fallbacks list
vora models image-fallbacks add <provider/model>
vora models image-fallbacks remove <provider/model>
vora models image-fallbacks clear
```

`vora models` (no subcommand) is a shortcut for `models status`.
...
`vora models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.vora/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Vora writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Vora regenerates `models.json`, including command-driven paths like `vora agent`.
