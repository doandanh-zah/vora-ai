...
[Honcho](https://honcho.dev) adds AI-native memory to Vora. It persists
...
```bash
vora plugins install @honcho-ai/vora-honcho
vora honcho setup
vora gateway --force
```
...
Settings live under `plugins.entries["vora-honcho"].config`:
    ...
    entries: {
      "vora-honcho": {
        config: {
          ...
          workspaceId: "vora", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `vora honcho setup` detects and
...
```bash
vora honcho setup                        # Configure API key and migrate files
vora honcho status                       # Check connection status
vora honcho ask <question>               # Query Honcho about the user
vora honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/vora-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Vora integration guide](https://docs.honcho.dev/v3/guides/integrations/vora)
- [Memory](/concepts/memory) -- Vora memory overview
