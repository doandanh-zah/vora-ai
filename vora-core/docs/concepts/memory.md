...
summary: "How Vora remembers things across sessions"
read_when:
...
Vora remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.vora/workspace`).
...
<Info>
Vora auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Vora
...
```bash
vora memory status          # Check index status and provider
vora memory search "query"  # Search from the command line
vora memory index --force   # Rebuild the index
```
