---
read_when:
  - 你想在 Vora 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Vora 中使用 MiniMax M2.5
title: MiniMax
x-i18n:
...
```bash
vora plugins enable minimax  # 如果已加载则跳过。
vora gateway restart  # 如果 gateway 已在运行，则重启
...
详情请参阅 [MiniMax plugin README](https://github.com/vora/vora/tree/main/extensions/minimax)。
...
- 使用 `vora models list` 和 `vora models set minimax/MiniMax-M2.5` 进行切换。
...
- 运行 `vora configure` 并选择 **MiniMax M2.5**，或者
