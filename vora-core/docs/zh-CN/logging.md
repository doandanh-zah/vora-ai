...
所有日志配置都在 `~/.vora/vora.json` 的 `logging` 下。
...
- Vora 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `vora plugins enable diagnostics-otel` 启用插件。
...
- `vora.tokens`（计数器，属性：`vora.token`、`vora.channel`、`vora.provider`、`vora.model`）
- `vora.cost.usd`（计数器，属性：`vora.channel`、`vora.provider`、`vora.model`）
- `vora.run.duration_ms`（直方图，属性：`vora.channel`、`vora.provider`、`vora.model`）
- `vora.context.tokens`（直方图，属性：`vora.context`、`vora.channel`、`vora.provider`、`vora.model`）

消息流：

- `vora.webhook.received`（计数器，属性：`vora.channel`、`vora.webhook`）
- `vora.webhook.error`（计数器，属性：`vora.channel`、`vora.webhook`）
- `vora.webhook.duration_ms`（直方图，属性：`vora.channel`、`vora.webhook`）
- `vora.message.queued`（计数器，属性：`vora.channel`、`vora.source`）
- `vora.message.processed`（计数器，属性：`vora.channel`、`vora.outcome`）
- `vora.message.duration_ms`（直方图，属性：`vora.channel`、`vora.outcome`）

队列 + 会话：

- `vora.queue.lane.enqueue`（计数器，属性：`vora.lane`）
- `vora.queue.lane.dequeue`（计数器，属性：`vora.lane`）
- `vora.queue.depth`（直方图，属性：`vora.lane` 或 `vora.channel=heartbeat`）
- `vora.queue.wait_ms`（直方图，属性：`vora.lane`）
- `vora.session.state`（计数器，属性：`vora.state`、`vora.reason`）
- `vora.session.stuck`（计数器，属性：`vora.state`）
- `vora.session.stuck_age_ms`（直方图，属性：`vora.state`）
- `vora.run.attempt`（计数器，属性：`vora.attempt`）
...
- `vora.webhook.error`
  - `vora.channel`、`vora.webhook`、`vora.chatId`、`vora.error`
- `vora.message.processed`
  - `vora.channel`、`vora.outcome`、`vora.chatId`、`vora.messageId`、`vora.sessionKey`、`vora.sessionId`、`vora.reason`
- `vora.session.stuck`
  - `vora.state`、`vora.ageMs`、`vora.queueDepth`、`vora.sessionKey`、`vora.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `vora doctor`。
