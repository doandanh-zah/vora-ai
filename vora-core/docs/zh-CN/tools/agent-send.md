...
read_when:
  - 添加或修改智能体 CLI 入口点
summary: 直接 `vora agent` CLI 运行（带可选投递）
title: Agent Send
x-i18n:
...
- 输出：
  ...
  - `--json`：打印结构化负载 + 元数据
- 可选使用 `--deliver` + `--channel` 将回复投递回渠道（目标格式与 `vora message --target` 匹配）。
...
vora agent --to +15555550123 --message "Summon reply" --deliver
