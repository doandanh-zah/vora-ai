...
CLI 新手引导是在 macOS、
Linux 或 Windows（通过 WSL2；强烈推荐）上设置 Vora 的**推荐**方式。
...
`vora dashboard` 并在浏览器中聊天。文档：[Dashboard](/web/dashboard)。
</Info>
...
`vora configure --section web` 进行配置。文档：[Web 工具](/tools/web)。
</Tip>
...
2. **工作区** —— 智能体文件的位置（默认 `~/.vora/workspace`）。会植入引导文件。
...
如果配置无效或包含旧版键，新手引导会先要求你运行 `vora doctor`。
</Note>
...
使用 `vora agents add <name>` 创建一个单独的智能体，它拥有自己的工作区、
...
- 默认工作区遵循 `~/.vora/workspace-<agentId>`。
...
- CLI 命令参考：[`vora onboard`](/cli/onboard)
