...
在 Vora 中，循环是每个会话的单次序列化运行，在模型思考、调用工具和流式输出时发出生命周期和流事件。本文档解释了这个真实循环是如何端到端连接的。
...
2. `agentCommand` 运行智能体：
   - 解析模型 + 思考/详细模式默认值
   - 加载 Skills 快照
   - 调用 `runEmbeddedVoraAgent`（vora-agent-core 运行时）
...
3. `runEmbeddedVoraAgent`：
   ...
   - 强制执行超时 -> 超时则中止运行
   - 返回有效负载 + 使用元数据
4. `subscribeEmbeddedPiSession` 将 vora-agent-core 事件桥接到 Vora `agent` 流：
   - 工具事件 => `stream: "tool"`
...
- 系统提示由 Vora 的基础提示、Skills 提示、引导上下文和每次运行的覆盖构建。
- 强制执行模型特定的限制和压缩保留令牌。
...
- 助手增量从 vora-agent-core 流式传输并作为 `assistant` 事件发出。
...
- `assistant`：从 vora-agent-core 流式传输的增量
- `tool`：从 vora-agent-core 流式传输的工具事件
...
- 智能体运行时：`agents.defaults.timeoutSeconds` 默认 172800 秒（48 小时）；在 `runEmbeddedVoraAgent` 中止计时器中强制执行。使用 `0` 可完全禁用超时。
