...
- Same path: run the app from a fixed location (for Vora, `dist/Vora.app`).
...
```bash
sudo tccutil reset Accessibility ai.vora.mac
sudo tccutil reset ScreenCapture ai.vora.mac
...
Workaround: move files into the Vora workspace (`~/.vora/workspace`) if you want to avoid per-folder grants.
