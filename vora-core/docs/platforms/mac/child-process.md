...
service via the external `vora` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.vora.gateway`
  (or `ai.vora.<profile>` when using `--profile`/`VORA_PROFILE`; legacy `com.vora.*` is supported).
