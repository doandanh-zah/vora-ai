---
summary: "Vora on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Vora on DigitalOcean
  - Looking for cheap VPS hosting for Vora
title: "DigitalOcean (Platform)"
---

# Vora on DigitalOcean

## Goal

Run a persistent Vora Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status vora-gateway.service

# View logs
journalctl --user -u vora-gateway.service -f
```
...
```bash
vora config set gateway.bind tailnet
vora gateway restart
```
...
- `~/.vora/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u vora --no-pager -n 50
```
