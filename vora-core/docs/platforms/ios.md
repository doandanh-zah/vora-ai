...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `vora.internal.`), **or**
...
Run `vora devices list` again before approval.

4. Verify connection:

```bash
vora nodes status
vora gateway call node.list --params "{}"
```
...
- `VORA_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Vora iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Vora builds.
...
```bash
export VORA_APNS_TEAM_ID="TEAMID"
export VORA_APNS_KEY_ID="KEYID"
export VORA_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_vora-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `vora.internal.`) and Tailscale split DNS.
...
```bash
vora nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__vora__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__vora__/canvas/` and `/__vora__/a2ui/`.
...
```bash
vora nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__vora; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
vora nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `vora devices list` and approve manually.
