---
summary: "Vora on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Vora on a Raspberry Pi
  - Running Vora on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# Vora on Raspberry Pi

## Goal

Run a persistent, always-on Vora Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
```bash
ssh user@gateway-host 'vora dashboard --no-open'
```
...
(or `VORA_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/vora-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/vora-compile-cache
mkdir -p /var/tmp/vora-compile-cache
export VORA_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `VORA_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Vora, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit vora
```

```ini
[Service]
Environment=VORA_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/vora-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Vora state/cache on SSD-backed storage to avoid SD-card
...
Most Vora features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled vora

# Enable if not
sudo systemctl enable vora

# Start on boot
sudo systemctl start vora
```
...
```bash
# Check logs
journalctl -u vora --no-pager -n 100

# Common fix: rebuild
cd ~/vora  # if using hackable install
npm run build
sudo systemctl restart vora
```
