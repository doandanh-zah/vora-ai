...
```bash
vora tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `vora tui --deliver`
...
- Local shell commands receive `VORA_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `VORA_THEME=light` before launching `vora tui`.
- To force the original dark palette instead, set `VORA_THEME=dark`.
...
- Check the Gateway logs: `vora logs --follow`.
- Confirm the agent can run: `vora status` and `vora models status`.
...
- No agents in picker: check `vora agents list` and your routing config.
