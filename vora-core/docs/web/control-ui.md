...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/vora`)
...
If the page fails to load, start the Gateway first: `vora gateway`.
...
created. Re-run `vora devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `vora devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.vora/vora.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop vora`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Vora
...
Vora **blocks** Control UI connections without device identity.
