...
Vora supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Vora also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `vora configure --section web`. It stores the key in
`~/.vora/vora.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.vora/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
