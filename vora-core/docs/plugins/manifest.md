...
read_when:
  - You are building an Vora plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (vora.plugin.json)

This page is for the **native Vora plugin manifest** only.
...
Vora auto-detects those bundle layouts too, but they are not validated
against the `vora.plugin.json` schema described here.

For compatible bundles, Vora currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches Vora runtime expectations.

Every native Vora plugin **must** ship a `vora.plugin.json` file in the
**plugin root**. Vora uses this manifest to validate configuration
...
`vora.plugin.json` is the metadata Vora reads before it loads your
plugin code.
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Vora can inspect without loading plugin code.                                      |
...
Vora reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, Vora falls back to `choiceId`.                                        |
...
Use `contracts` only for static capability ownership metadata that Vora can
...
`imageGenerationProviders` are deprecated. Use `vora doctor --fix` to move
...
| File                   | Use it for                                                                                                         |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `vora.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs           |
| `package.json`         | npm metadata, dependency installation, and the `vora` block used for entrypoints and setup or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Vora must know it before loading plugin code, put it in `vora.plugin.json`
...
- The manifest is **required for native Vora plugins**, including local filesystem loads.
