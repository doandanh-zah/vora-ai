...
summary: "Step-by-step guide to building a messaging channel plugin for Vora"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Vora to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects Vora to a
...
<Info>
  If you have not built any Vora plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Vora keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `vora/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
...
<Steps>
    ...
    ```json package.json
    {
      ...
      "vora": {
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Vora to Acme Chat."
        }
      }
    }
    ```

    ```json vora.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    } from "vora/plugin-sdk/core";
    import type { VoraConfig } from "vora/plugin-sdk/core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "vora/plugin-sdk/core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Vora
  ...
  <Step title="Add a setup entry">
    ...
    Vora loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    Vora. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Vora.
...
├── package.json              # vora.channel metadata
├── vora.plugin.json      # Manifest with config schema
