...
read_when:
  - You want to place an outbound voice call from Vora
...
Voice calls for Vora via a plugin. Supports outbound notifications and
...
- Use `vora voicecall ...` or the `voice_call` tool
...
vora plugins install "$PLUGIN_SRC"
...
```bash
vora voicecall call --to "+15555550123" --message "Hello from Vora"
vora voicecall start --to "+15555550123"   # alias for call
vora voicecall continue --call-id <id> --message "Any questions?"
vora voicecall speak --call-id <id> --message "One moment"
vora voicecall end --call-id <id>
vora voicecall status --call-id <id>
vora voicecall tail
vora voicecall latency                     # summarize turn latency from logs
vora voicecall expose --mode funnel
```
