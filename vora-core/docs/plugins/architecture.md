...
read_when:
  - Building or debugging native Vora plugins
...
This page covers the internal architecture of the Vora plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Vora. Every
native Vora plugin registers against one or more capability types:
...
Vora classifies every loaded plugin into a shape based on its actual
...
Use `vora plugins inspect <id>` to see a plugin's shape and capability
...
When you run `vora doctor` or `vora plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `vora status --all` and `vora plugins doctor`.

## Architecture overview

Vora's plugin system has four layers:

1. **Manifest + discovery**
   Vora finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `vora.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Vora plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Vora reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting Vora
...
That split lets Vora validate config, explain missing/disabled plugins, and
...
normal chat actions. Vora keeps one shared `message` tool in core, and
...
Vora treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Vora-facing
  surfaces
...
So if Vora adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Vora has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { VoraPluginDefinition } from "vora/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "vora/plugin-sdk/media-understanding";

const plugin: VoraPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Vora already treats image/audio/video understanding as one shared
...
If Vora adds a new domain later, such as video generation, use the same
...
`VoraPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Vora can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Vora knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `VoraPluginApi` or
  `api.runtime`
...
Native Vora plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because Vora currently treats them
...
name: `@vora/<id>` by default, or an approved typed suffix such as
...
Vora exports capabilities, not implementation convenience.
...
At startup, Vora does roughly this:
...
The manifest is the control-plane source of truth. Vora uses it to:
...
Vora keeps short in-process caches for:
...
- Set `VORA_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `VORA_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `VORA_PLUGIN_DISCOVERY_CACHE_MS` and
  `VORA_PLUGIN_MANIFEST_CACHE_MS`.
...
Vora still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Vora calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Vora tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on Vora's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Vora's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Vora adds those
...
- If Vora adds a new capability such as video generation later, define the
...
- Vora only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Vora plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `vora/plugin-sdk` import when
authoring plugins:

- `vora/plugin-sdk/plugin-entry` for plugin registration primitives.
- `vora/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `vora/plugin-sdk/channel-setup`,
  `vora/plugin-sdk/channel-pairing`,
  `vora/plugin-sdk/channel-contract`,
  `vora/plugin-sdk/channel-feedback`,
  `vora/plugin-sdk/channel-inbound`,
  `vora/plugin-sdk/channel-lifecycle`,
  `vora/plugin-sdk/channel-reply-pipeline`,
  `vora/plugin-sdk/command-auth`,
  `vora/plugin-sdk/secret-input`, and
  `vora/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `vora/plugin-sdk/channel-config-helpers`,
  `vora/plugin-sdk/allow-from`,
  `vora/plugin-sdk/channel-config-schema`,
  `vora/plugin-sdk/channel-policy`,
  `vora/plugin-sdk/config-runtime`,
  `vora/plugin-sdk/infra-runtime`,
  `vora/plugin-sdk/agent-runtime`,
  `vora/plugin-sdk/lazy-runtime`,
  `vora/plugin-sdk/reply-history`,
  `vora/plugin-sdk/routing`,
  `vora/plugin-sdk/status-helpers`,
  `vora/plugin-sdk/runtime-store`, and
  `vora/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `vora/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `vora/plugin-sdk/*` subpaths. Vora core/test code may use the repo
...
- Avoid the root `vora/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `vora/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `vora/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `vora/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `vora/plugin-sdk/channel-targets`; channel-specific
...
`vora/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Vora writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Vora's
...
- if both `catalog` and `discovery` are registered, Vora uses `catalog`
...
- Read-only command paths such as `vora status`, `vora status --all`,
  `vora channels status`, `vora channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `vora.extensions`:
...
Security guardrail: every `vora.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `vora plugins install` installs plugin dependencies with
...
Optional: `vora.setupEntry` can point at a lightweight setup-only module.
When Vora needs setup surfaces for a disabled channel plugin, or
...
Optional: `vora.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Vora load the
...
Channel plugins can advertise setup/discovery metadata via `vora.channel` and
install hints via `vora.install`. This keeps the core catalog data-free.
...
Vora can also merge **external channel catalogs** (for example, an MPM
...
Or point `VORA_PLUGIN_CATALOG_PATHS` (or `VORA_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "vora": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { delegateCompactionToRuntime } from "vora/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `VoraPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Vora stays opinionated without becoming hardcoded to one
