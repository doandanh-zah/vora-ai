...
read_when:
  ...
  - You are defining plugin config schemas or package.json vora metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`vora.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `vora` field that tells the plugin system what
...
```json vora-clawhub-package.json
{
  "name": "@myorg/vora-my-plugin",
  "version": "1.0.0",
  "type": "module",
  "vora": {
    "extensions": ["./index.ts"],
...
When enabled, Vora loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `vora.plugin.json` in the package root.
Vora uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Vora",
  "configSchema": {
    "type": "object",
...
Vora loads when it only needs setup surfaces (onboarding, config repair,
disabled channel inspection).

```typescript
// setup-entry.ts
import { defineSetupPluginEntry } from "vora/plugin-sdk/core";
...
**When Vora uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `vora/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Vora validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "vora/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `vora onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "vora/plugin-sdk/channel-setup";
...
helpers from `vora/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`vora/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `vora/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "vora/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/vora-my-channel",
...
```bash
vora plugins install @myorg/vora-my-plugin
```

Vora tries ClawHub first and falls back to npm automatically. You can also
force a specific source:

```bash
vora plugins install clawhub:@myorg/vora-my-plugin   # ClawHub only
vora plugins install npm:@myorg/vora-my-plugin       # npm only
```
...
```bash
vora plugins search <query>
vora plugins install <package-name>
```

<Info>
  For npm-sourced installs, `vora plugins install` runs
