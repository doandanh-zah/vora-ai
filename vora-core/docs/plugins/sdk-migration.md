...
read_when:
  - You see the VORA_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the VORA_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Vora plugin
---

# Plugin SDK Migration

Vora has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`vora/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`vora/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`vora/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "vora/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    import { resolveControlCommandGate } from "vora/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedVoraAgent } from "vora/extension-api";
    const result = await runEmbeddedVoraAgent({ sessionId, prompt });

    // After (injected runtime)
    const result = await api.runtime.agent.runEmbeddedVoraAgent({ sessionId, prompt });
    ```
...
```bash
VORA_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 vora gateway run
VORA_SUPPRESS_EXTENSION_API_WARNING=1 vora gateway run
```
