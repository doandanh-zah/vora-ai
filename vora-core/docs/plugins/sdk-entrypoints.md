...
**Import:** `vora/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `VoraPluginConfigSchema \| () => VoraPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: VoraPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `vora.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.

## `defineChannelPluginEntry`

**Import:** `vora/plugin-sdk/core`
...
| `registerCliMetadata` | `(api: VoraPluginApi) => void`                               | No       | —                   |
| `registerFull`        | `(api: VoraPluginApi) => void`                               | No       | —                   |
...
Vora loads this instead of the full entry when a channel is disabled,
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Vora to lazy-load the real CLI module on first invocation
...
Vora classifies loaded plugins by their registration behavior:
...
Use `vora plugins inspect <id>` to see a plugin's shape.
