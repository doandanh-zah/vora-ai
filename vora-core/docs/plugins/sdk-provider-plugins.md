...
summary: "Step-by-step guide to building a model provider plugin for Vora"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Vora
...
(LLM) to Vora. By the end you will have a provider with a model catalog,
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so Vora can detect
    credentials without loading your plugin runtime. If you publish the
    provider on ClawHub, those `vora.compat` and `vora.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "vora/plugin-sdk/provider-auth";
    ...
    `vora onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "vora/plugin-sdk/provider-entry";
    ...
    `vora/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      Vora calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Vora classifies this as a **hybrid-capability** plugin. This is the
...
├── package.json              # vora.providers metadata
├── vora.plugin.json      # Manifest with providerAuthEnvVars
