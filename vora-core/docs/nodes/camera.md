...
Vora supports **camera capture** for agent workflows:
...
```bash
vora nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
vora nodes camera snap --node <id> --facing front
vora nodes camera clip --node <id> --duration 3000
vora nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`vora.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `vora` CLI to invoke camera commands on the macOS node.

Examples:

```bash
vora nodes camera list --node <id>            # list camera ids
vora nodes camera snap --node <id>            # prints MEDIA:<path>
vora nodes camera snap --node <id> --max-width 1280
vora nodes camera snap --node <id> --delay-ms 2000
vora nodes camera snap --node <id> --device-id <id>
vora nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
vora nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
vora nodes camera clip --node <id> --device-id <id>
vora nodes camera clip --node <id> --no-audio
```

Notes:

- `vora nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
vora nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
