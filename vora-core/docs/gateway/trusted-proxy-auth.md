...
read_when:
  - Running Vora behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Vora
...
- You run Vora behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Vora checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Vora extracts the user identity from the configured header
...
- Vora can stay on loopback HTTP behind the proxy.
...
If Vora itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://vora.example.com
    to: http://vora-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
Vora rejects ambiguous configurations where both a `gateway.auth.token` (or `VORA_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
`vora security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Vora config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `vora security audit` and review findings
