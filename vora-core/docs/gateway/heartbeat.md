...
- During heartbeat runs, Vora treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `vora sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Vora skips the heartbeat run entirely (no model call).
