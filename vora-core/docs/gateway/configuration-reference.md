...
summary: "Complete reference for every Vora config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.vora/vora.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Vora uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.vora/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `vora doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `vora doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["vora-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-vora",
          requireMention: false,
...
- Vora additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `vora plugins install @vora/mattermost`.
...
- `commands.callbackUrl` must resolve to the Vora gateway endpoint and be reachable from the Mattermost server.
...
Vora spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `vora channels add` (or channel onboarding) while still on a single-account top-level channel config, Vora moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `vora doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@vora", "vora"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@vora"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `vora.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Vora auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/vora" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Vora assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Vora aborts it. Default: `900`.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.vora/sandboxes",
        docker: {
          image: "vora-sandbox:bookworm-slim",
          containerPrefix: "vora-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/vora-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "vora-sandbox-browser:bookworm-slim",
          network: "vora-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Vora materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.vora/sandboxes`
...
In `remote` mode, host-local edits made outside Vora are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `vora.json`.
noVNC observer access uses VNC auth by default and Vora emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `vora-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from VORA_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `VORA_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `VORA_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `VORA_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.vora/workspace",
        agentDir: "~/.vora/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@vora"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.vora/workspace-home" },
      { id: "work", workspace: "~/.vora/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Vora resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.vora/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.vora/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.vora/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.vora/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Vora starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.vora/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Vora treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:vora`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.vora/attachments/<uuid>/` with a `.manifest.json`.
...
Vora uses the built-in model catalog. Add custom providers via `models.providers` in config or `~/.vora/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `VORA_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Vora forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `vora onboard --auth-choice opencode-zen` or `vora onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `vora onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `vora onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `vora onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `vora onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `vora onboard --auth-choice minimax-api`.
...
- Loaded from `~/.vora/extensions`, `<workspace>/.vora/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Vora plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Vora plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Vora applies those as sanitized agent settings, not as raw Vora config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `vora plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      vora: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or VORA_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `VORA_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `VORA_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `VORA_APNS_RELAY_BASE_URL` / `VORA_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `VORA_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
VORA_CONFIG_PATH=~/.vora/a.json \
VORA_STATE_DIR=~/.vora-a \
vora gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.vora-dev` + port `19001`), `--profile <name>` (uses `~/.vora-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/vora/tls/server.crt",
      keyPath: "/etc/vora/tls/server.key",
      caPath: "/etc/vora/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.vora/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-vora-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `VORA_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.vora/workspace/canvas",
    liveReload: true,
    // enabled: false, // or VORA_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__vora__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__vora__/a2ui/`
...
- Also serves A2UI at `/__vora__/a2ui/`.
...
- Hostname defaults to `vora`. Override with `VORA_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.vora/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `vora dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.vora/.env` (neither overrides existing vars).
...
- `secrets apply` targets supported `vora.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.vora/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/vora-vault-resolver",
...
- Legacy OAuth imports from `~/.vora/credentials/oauth.json`.
...
- Default log file: `/tmp/vora/vora-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Vora.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `VORA_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `vora doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `vora.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
