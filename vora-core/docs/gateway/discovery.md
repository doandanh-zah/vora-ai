...
Vora has two distinct problems that look similar on the surface:

1. **Operator remote control**: the macOS menu bar app controlling a gateway running elsewhere.
2. **Node pairing**: iOS/Android (and future nodes) finding a gateway and pairing securely.

The design goal is to keep all network discovery/advertising in the **Node Gateway** (`vora gateway`) and keep clients (mac app, iOS) as consumers.
...
- Service types:
  - `_vora-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  - `transport=gateway`
  ...
  - `cliPath=<path>` (optional; absolute path to a runnable `vora` entrypoint or binary)
...
- `VORA_DISABLE_BONJOUR=1` disables advertising.
- `gateway.bind` in `~/.vora/vora.json` controls the Gateway bind mode.
- `VORA_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
- `VORA_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `VORA_CLI_PATH` overrides the advertised CLI path.
