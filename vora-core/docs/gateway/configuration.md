...
read_when:
  - Setting up Vora for the first time
...
Vora reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.vora/vora.json`.

If the file is missing, Vora uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `vora onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    vora onboard       # full onboarding flow
    vora configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    vora config get agents.defaults.workspace
    vora config set agents.defaults.heartbeat.every "2h"
    vora config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.vora/vora.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Vora only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`vora doctor`, `vora logs`, `vora health`, `vora status`)
- Run `vora doctor` to see exact issues
- Run `vora doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `vora.json`.
    ...
    ```bash
    vora config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `VORA_APNS_RELAY_BASE_URL` and `VORA_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `VORA_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.vora/vora.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `vora config set` for single keys.
    </Warning>
    ...
    ```bash
    vora gateway call config.get --params '{}'  # capture payload.hash
    vora gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.vora/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    vora gateway call config.patch --params '{
...
Vora reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.vora/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Vora runs your login shell and imports only the missing keys:
...
Env var equivalent: `VORA_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${VORA_GATEWAY_TOKEN}" } },
