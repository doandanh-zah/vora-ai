---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Vora agents"
read_when:
...
OpenShell is a managed sandbox backend for Vora. Instead of running Docker
containers locally, Vora delegates sandbox lifecycle to the `openshell` CLI,
...
- Vora Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Vora creates an OpenShell
...
- You edit files locally outside Vora and want those changes visible in the
...
- When the sandbox is first created, Vora seeds the remote workspace from
...
- Vora does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Vora after the initial seed,
the remote sandbox does **not** see those changes. Use
`vora sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"vora"`  | Sandbox source for first-time create                  |
...
1. Vora calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Vora calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `vora sandbox` commands
