...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Vora runs”).
...
- `vora health` and `vora status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `vora gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`VORA_GATEWAY_PORT`).
...
- URL override safety:
  ...
  - Env URL overrides (`VORA_GATEWAY_URL`) may use env credentials only (`VORA_GATEWAY_TOKEN` / `VORA_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `VORA_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `VORA_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `VORA_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `VORA_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `VORA_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `VORA_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
```bash
vora config set gateway.remote.token "<your-token>"
```

#### Step 4: create the LaunchAgent

Save this as `~/Library/LaunchAgents/ai.vora.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.vora.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.vora.ssh-tunnel.plist
```

The tunnel will start automatically at login, restart on crash, and keep the forwarded port live.

Note: if you have a leftover `com.vora.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
...
```bash
launchctl kickstart -k gui/$UID/ai.vora.ssh-tunnel
```

Stop the tunnel:

```bash
launchctl bootout gui/$UID/ai.vora.ssh-tunnel
```
