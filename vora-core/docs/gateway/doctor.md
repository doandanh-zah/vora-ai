...
`vora doctor` is the repair + migration tool for Vora. It fixes stale
...
```bash
vora doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
vora doctor --non-interactive
```
...
- Extra workspace dir detection (`~/vora`).
...
you to run `vora doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.vora/vora.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.vora/sessions/` to `~/.vora/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.vora/agent/` to `~/.vora/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.vora/credentials/*.json` (except `oauth.json`)
  - to `~/.vora/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `vora doctor`.
...
Doctor also checks the cron job store (`~/.vora/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.vora` folders exist across
  home directories or when `VORA_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.vora/vora.json` is
...
offers to remove them and install the Vora service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Vora gateway services are considered first-class and are not
flagged as "extra."
...
startup continues. In read-only mode (`vora doctor` without `--fix`) this check
is skipped entirely.
...
- **Legacy workspace dirs**: warns when `~/vora` or other legacy workspace directories
...
- If the shell profile uses a slow dynamic completion pattern
  (`source <(vora completion ...)`), doctor upgrades it to the faster
  cached file variant.
...
Run `vora completion --write-state` to regenerate the cache manually.
...
- `vora doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `vora doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
Use `vora memory status --deep` to verify embedding readiness at runtime.
...
- `vora doctor` prompts before rewriting supervisor config.
- `vora doctor --yes` accepts the default repair prompts.
- `vora doctor --repair` applies recommended fixes without prompts.
- `vora doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `vora gateway install --force`.
