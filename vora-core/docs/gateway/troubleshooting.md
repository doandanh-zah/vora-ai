...
- `vora gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `vora doctor` reports no blocking config/service issues.
- `vora channels status --probe` shows connected/ready channels.
...
vora logs --follow
vora models status
vora config get agents.defaults.models
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `vora config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `vora devices list` then `vora devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `vora configure`). If you are running Vora via Podman, the default config path is `~/.vora/vora.json`.
...
vora approvals get --node <idOrNameOrIp>
vora logs --follow
vora status
```
...
```bash
vora browser status
vora browser start --browser-profile vora
...
```bash
vora gateway status
vora config get gateway.mode
vora config get gateway.remote.url
vora config get gateway.auth.mode
```
...
```bash
vora config get gateway.bind
vora config get gateway.auth.token
vora gateway status
vora logs --follow
```
