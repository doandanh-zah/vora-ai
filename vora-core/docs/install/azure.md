---
summary: "Run Vora Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Vora running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Vora Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# Vora on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs Vora.
...
- Install Vora with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    VM_SUBNET_NAME="snet-vora-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-vora-vm"
    VM_NAME="vm-vora"
    ADMIN_USERNAME="vora"
    BASTION_NAME="bas-vora"
    BASTION_PIP_NAME="pip-vora-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Vora (in the VM shell)">
    ```bash
    curl -fsSL https://vora.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node LTS and dependencies if not already present, installs Vora, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Vora onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The Vora Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on Vora Azure deployment with the GitHub Copilot model provider: [Vora on Azure with GitHub Copilot](https://github.com/johnsonshi/vora-azure-github-copilot)
