---
summary: "ClawDock shell helpers for Docker-based Vora installs"
read_when:
  - You run Vora with Docker often and want shorter day-to-day commands
...
ClawDock is a small shell-helper layer for Docker-based Vora installs.
...
| `clawdock-cli <command>`  | Run Vora CLI commands in Docker           |
...
| `clawdock-cd`          | Jump to the Vora project directory  |
| `clawdock-config`      | Open `~/.vora`                      |
...
- `~/.vora/.env` for provider keys and bot tokens
- `~/.vora/vora.json` for behavior config
