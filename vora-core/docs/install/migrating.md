---
summary: "Move (migrate) an Vora install from one machine to another"
read_when:
  - You are moving Vora to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Vora to a New Machine

This guide moves an Vora gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.vora/` by default) and your **workspace**, you preserve:

- **Config** -- `vora.json` and all gateway settings
...
<Tip>
Run `vora status` on the old machine to confirm your state directory path.
Custom profiles use `~/.vora-<profile>/` or a path set via `VORA_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    vora gateway stop
    cd ~
    tar -czf vora-state.tgz .vora
    ```

    If you use multiple profiles (e.g. `~/.vora-work`), archive each separately.

  </Step>

  <Step title="Install Vora on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.vora/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `VORA_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `vora doctor`.
  </Accordion>

  <Accordion title="Copying only vora.json">
...
- [ ] `vora status` shows the gateway running
