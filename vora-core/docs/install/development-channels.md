...
Vora ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/vora`, override with
  `VORA_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
vora update --tag 2026.4.1-beta.1
...
vora update --tag vora@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `vora update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Vora prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `vora update` would do without making changes:

```bash
vora update --dry-run
vora update --channel beta --dry-run
vora update --tag 2026.4.1-beta.1 --dry-run
vora update --dry-run --json
```
...
When you switch channels with `vora update`, Vora also syncs plugin
sources:
