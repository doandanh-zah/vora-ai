---
summary: "Host Vora on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Vora on Oracle Cloud
  - Looking for free VPS hosting for Vora
...
Run a persistent Vora Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@vora`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=vora --reset` to re-authenticate.

**Gateway will not start** -- Run `vora doctor --non-interactive` and check logs with `journalctl --user -u vora-gateway -n 50`.
...
- [Updating](/install/updating) -- keep Vora up to date
