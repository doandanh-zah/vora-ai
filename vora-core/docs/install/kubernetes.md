---
summary: "Deploy Vora Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Vora on a Kubernetes cluster
  - You want to test Vora in a Kubernetes environment
title: "Kubernetes"
---

# Vora on Kubernetes

A minimal starting point for running Vora on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Vora is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/vora 18789:18789 -n vora
...
```bash
kubectl get secret vora-secrets -n vora -o jsonpath='{.data.VORA_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: vora (configurable via VORA_NAMESPACE)
├── Deployment/vora        # Single pod, init container + gateway
├── Service/vora           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/vora-config  # vora.json + AGENTS.md
└── Secret/vora-secrets    # Gateway token + API keys
```
...
Edit `vora.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret vora-secrets -n vora \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/vora -n vora
```

### Custom namespace

```bash
VORA_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/vora/vora:latest # or pin to a specific version from https://github.com/vora/vora/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # vora.json + AGENTS.md
