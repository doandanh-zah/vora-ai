---
title: Fly.io
summary: "Step-by-step Fly.io deployment for Vora with persistent storage and HTTPS"
read_when:
  - Deploying Vora on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** Vora Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
<Steps>
  <Step title="Create the Fly app">
    ...
    fly apps create my-vora

    # Create a persistent volume (1GB is usually enough)
    fly volumes create vora_data --size 1 --region iad
    ```
  ...
  <Step title="Configure fly.toml">
    ...
    **Key settings:**
    ...
    | `internal_port = 3000`         | Must match `--port 3000` (or `VORA_GATEWAY_PORT`) for Fly health checks |
    | `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
    | `VORA_STATE_DIR = "/data"` | Persists state on the volume                                                |
  ...
  <Step title="Set secrets">
    ...
    fly secrets set VORA_GATEWAY_TOKEN=$(openssl rand -hex 32)
    ...
    **Notes:**
    ...
    - Non-loopback binds (`--bind lan`) require `VORA_GATEWAY_TOKEN` for security.
    - Treat these tokens like passwords.
    - **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `vora.json` where they could be accidentally exposed or logged.
  ...
  <Step title="Create config file">
    ...
    **Note:** With `VORA_STATE_DIR=/data`, the config path is `/data/vora.json`.
  ...
  <Step title="Access the Gateway">
    ...
    Or visit `https://my-vora.fly.dev/`

    Paste your gateway token (the one from `VORA_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `VORA_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/vora.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/vora.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/vora.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/vora.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/vora.json"
```
...
**Fix:** Ensure `VORA_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-vora

# Release public IPs
fly ips release <public-ipv4> -a my-vora
fly ips release <public-ipv6> -a my-vora
...
fly ips allocate-v6 --private -a my-vora
```
