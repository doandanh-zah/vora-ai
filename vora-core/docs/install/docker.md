---
summary: "Optional Docker-based setup and onboarding for Vora"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Vora on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export VORA_IMAGE="ghcr.io/vora/vora:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/vora/vora/pkgs/container/vora).
  ...
  <Step title="Complete onboarding">
    ...
    `vora-gateway` directly. `vora-cli` is for commands you run after
...
docker compose run --rm --no-deps --entrypoint node vora-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `VORA_EXTRA_MOUNTS`
or `VORA_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `vora-cli` shares `vora-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d vora-gateway`, run onboarding
and setup-time config writes through `vora-gateway` with
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `VORA_IMAGE`               | Use a remote image instead of building locally                   |
| `VORA_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `VORA_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `VORA_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `VORA_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `VORA_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `VORA_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
`scripts/docker/setup.sh` defaults `VORA_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `VORA_CONFIG_DIR` to `/home/node/.vora` and
`VORA_WORKSPACE_DIR` to `/home/node/.vora/workspace`, so those paths
...
and rolling file logs under `/tmp/vora/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/vora/vora/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export VORA_SANDBOX=1
    export VORA_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm vora-cli gateway probe
    docker compose run -T --rm vora-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `vora-cli` uses `network_mode: "service:vora-gateway"` so CLI
    ...
    `no-new-privileges` on `vora-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.vora`, make sure your host bind mounts are owned by uid 1000:
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export VORA_HOME_VOLUME="vora_home"`
    2. **Bake system deps**: `export VORA_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `VORA_HOME_VOLUME` or `VORA_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/vora/vora/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Vora runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm vora-cli config set gateway.mode local
    docker compose run --rm vora-cli config set gateway.bind lan
    docker compose run --rm vora-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping Vora up to date
