---
summary: "Run Vora Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Vora running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Vora on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Vora Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Vora 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Vora Gateway in Docker
- Persist `~/.vora` + `~/.vora/workspace` on the host (survives restarts/rebuilds)
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-vora-project --name="Vora Gateway"
    gcloud config set project my-vora-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create vora-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh vora-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Vora repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    VORA_IMAGE=vora:latest
    VORA_GATEWAY_TOKEN=change-me-now
    VORA_GATEWAY_BIND=lan
    VORA_GATEWAY_PORT=18789

    VORA_CONFIG_DIR=/home/$USER/.vora
    VORA_WORKSPACE_DIR=/home/$USER/.vora/workspace

    GOG_KEYRING_PASSWORD=change-me-now
    XDG_CONFIG_HOME=/home/node/.vora
    ```
    ...
    services:
      vora-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - VORA_GATEWAY_BIND=${VORA_GATEWAY_BIND}
          - VORA_GATEWAY_PORT=${VORA_GATEWAY_PORT}
          - VORA_GATEWAY_TOKEN=${VORA_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${VORA_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`VORA_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm vora-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh vora-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop vora-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type vora-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start vora-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create vora-deploy \
     --display-name="Vora Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-vora-project \
     --member="serviceAccount:vora-deploy@my-vora-project.iam.gserviceaccount.com" \
