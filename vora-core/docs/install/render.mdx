---
summary: "Deploy Vora on Render with Infrastructure-as-Code"
read_when:
  - Deploying Vora to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Vora on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/vora/vora)
...
repository configures everything needed to run Vora:
...
services:
  - type: web
    ...
    envVars:
      - key: VORA_GATEWAY_PORT
        value: "8080"
      - key: VORA_STATE_DIR
        value: /data/.vora
      - key: VORA_WORKSPACE_DIR
        value: /data/workspace
      - key: VORA_GATEWAY_TOKEN
...
Connect using the `VORA_GATEWAY_TOKEN` value that was auto-generated during deploy (find it in **Dashboard → your service → Environment**).
...
If you use the original Vora repository, Render will not auto-deploy your Vora. To update it, run a manual Blueprint sync from the dashboard.
...
For Vora, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive you can restore on any Vora host. See [Backup](/cli/backup) for details.
...
- Missing `VORA_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `VORA_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export your config via `vora backup create` in the Render shell.
