...
CLI onboarding is the **recommended** way to set up Vora on macOS,
...
`vora dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`vora configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.vora/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `vora doctor` first.
</Note>
...
Use `vora agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.vora/workspace-<agentId>`.
...
- CLI command reference: [`vora onboard`](/cli/onboard)
