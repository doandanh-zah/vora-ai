...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Vora
...
Running Vora via Node with `tsx` fails at startup with:

```
[vora] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `vora status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch vora.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node vora.mjs status` works on Node 25.
