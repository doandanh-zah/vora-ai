...
This guide summarizes a sane workflow for working on the pi integration in Vora.
...
```bash
VORA_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm vora agent --message "Hello" --thinking low`
...
State lives under the Vora state directory. Default is `~/.vora`. If `VORA_STATE_DIR` is set, use that directory instead.

To reset everything:

- `vora.json` for config
