...
- **gateway host** → `vora` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Vora also tries to bind
...
When `tools.exec.strictInlineEval=true`, Vora treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.vora/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `vora approvals allowlist ...`).
- `vora security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `vora doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Vora still rejects the `env` builtin in safe-bin
...
`~/.vora/exec-approvals.json` directly.

CLI: `vora approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, Vora sends a followup `agent` turn to the same session.
...
topics, Vora preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, Vora prevents the agent from reusing
