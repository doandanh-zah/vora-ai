...
Vora can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Vora can send audio.
...
When `messages.tts.provider` is unset, Vora picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `vora.json`.
...
- If `provider` is **unset**, Vora uses the first configured speech provider in registry auto-select order.
...
`~/.vora/settings/tts.json`, override with `VORA_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Vora retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Vora registers
