...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.vora/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Vora picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    vora agent --message "give me a greeting"
    ```
...
| `metadata.vora.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.vora.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.vora.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `vora agent --message "..."` to test before sharing
...
| `~/.vora/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Vora) | Lowest     | Global                |
