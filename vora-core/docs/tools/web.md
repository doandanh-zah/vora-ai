...
Vora also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Vora plugin SDK" });
    ```
...
Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of Vora's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, Vora keeps the normal managed `web_search` behavior.
...
If no `provider` is set, Vora checks for API keys in this order and uses
the first one found:
...
<Note>
  ...
  Vora resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
When you choose Grok during `vora onboard` or `vora configure --section web`,
Vora can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, Vora does not
...
<Tabs>
  <Tab title="Config file">
    Run `vora configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.vora/.env`.
...
optional structured filters. Vora only enables the built-in xAI `x_search`
