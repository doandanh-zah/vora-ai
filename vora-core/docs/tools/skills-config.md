...
All skills-related configuration lives under `skills` in `~/.vora/vora.json`.
...
- Built-in skill roots always include `~/.vora/skills`, `~/.agents/skills`,
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.vora.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.vora/skills` → bundled skills →
