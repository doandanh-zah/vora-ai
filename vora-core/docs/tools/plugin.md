---
summary: "Install, configure, and manage Vora plugins"
read_when:
...
Plugins extend Vora with new capabilities: channels, model providers, tools,
skills, speech, image generation, and more. Some plugins are **core** (shipped
with Vora), others are **external** (published on npm by the community).
...
<Steps>
  ...
  <Step title="Install a plugin">
    ...
    vora plugins install ./my-plugin
    vora plugins install ./my-plugin.tgz
    ```
...
```text
/plugin install clawhub:@vora/voice-call
...
Vora recognizes two plugin formats:

| Format     | How it works                                                       | Examples                                               |
| ---------- | ------------------------------------------------------------------ | ------------------------------------------------------ |
| **Native** | `vora.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/Claude/Cursor-compatible layout; mapped to Vora features | `.codex-plugin/`, `.claude-plugin/`, `.cursor-plugin/` |

Both show up under `vora plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
| Plugin          | Package                | Docs                                 |
| --------------- | ---------------------- | ------------------------------------ |
| Matrix          | `@vora/matrix`     | [Matrix](/channels/matrix)           |
| Microsoft Teams | `@vora/msteams`    | [Microsoft Teams](/channels/msteams) |
| Nostr           | `@vora/nostr`      | [Nostr](/channels/nostr)             |
| Voice Call      | `@vora/voice-call` | [Voice Call](/plugins/voice-call)    |
| Zalo            | `@vora/zalo`       | [Zalo](/channels/zalo)               |
| Zalo Personal   | `@vora/zalouser`   | [Zalo Personal](/plugins/zalouser)   |

### Core (shipped with Vora)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` — bundled browser plugin for the browser tool, `vora browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
watch + in-process restart enabled (the default `vora gateway` path), that
...
Vora scans for plugins in this order (first match wins):
...
<Steps>
  ...
  <Step title="Workspace extensions">
    `\<workspace\>/.vora/<plugin-root>/*.ts` and `\<workspace\>/.vora/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global extensions">
    `~/.vora/<plugin-root>/*.ts` and `~/.vora/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with Vora. Many are enabled by default (model providers, speech).
...
```bash
vora plugins list                    # compact inventory
vora plugins inspect <id>            # deep detail
vora plugins inspect <id> --json     # machine-readable
vora plugins status                  # operational summary
vora plugins doctor                  # diagnostics

vora plugins install <package>        # install (ClawHub first, then npm)
vora plugins install clawhub:<pkg>   # install from ClawHub only
vora plugins install <path>          # install from local path
vora plugins install -l <path>       # link (no copy) for dev
vora plugins install <spec> --dangerously-force-unsafe-install
vora plugins update <id>             # update one plugin
vora plugins update --all            # update all
...
instead, while `vora skills install` remains the separate ClawHub skill
download/install flow.

See [`vora plugins` CLI reference](/cli/plugins) for full details.
