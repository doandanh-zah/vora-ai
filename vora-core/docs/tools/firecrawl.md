...
Vora can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `vora configure --section web` enables the bundled Firecrawl plugin automatically.
...
Vora always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
