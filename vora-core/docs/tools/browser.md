...
read_when:
  ...
  - Debugging why vora is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (Vora-managed)"
---

# Browser (vora-managed)

Vora can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `vora` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **vora** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`vora`, `work`, `remote`, ...).
...
```bash
vora browser --browser-profile vora status
...
If `vora browser` is missing entirely, or the agent says the browser tool
...
If you turn off only the plugin, the bundled browser CLI (`vora browser`),
...
If `vora browser` suddenly becomes an unknown command after an upgrade, or
...
Set `browser.defaultProfile: "vora"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.vora/vora.json`.
...
- If you override the Gateway port (`gateway.port` or `VORA_GATEWAY_PORT`),
...
- Default profile is `vora` (Vora-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `vora` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
Vora uses it automatically. Set `browser.executablePath` to override
auto-detection:

CLI example:

```bash
vora config set browser.executablePath "/usr/bin/google-chrome"
```
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Vora will not launch a local browser.
...
Vora preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Vora can
...
- If you set `nodeHost.browserProxy.allowProfiles`, Vora treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. Vora can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let Vora
  discover `/json/version`.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Vora supports both:

- **HTTP(S) endpoints** — Vora calls `/json/version` to discover the
  WebSocket debugger URL, then connects.
- **WebSocket endpoints** (`ws://` / `wss://`) — Vora connects directly,
...
- If browser control is enabled and no auth is configured, Vora auto-generates `gateway.auth.token` on startup and persists it to config.
...
Vora supports multiple named profiles (routing configs). Profiles can be:

- **vora-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `vora` profile is auto-created if missing.
...
Vora can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Vora attaches.
...
```bash
vora browser --browser-profile user start
vora browser --browser-profile user status
vora browser --browser-profile user tabs
vora browser --browser-profile user snapshot --format ai
```
...
- `vora doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `vora` profile because it can
  act inside your signed-in browser session.
- Vora does not launch the browser for this driver; it attaches to an
  existing session only.
- Vora uses the official Chrome DevTools MCP `--autoConnect` flow here. If
  `userDataDir` is set, Vora passes it through to target that explicit
...
When launching locally, Vora picks the first available:
...
- `x-vora-password: <gateway password>` or HTTP Basic auth with that password

### Playwright requirement

Some features (navigate/act/AI snapshot/role snapshot, element screenshots, PDF) require
Playwright. If Playwright isn’t installed, those endpoints return a clear 501
error. ARIA snapshots and basic screenshots still work for vora-managed Chrome.

If you see `Playwright is not available in this gateway build`, install the full
Playwright package (not `playwright-core`) and restart the gateway, or reinstall
Vora with browser support.
...
```bash
docker compose run --rm vora-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

To persist browser downloads, set `PLAYWRIGHT_BROWSERS_PATH` (for example,
`/home/node/.cache/ms-playwright`) and make sure `/home/node` is persisted via
`VORA_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `vora browser tabs`
- `vora browser tab`
- `vora browser tab new`
- `vora browser tab select 2`
- `vora browser tab close 2`
...
- `vora browser screenshot --ref e12`
- `vora browser snapshot`
- `vora browser snapshot --format aria --limit 200`
- `vora browser snapshot --interactive --compact --depth 6`
- `vora browser snapshot --efficient`
- `vora browser snapshot --labels`
- `vora browser snapshot --selector "#main" --interactive`
- `vora browser snapshot --frame "iframe#main" --interactive`
- `vora browser console --level error`
- `vora browser errors --clear`
- `vora browser requests --filter api --clear`
- `vora browser pdf`
- `vora browser responsebody "**/api" --max-chars 5000`
...
- `vora browser click e12 --double`
- `vora browser type 23 "hello" --submit`
- `vora browser press Enter`
- `vora browser hover 44`
- `vora browser scrollintoview e12`
- `vora browser drag 10 11`
- `vora browser select 9 OptionA OptionB`
- `vora browser download e12 report.pdf`
- `vora browser waitfordownload report.pdf`
- `vora browser upload /tmp/vora/uploads/file.pdf`
- `vora browser fill --fields '[{"ref":"1","type":"text","value":"Ada"}]'`
- `vora browser dialog --accept`
- `vora browser wait --text "Done"`
- `vora browser wait "#main" --url "**/dash" --load networkidle --fn "window.ready===true"`
- `vora browser evaluate --fn '(el) => el.textContent' --ref 7`
- `vora browser highlight e12`
- `vora browser trace start`
- `vora browser trace stop`

State:

- `vora browser cookies`
- `vora browser cookies set session abc123 --url "https://example.com"`
- `vora browser cookies clear`
- `vora browser storage local get`
- `vora browser storage local set theme dark`
- `vora browser storage session clear`
- `vora browser set offline on`
- `vora browser set headers --headers-json '{"X-Debug":"1"}'`
- `vora browser set credentials user pass`
- `vora browser set credentials --clear`
- `vora browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- `vora browser set geo --clear`
- `vora browser set media dark`
- `vora browser set timezone America/New_York`
- `vora browser set locale en-US`
- `vora browser set device "iPhone 14"`

Notes:

- `upload` and `dialog` are **arming** calls; run them before the click/press
  that triggers the chooser/dialog.
- Download and trace output paths are constrained to Vora temp roots:
  - traces: `/tmp/vora` (fallback: `${os.tmpdir()}/vora`)
  - downloads: `/tmp/vora/downloads` (fallback: `${os.tmpdir()}/vora/downloads`)
- Upload paths are constrained to an Vora temp uploads root:
  - uploads: `/tmp/vora/uploads` (fallback: `${os.tmpdir()}/vora/uploads`)
...
Vora supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `vora browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `vora browser click 12`, `vora browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `vora browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `vora browser click e12`, `vora browser highlight e12`.
...
- Wait for URL (globs supported by Playwright):
  - `vora browser wait --url "**/dash"`
- Wait for load state:
  - `vora browser wait --load networkidle`
- Wait for a JS predicate:
  - `vora browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `vora browser wait "#main"`
...
1. `vora browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `vora browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `vora browser trace start`
   - reproduce the issue
   - `vora browser trace stop` (prints `TRACE:<path>`)
...
```bash
vora browser status --json
vora browser snapshot --interactive --json
vora browser requests --filter api --json
vora browser cookies --json
```
...
- The vora browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `vora browser evaluate` and `wait --fn`
...
- `browser` accepts:
  - `profile` to choose a named browser profile (vora, chrome, or remote CDP).
