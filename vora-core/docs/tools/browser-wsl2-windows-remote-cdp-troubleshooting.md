...
read_when:
  - Running Vora Gateway in WSL2 while Chrome lives on Windows
...
- Vora Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/vora/vora/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Vora and Chrome are on the same machine
...
- Vora points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Vora is not the problem yet.
...
Fix that before touching Vora config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Vora at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Vora to succeed
...
```bash
vora browser open https://example.com --browser-profile remote
vora browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `vora browser tabs` returns the target
...
3. Vora config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Vora config or Control UI auth
