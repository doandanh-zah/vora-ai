---
summary: "ClawHub guide: public registry, native Vora install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Vora skills and plugins**.

- Use native `vora` commands to search/install/update skills and install
...
```bash
vora plugins install clawhub:<package>
...
Native `vora` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for Vora skills and plugins.
...
4. Users browse, download, and install skills in Vora.
...
If you want to add new capabilities to your Vora agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `vora skills install <skill-slug>`
3. Start a new Vora session so it picks up the new skill.
...
## How it fits into Vora

Native `vora skills install` installs into the active workspace `skills/`
directory. `vora plugins install clawhub:...` records a normal managed
...
current working directory. If an Vora workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Vora loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.vora/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Vora how to perform a
...
Interested in becoming a moderator? Ask in the Vora Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Vora workspace).
...
Code plugins must include the required Vora metadata in `package.json`:
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/vora/skills` and `~/.vora/skills`). This is designed to find older skill installs without extra flags.
