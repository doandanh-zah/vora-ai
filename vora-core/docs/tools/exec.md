...
- `gateway`/`node` approvals are controlled by `~/.vora/exec-approvals.json`.
...
- Vora sets `VORA_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Vora prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`vora security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `vora doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`vora security audit` and `vora doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
