...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `vora sandbox explain`.
...
Legacy `agent.*` configs are migrated by `vora doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=vora-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${VORA_STATE_DIR:-$HOME/.vora}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
