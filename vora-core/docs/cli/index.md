---
summary: "Vora CLI reference for `vora` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.vora-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.vora-<name>`.
- `--container <name>`: target a named container for execution.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `vora update` (source installs only).
...
Vora uses a lobster palette for CLI output.
...
- `vora security audit` — audit config + local state for common security foot-guns.
- `vora security audit --deep` — best-effort live Gateway probe.
- `vora security audit --fix` — tighten safe defaults and chmod state/config.

## Secrets

- `vora secrets reload` — re-resolve refs and atomically swap the runtime snapshot.
- `vora secrets audit` — scan for plaintext residues, unresolved refs, and precedence drift (`--allow-exec` to execute exec providers during audit).
- `vora secrets configure` — interactive helper for provider setup + SecretRef mapping + preflight/apply (`--allow-exec` to execute exec providers during preflight and exec-containing apply flows).
- `vora secrets apply --from <plan.json>` — apply a previously generated plan (`--dry-run` supported; use `--allow-exec` to permit exec providers in dry-run and exec-containing write plans).

## Plugins

Manage extensions and their config:

- `vora plugins list` — discover plugins (use `--json` for machine output).
- `vora plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `vora plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `vora plugins marketplace list <marketplace>` — list marketplace entries before install.
- `vora plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `vora plugins doctor` — report plugin load errors.
...
- `vora memory status` — show index stats.
- `vora memory index` — reindex memory files.
- `vora memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
...
- `--workspace <dir>`: agent workspace path (default `~/.vora/workspace`).
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `vora config` with no
...
- `config set --dry-run`: validate assignments without writing `vora.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `vora.json`.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `vora health` or `vora status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `vora doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, Vora moves account-scoped values into `channels.<channel>.accounts.default` before writing the new account.
...
```bash
vora channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
vora channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
vora channels remove --channel discord --account work --delete
vora channels status --probe
vora status --deep
```
...
Tip: use `vora skills search`, `vora skills install`, and `vora skills update` for ClawHub-backed skills.
...
- `vora message send --target +15555550123 --message "Hi"`
- `vora message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, Vora may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Vora can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `vora status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Vora services are treated as first-class and aren't flagged as "extra".
...
```bash
claude setup-token
vora models auth setup-token --provider anthropic
vora models status
```
...
```bash
vora models auth login --provider anthropic --method cli --set-default
```
...
`vora models` is an alias for `models status`.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `VORA_GATEWAY_TOKEN` / `VORA_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
- Node-host auth resolution only honors `VORA_GATEWAY_*` env vars.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`vora browser`](/cli/browser) and the [Browser tool](/tools/browser).
