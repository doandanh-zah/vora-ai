---
summary: "CLI reference for `vora onboard` (interactive onboarding)"
read_when:
...
```bash
vora onboard
vora onboard --flow quickstart
vora onboard --flow manual
vora onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`VORA_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `vora gateway run`.
...
- Fastest first chat: `vora dashboard` (Control UI, no channel setup).
