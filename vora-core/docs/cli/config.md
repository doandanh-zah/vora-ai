---
summary: "CLI reference for `vora config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `vora.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `vora configure`).
...
vora config file
vora config schema
vora config get browser.executablePath
...
vora config set secrets.providers.vaultfile --provider-source file --provider-path /etc/vora/secrets.json --provider-mode json
...
vora config validate
vora config validate --json
```

### `config schema`

Print the generated JSON schema for `vora.json` to stdout as plain text.
...
```bash
vora config schema > vora.schema.json
```
...
vora config get agents.list[0].id
```
...
vora config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
vora config set agents.defaults.heartbeat.every "0m"
vora config set gateway.port 19001 --strict-json
vora config set channels.whatsapp.groups '["*"]' --strict-json
```

## `config set` modes

`vora config set` supports four assignment styles:

1. Value mode: `vora config set <path> <value>`
2. SecretRef builder mode:

```bash
vora config set channels.discord.token \
...
```bash
vora config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/vora-vault \
  --provider-arg read \
...
```bash
vora config set --batch-file ./config-set.batch.json --dry-run
```
...
vora config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/vora/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `vora.json`.
...
- `config file`: Print the active config file path (resolved from `VORA_CONFIG_PATH` or default location).
