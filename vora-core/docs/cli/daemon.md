---
summary: "CLI reference for `vora daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `vora daemon ...` in scripts
...
`vora daemon ...` maps to the same service control surface as `vora gateway ...` service commands.
...
Use [`vora gateway`](/cli/gateway) for current docs and examples.
