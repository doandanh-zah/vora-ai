---
summary: "CLI reference for `vora configure` (interactive configuration prompts)"
read_when:
...
Tip: `vora config` without a subcommand opens the same wizard. Use
`vora config get|set|unset` for non-interactive edits.

For web search, `vora configure --section web` lets you choose a provider
...
vora configure --section model --section channels
```
