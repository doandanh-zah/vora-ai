---
summary: "CLI reference for `vora plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
vora plugins inspect <id>
vora plugins enable <id>
vora plugins disable <id>
vora plugins uninstall <id>
vora plugins doctor
vora plugins update <id>
vora plugins update --all
vora plugins marketplace list <marketplace>
```

Bundled plugins ship with Vora but start disabled. Use `plugins enable` to
activate them.

Native Vora plugins must ship `vora.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: vora` or `Format: bundle`. Verbose list/info
...
vora plugins install clawhub:<package>              # ClawHub only
vora plugins install <package> --pin                # pin version
vora plugins install <package> --dangerously-force-unsafe-install
vora plugins install <path>                         # local path
vora plugins install <plugin>@<marketplace>         # marketplace
vora plugins install <plugin> --marketplace <name>  # marketplace (explicit)
```
...
This CLI flag applies to `vora plugins install`. Gateway-backed skill
dependency installs use the matching `dangerouslyForceUnsafeInstall` request
override, while `vora skills install` remains a separate ClawHub skill
download/install flow.

`plugins install` is also the install surface for hook packs that expose
`vora.hooks` in `package.json`. Use `vora hooks` for filtered hook
...
If a bare install spec matches a bundled plugin id (for example `diffs`), Vora
...
```bash
vora plugins install clawhub:vora-codex-app-server
vora plugins install clawhub:vora-codex-app-server@1.2.3
```

Vora now also prefers ClawHub for bare npm-safe plugin specs. It only falls
back to npm if ClawHub does not have that package or version:

```bash
vora plugins install vora-codex-app-server
```

Vora downloads the package archive from ClawHub, checks the advertised
...
```bash
vora plugins marketplace list <marketplace-name>
vora plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
vora plugins install <plugin-name> --marketplace <marketplace-name>
vora plugins install <plugin-name> --marketplace <owner/repo>
vora plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. Vora accepts relative path sources from
that repo and rejects external git, GitHub, URL/archive, and absolute-path
plugin sources from remote manifests.

For local paths and archives, Vora auto-detects:

- native Vora plugins (`vora.plugin.json`)
...
```bash
vora plugins install -l ./my-plugin
```
...
```bash
vora plugins uninstall <id>
vora plugins uninstall <id> --dry-run
vora plugins uninstall <id> --keep-files
```
...
```bash
vora plugins update <id-or-npm-spec>
vora plugins update --all
vora plugins update <id-or-npm-spec> --dry-run
vora plugins update @vora/voice-call@beta
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, Vora reuses the recorded install spec for that
...
or exact version. Vora resolves that package name back to the tracked plugin
