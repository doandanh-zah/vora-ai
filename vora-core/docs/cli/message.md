---
summary: "CLI reference for `vora message` (send + channel actions)"
read_when:
...
- On cache miss, Vora will attempt a live directory lookup when the provider supports it.
...
- `vora message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Vora does not force a `default` account SecretRef scope
...
```
vora message send --channel discord \
...
```
vora message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
vora message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
vora message poll --channel msteams \
...
```
vora message react --channel slack \
...
```
vora message react --channel signal \
...
```
vora message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
vora message send --channel telegram --target @mychat \
