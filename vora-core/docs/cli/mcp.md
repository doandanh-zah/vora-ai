---
summary: "Expose Vora channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Vora-backed channels
  - Running `vora mcp serve`
  - Managing Vora-saved MCP server definitions
title: "mcp"
---

# mcp

`vora mcp` has two jobs:

- run Vora as an MCP server with `vora mcp serve`
- manage Vora-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is Vora acting as an MCP server
- `list` / `show` / `set` / `unset` is Vora acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`vora acp`](/cli/acp) when Vora should host a coding harness
session itself and route that runtime through ACP.

## Vora as an MCP server

This is the `vora mcp serve` path.

## When to use `serve`

Use `vora mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  Vora-backed channel conversations
- you already have a local or remote Vora Gateway with routed sessions
- you want one MCP server that works across Vora's channel backends instead
  of running separate per-channel bridges

Use [`vora acp`](/cli/acp) instead when Vora should host the coding
runtime itself and keep the agent session inside Vora.

## How it works

`vora mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote Vora Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `vora mcp serve`
...
conversations. A conversation appears when Vora already has session state
...
# Remote Gateway
vora mcp serve --url wss://gateway-host:18789 --token-file ~/.vora/gateway.token

# Remote Gateway with password auth
vora mcp serve --url wss://gateway-host:18789 --password-file ~/.vora/gateway.password

# Enable verbose bridge logs
vora mcp serve --verbose

# Disable Claude-specific push notifications
vora mcp serve --claude-channel-mode off
```
...
Vora equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying Vora channel configuration
...
Vora ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `vora mcp serve`
...
## Vora as an MCP client registry

This is the `vora mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Vora over MCP. They manage Vora-owned MCP
server definitions under `mcp.servers` in Vora config.

Those saved definitions are for runtimes that Vora launches or configures
later, such as embedded Pi and other runtime adapters. Vora stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write Vora config
...
Vora also stores a lightweight MCP server registry in config for surfaces
that want Vora-managed MCP definitions.

Commands:

- `vora mcp list`
- `vora mcp show [name]`
- `vora mcp set <name> <json>`
- `vora mcp unset <name>`
...
```bash
vora mcp list
vora mcp show context7 --json
vora mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
vora mcp set docs '{"url":"https://mcp.example.com"}'
vora mcp unset context7
```
