---
summary: "CLI reference for `vora memory` (status/index/search)"
read_when:
...
```bash
vora memory status
vora memory status --deep
vora memory index --force
vora memory search "meeting notes"
vora memory search --query "deployment" --max-results 20
vora memory status --json
vora memory status --deep --index
vora memory status --deep --index --verbose
vora memory status --agent main
vora memory index --agent main --verbose
```
