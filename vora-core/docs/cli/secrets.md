---
summary: "CLI reference for `vora secrets` (reload, audit, configure, apply)"
read_when:
...
Use `vora secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `vora.json` refs)
...
vora secrets audit
vora secrets audit --check
vora secrets audit --json
vora secrets audit --allow-exec
```
...
```bash
vora secrets configure
vora secrets configure --plan-out /tmp/vora-secrets-plan.json
vora secrets configure --apply --yes
vora secrets configure --providers-only
vora secrets configure --skip-provider-setup
vora secrets configure --agent ops
vora secrets configure --json
```
...
- `configure` targets secret-bearing fields in `vora.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Vora fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
vora secrets apply --from /tmp/vora-secrets-plan.json --json
```
...
- `vora.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.vora/.env` known secret keys whose values were migrated
