...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Vora Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`vora acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Vora channel
conversations instead of hosting an ACP harness session, use
[`vora mcp serve`](/cli/mcp) instead.
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Vora gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
vora acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
vora acp --url wss://gateway-host:18789 --token-file ~/.vora/gateway.token

# Attach to an existing session key
vora acp --session agent:main:main

# Attach by label (must already exist)
vora acp --session-label "support inbox"

# Reset the session key before the first prompt
vora acp --session agent:main:main --reset-session
```
...
vora acp client --server-args --url wss://gateway-host:18789 --token-file ~/.vora/gateway.token

# Override the server command (default: vora)
vora acp client --server "node" --server-args vora.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Vora through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Vora Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `vora acp` over stdio.

Example config (persisted):

```bash
vora config set gateway.remote.url wss://gateway-host:18789
vora config set gateway.remote.token <token>
```
...
```bash
vora acp --session agent:main:main
vora acp --session agent:design:main
vora acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Vora plugin tools, enable the
...
Vora bot over ACP, use `acpx` with its built-in `vora` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx vora` at `vora acp`.
3. Target the Vora session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Vora ACP session
acpx vora exec "Summarize the active Vora session state."

# Persistent named session for follow-up turns
acpx vora sessions ensure --name codex-bridge
acpx vora -s codex-bridge --cwd /path/to/repo \
  "Ask my Vora work agent for recent context relevant to this repo."
```

If you want `acpx vora` to target a specific Gateway and session key every
time, override the `vora` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "vora": {
      "command": "env VORA_HIDE_BANNER=1 VORA_SUPPRESS_NOTES=1 vora acp --url ws://127.0.0.1:18789 --token-file ~/.vora/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Vora checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env VORA_HIDE_BANNER=1 VORA_SUPPRESS_NOTES=1 node vora.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Vora agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Vora ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`VORA_GATEWAY_TOKEN`, `VORA_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`VORA_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `VORA_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `vora acp client` sets `VORA_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `vora`).
