...
This is the full reference for `vora onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.vora/vora.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `vora doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.vora/credentials/oauth.json`; auth profiles live in `~/.vora/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.vora/credentials/oauth.json` (or `$VORA_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.vora/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `vora pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `vora configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `vora health`.
    - Tip: `vora status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export VORA_GATEWAY_TOKEN="your-token"
vora onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env VORA_GATEWAY_TOKEN
```
...
- Stores it under `~/.vora/tools/signal-cli/<version>/`.
...
Typical fields in `~/.vora/vora.json`:
...
`vora agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.vora/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.vora/agents/<agentId>/sessions/`.
