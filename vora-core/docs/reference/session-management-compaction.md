...
This document explains how Vora manages sessions end-to-end:
...
Vora is designed around a single **Gateway process** that owns session state.
...
Vora persists sessions in two layers:
...
- Transcripts: `~/.vora/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Vora resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Vora reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
vora sessions cleanup --dry-run
vora sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.vora/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Vora intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Vora consumes the events, but Pi decides when to compact).
...
Vora also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Vora bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Vora leaves it alone.
...
Vora supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Vora strips/suppresses this in the delivery layer.

As of `2026.1.10`, Vora also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Vora uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Vora’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `vora status`.
