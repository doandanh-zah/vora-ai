---
summary: "How Vora builds prompt context and reports token usage + costs"
read_when:
...
Vora tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Vora assembles its own system prompt on every run. It includes:
...
For images, Vora downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Vora session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `vora status --usage` and `vora channels list` show
...
`cacheWrite`. If pricing is missing, Vora shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Vora can
...
Anthropic's 1M context window is currently beta-gated. Vora can inject the
...
Vora skips the `context-1m-*` beta header because Anthropic currently
