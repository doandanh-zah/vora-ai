...
Vora integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Vora owns lifecycle when `channels.signal.autoStart=true`.
...
- Vora spawns `imsg rpc` as a child process (legacy iMessage integration).
