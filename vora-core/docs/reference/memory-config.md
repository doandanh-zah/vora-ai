...
This page lists every configuration knob for Vora memory search. For
...
`vora.json` unless noted otherwise.
...
When `provider` is not set, Vora selects the first available:
...
Remote embeddings require an API key. Vora resolves from:
...
When sqlite-vec is unavailable, Vora falls back to in-process cosine
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.vora/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
