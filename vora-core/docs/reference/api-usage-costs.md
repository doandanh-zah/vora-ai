...
Vora features that can generate provider usage or paid API calls.
...
- `vora status --usage` and `vora channels list` show provider **usage windows**
...
Vora can pick up credentials from:
...
`vora models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
