...
- With Anthropic API-key auth profiles, Vora seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Vora injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Vora exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.vora/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$VORA_STATE_DIR/logs/cache-trace.jsonl`
...
- `VORA_CACHE_TRACE=1` enables cache tracing.
- `VORA_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `VORA_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `VORA_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `VORA_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
