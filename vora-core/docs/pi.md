...
summary: "Architecture of Vora's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Vora
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Vora integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `vora-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Vora uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Vora directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
```json
{
  "@mariozechner/vora-agent-core": "0.61.1",
...
| `vora-agent-core`   | Agent loop, tool execution, `AgentMessage` types                                                       |
| `pi-coding-agent` | High-level SDK: `createAgentSession`, `SessionManager`, `AuthStorage`, `ModelRegistry`, built-in tools |
| `pi-tui`          | Terminal UI components (used in Vora's local TUI mode)                                             |
...
├── pi-embedded-runner/
│   ├── run.ts                     # Main entry: runEmbeddedVoraAgent()
│   ├── run/
│   │   ├── attempt.ts             # Single attempt logic with session setup
│   │   ├── params.ts              # RunEmbeddedVoraAgentParams type
...
│   ├── types.ts                   # EmbeddedVoraAgentMeta, EmbeddedPiRunResult
...
├── pi-tools.ts                    # createVoraCodingTools()
...
├── vora-tools.ts              # Vora-specific tools
...
The main entry point is `runEmbeddedVoraAgent()` in `pi-embedded-runner/run.ts`:

```typescript
import { runEmbeddedVoraAgent } from "./agents/pi-embedded-runner.js";
...
Inside `runEmbeddedAttempt()` (called by `runEmbeddedVoraAgent()`), the pi SDK is used:
...
Image injection is prompt-local: Vora loads image refs from the current prompt and
...
2. **Custom Replacements**: Vora replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Vora Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
vora-agent-core's `AgentTool` has a different `execute` signature than pi-coding-agent's `ToolDefinition`. The adapter in `pi-tool-definition-adapter.ts` bridges this:
...
export function toToolDefinitions(tools: AnyAgentTool[]): ToolDefinition[] {
  return tools.map((tool) => ({
    ...
    execute: async (toolCallId, params, onUpdate, _ctx, signal) => {
      // pi-coding-agent signature differs from vora-agent-core
...
This ensures Vora's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Vora CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Vora wraps this with `guardSessionManager()` for tool result safety.
...
Vora maintains an auth profile store with multiple API keys per provider:
...
Vora loads custom pi extensions for specialized behavior:
...
Vora also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Vora Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Vora tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.vora/agents/<agentId>/sessions/` (or `$VORA_STATE_DIR/agents/<agentId>/sessions/`) |
...
1. **Tool signature alignment**: Currently adapting between vora-agent-core and pi-coding-agent signatures
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `VORA_LIVE_TEST=1`)
