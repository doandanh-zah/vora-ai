...
read_when:
  - Setting up Zalo Personal for Vora
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Vora.
...
- Install via CLI: `vora plugins install @vora/zalouser`
- Or from a source checkout: `vora plugins install ./path/to/local/zalouser-plugin`
...
2. Login (QR, on the Gateway machine):
   - `vora channels login --channel zalouser`
...
```bash
vora directory self --channel zalouser
vora directory peers list --channel zalouser --query "name"
vora directory groups list --channel zalouser --query "work"
```
...
- `vora pairing list zalouser`
- `vora pairing approve zalouser <code>`
...
- On startup, Vora resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Vora stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Vora state. Example:
...
- Vora sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Vora sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `vora channels logout --channel zalouser && vora channels login --channel zalouser`
...
- The channel now runs fully in Vora without external CLI binaries.
