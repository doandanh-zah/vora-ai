...
read_when:
  - Setting up Twitch chat integration for Vora
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Vora connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
vora plugins install @vora/twitch
```

Local checkout (when running from a git repo):

```bash
vora plugins install ./path/to/local/twitch-plugin
```
...
4. Configure the token:
   - Env: `VORA_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "vora", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use VORA_TWITCH_ACCESS_TOKEN env var)
...
```bash
VORA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
