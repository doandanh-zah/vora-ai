---
summary: "Synology Chat webhook setup and Vora config"
read_when:
  - Setting up Synology Chat with Vora
...
```bash
vora plugins install ./path/to/local/synology-chat-plugin
```
...
1. Install and enable the Synology Chat plugin.
   - `vora onboard` now shows Synology Chat in the same channel setup list as `vora channels add`.
   - Non-interactive setup: `vora channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Vora gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Vora.
   - Guided: `vora onboard`
   - Direct: `vora channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `vora pairing list synology-chat`
  - `vora pairing approve synology-chat <CODE>`
...
```bash
vora message send --channel synology-chat --target 123456 --text "Hello from Vora"
vora message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Vora now rejects duplicate exact paths
