---
summary: "Mattermost bot setup and Vora config"
read_when:
...
```bash
vora plugins install @vora/mattermost
```

Local checkout (when running from a git repo):

```bash
vora plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure Vora and start the gateway.
...
Native slash commands are opt-in. When enabled, Vora registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Vora derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Vora.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Vora.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Vora, not `404`.
...
- Approve via:
  - `vora pairing list mattermost`
  - `vora pairing approve mattermost <CODE>`
...
Use these target formats with `vora message send` or cron/webhooks:
...
Vora resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Vora sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Vora sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Vora derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Vora run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="vora-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"vora-mattermost-interactions",
...
`vora message send` and cron/webhook deliveries.
