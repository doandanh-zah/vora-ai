...
read_when:
  - You want to connect Vora to LINE
...
LINE connects to Vora via the LINE Messaging API. The plugin runs as a webhook
...
```bash
vora plugins install @vora/line
```

Local checkout (when running from a git repo):

```bash
vora plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Vora applies strict pre-auth body limits and timeout before verification.
- Vora processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
vora pairing list line
vora pairing approve line <CODE>
```
