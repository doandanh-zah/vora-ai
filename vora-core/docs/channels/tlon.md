...
Tlon is a decentralized messenger built on Urbit. Vora connects to your Urbit ship and can
...
```bash
vora plugins install @vora/tlon
```

Local checkout (when running from a git repo):

```bash
vora plugins install ./path/to/local/tlon-plugin
```
...
By default, Vora blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `vora message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Vora replies in-thread.
