...
```bash
vora plugins install @vora/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
vora plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Vora" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `vora pairing list nextcloud-talk`
  - `vora pairing approve nextcloud-talk <CODE>`
