...
read_when:
  ...
  - Reviewing Vora security posture
title: "Pairing"
---

# Pairing

“Pairing” is Vora’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `vora-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.vora/credentials/`:
...
3. On your phone, open the Vora iOS app → Settings → Gateway.
...
- The legacy `node.pair.*` API (CLI: `vora nodes pending/approve`) is a
