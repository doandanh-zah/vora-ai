...
read_when:
  - You want to connect Vora to QQ
...
QQ Bot connects to Vora via the official QQ Bot API (WebSocket gateway). The
...
Current Vora installs bundle QQ Bot. You do not need a separate
`vora plugins install` step for normal setup.
...
```bash
vora channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `vora channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single Vora instance:
...
```bash
vora channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
...
| `/bot-version` | Show the Vora framework version  |
