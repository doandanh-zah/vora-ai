...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Vora to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Vora releases, so no separate plugin install
is required.
...
```bash
vora plugins install @vora/feishu
```
...
If you just installed Vora, run onboarding:
...
2. Configuring app credentials in Vora
...
1. You already ran `vora channels add` for Feishu
2. The gateway is running (`vora gateway status`)
...
```bash
vora pairing approve feishu <CODE>
```
...
2. Run `vora logs --follow` and look for `chat_id`
...
2. Run `vora logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `vora gateway status`  | Show gateway status           |
| `vora gateway install` | Install/start gateway service |
| `vora gateway stop`    | Stop gateway service          |
| `vora gateway restart` | Restart gateway service       |
| `vora logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `vora gateway status`
6. Check logs: `vora logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.vora/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.vora/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
