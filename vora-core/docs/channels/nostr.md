...
read_when:
  - You want Vora to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Vora to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`vora onboard`) and `vora channels add` list optional channel plugins.
...
```bash
vora plugins install @vora/nostr
```

Use a local checkout (dev workflows):

```bash
vora plugins install --link <path-to-local-nostr-plugin>
```
...
vora channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "vora@example.com",
        lud16: "vora@example.com",
      },
    },
  },
}
