...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Vora HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `vora pairing approve slack <code>`.
...
- The interactive callback values are Vora-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Vora falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Vora is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Vora",
    "description": "Slack connector for Vora"
  },
  "features": {
    "bot_user": {
      "display_name": "Vora",
    ...
    "slash_commands": [
      {
        "command": "/vora",
        "description": "Send a message to Vora",
        "should_escape": false
      }
    ]
  },
...
Vora supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Vora falls back to normal delivery for remaining payloads.
