...
```bash
vora plugins install @vora/msteams
```

Local checkout (when running from a git repo):

```bash
vora plugins install ./path/to/local/msteams-plugin
```

If you choose Teams during setup and a git checkout is detected,
Vora will offer the local install path automatically.
...
3. Configure Vora with those credentials.
...
- Talk to Vora via Teams DMs, group chats, or channels.
...
- On startup, Vora resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.vora/vora.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Vora, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `vora-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `vora plugins install @vora/msteams`
   - From a local checkout: `vora plugins install ./path/to/local/msteams-plugin`
...
Vora exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "Vora in Teams", full: "Vora in Teams" },
...
Vora handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Vora only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/VoraShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Vora sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `vora message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.vora/msteams-polls.json`.
...
```bash
vora message send --channel msteams \
...
vora message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
vora message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
vora message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
vora message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
