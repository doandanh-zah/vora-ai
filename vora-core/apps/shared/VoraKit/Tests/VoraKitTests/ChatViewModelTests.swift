...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> VoraChatHistoryPayload
{
    VoraChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> VoraChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> VoraChatSessionEntry
{
    VoraChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> VoraChatModelChoice {
    VoraChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [VoraChatHistoryPayload],
    sessionsResponses: [VoraChatSessionsListResponse] = [],
    modelResponses: [[VoraChatModelChoice]] = [],
    ...
    -> (TestChatTransport, VoraChatViewModel)
{
...
private func sendUserMessage(_ vm: VoraChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, VoraChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [VoraChatHistoryPayload]
    private let sessionsResponses: [VoraChatSessionsListResponse]
    private let modelResponses: [[VoraChatModelChoice]]
    ...
    private let stream: AsyncStream<VoraChatTransportEvent>
    private let continuation: AsyncStream<VoraChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<VoraChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> VoraChatHistoryPayload {
        ...
        return self.historyResponses.last ?? VoraChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [VoraChatAttachmentPayload]) async throws -> VoraChatSendResponse
    {
        ...
        return VoraChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> VoraChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? VoraChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [VoraChatModelChoice] {
    ...
    func emit(_ evt: VoraChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = VoraChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = VoraChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: VoraChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = VoraChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: VoraChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(VoraChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == VoraChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = VoraChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: VoraChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = VoraChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = VoraChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = VoraChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { VoraChatViewModel(sessionKey: "main", transport: transport) }
