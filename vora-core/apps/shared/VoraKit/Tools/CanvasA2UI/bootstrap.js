...
import "@a2ui/lit/ui";
import { themeContext } from "@vora/a2ui-theme-context";
...
class VoraA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.voraA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["vora:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.voraCanvasA2UIAction ??
      globalThis.voraCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.voraCanvasA2UIAction) {
...
if (!customElements.get("vora-a2ui-host")) {
  customElements.define("vora-a2ui-host", VoraA2UIHost);
}
