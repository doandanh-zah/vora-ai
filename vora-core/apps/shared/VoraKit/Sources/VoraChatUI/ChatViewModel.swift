...
private let chatUILogger = Logger(subsystem: "ai.vora", category: "VoraChatUI")

@MainActor
@Observable
public final class VoraChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [VoraChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [VoraChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [VoraPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [VoraChatPendingToolCall] = []
    public private(set) var sessions: [VoraChatSessionEntry] = []
    private let transport: any VoraChatTransport
    private var sessionDefaults: VoraChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: VoraChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any VoraChatTransport,
    ...
    public var sessionChoices: [VoraChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [VoraChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: VoraPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [VoraChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: VoraChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: VoraChatMessage) -> VoraChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> VoraChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return VoraChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: VoraChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: VoraChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: VoraChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [VoraChatMessage],
        incoming: [VoraChatMessage]) -> [VoraChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [VoraChatMessage]) -> [VoraChatMessage] {
        var result: [VoraChatMessage] = []
    ...
    private static func dedupeKey(for message: VoraChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [VoraChatMessageContent] = [
            VoraChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> VoraChatAttachmentPayload in
            VoraChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> VoraChatSessionEntry {
        VoraChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = VoraChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: VoraChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: VoraChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: VoraAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = VoraChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> VoraPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
