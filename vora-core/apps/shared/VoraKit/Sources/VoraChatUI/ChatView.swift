...
@MainActor
public struct VoraChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: VoraChatViewModel
    ...
    private var visibleMessages: [VoraChatMessage] {
        let base: [VoraChatMessage]
    ...
    private func mergeToolResults(in messages: [VoraChatMessage]) -> [VoraChatMessage] {
    ...
    private func isToolResultMessage(_ message: VoraChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: VoraChatMessage) -> Bool {
    ...
    private func primaryText(in message: VoraChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: VoraChatMessage) -> Bool {
    ...
    private func toolCalls(in message: VoraChatMessage) -> [VoraChatMessageContent] {
    ...
    private func inlineToolResults(in message: VoraChatMessage) -> [VoraChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: VoraChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: VoraChatMessage) -> String {
