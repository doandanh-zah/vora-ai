...
enum VoraPlatformImageFactory {
    static func image(_ image: VoraPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
