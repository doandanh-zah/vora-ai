...
struct ChatMessageBubble: View {
    let message: VoraChatMessage
    let style: VoraChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? VoraChatTheme.userText : VoraChatTheme.assistantText
    ...
    private var inlineAttachments: [VoraChatMessageContent] {
    ...
    private var toolCalls: [VoraChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [VoraChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? VoraChatTheme.userBubble
        }
        if self.style == .onboarding {
            return VoraChatTheme.onboardingAssistantBubble
        }
        return VoraChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return VoraChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: VoraChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? VoraChatTheme.userText : VoraChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: VoraChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(VoraChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [VoraChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: VoraChatTheme.assistantText)
            }
        }
    }
}
