...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case VoraCanvasCommand.present.rawValue,
                 VoraCanvasCommand.hide.rawValue,
                 VoraCanvasCommand.navigate.rawValue,
                 VoraCanvasCommand.evalJS.rawValue,
                 VoraCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case VoraCanvasA2UICommand.reset.rawValue,
                 VoraCanvasA2UICommand.push.rawValue,
                 VoraCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleA2UIInvoke(req)
            case VoraBrowserCommand.proxy.rawValue:
                return try await self.handleBrowserProxyInvoke(req)
            case VoraCameraCommand.snap.rawValue,
                 VoraCameraCommand.clip.rawValue,
                 VoraCameraCommand.list.rawValue:
                return try await self.handleCameraInvoke(req)
            case VoraLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case MacNodeScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            case VoraSystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case VoraSystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case VoraSystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case VoraSystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case VoraSystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case VoraCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(VoraCanvasPresentParams.self, from: req.paramsJSON)) ??
                VoraCanvasPresentParams()
        ...
        case VoraCanvasCommand.hide.rawValue:
            let sessionKey = self.mainSessionKey
            await MainActor.run {
                CanvasManager.shared.hide(sessionKey: sessionKey)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case VoraCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(VoraCanvasNavigateParams.self, from: req.paramsJSON)
            let sessionKey = self.mainSessionKey
            try await MainActor.run {
                _ = try CanvasManager.shared.show(sessionKey: sessionKey, path: params.url)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case VoraCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(VoraCanvasEvalParams.self, from: req.paramsJSON)
            let sessionKey = self.mainSessionKey
        ...
        case VoraCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(VoraCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case VoraCanvasA2UICommand.push.rawValue,
             VoraCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleBrowserProxyInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        guard VoraConfigFile.browserControlEnabled() else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: VoraNodeError(
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        switch req.command {
        case VoraCameraCommand.snap.rawValue:
            let params = (try? Self.decodeParams(VoraCameraSnapParams.self, from: req.paramsJSON)) ??
                VoraCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case VoraCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(VoraCameraClipParams.self, from: req.paramsJSON)) ??
                VoraCameraClipParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case VoraCameraCommand.list.rawValue:
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(VoraLocationGetParams.self, from: req.paramsJSON)) ??
            VoraLocationGetParams()
        ...
        do {
            ...
            let payload = VoraLocationPayload(
    ...
    private func handleA2UIReset(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        (() => {
          const host = globalThis.voraA2UI;
          if (!host) return JSON.stringify({ ok: false, error: "missing voraA2UI" });
    ...
    private func handleA2UIPush(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        let messages: [VoraKit.AnyCodable]
        if command == VoraCanvasA2UICommand.pushJSONL.rawValue {
            let params = try Self.decodeParams(VoraCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
            messages = try VoraCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
        } else {
            do {
                let params = try Self.decodeParams(VoraCanvasA2UIPushParams.self, from: req.paramsJSON)
        ...
        let messagesJSON = try VoraCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
        let js = """
        (() => {
          try {
            const host = globalThis.voraA2UI;
    ...
    private func resolveA2UIHostUrl() async -> String? {
        ...
        return baseUrl.appendingPathComponent("__vora__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(VoraSystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(VoraSystemWhichParams.self, from: req.paramsJSON)
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(VoraSystemNotifyParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> VoraLocationMode {
        let raw = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        return VoraLocationMode(rawValue: raw) ?? .off
    }
    ...
    private static func errorResponse(
        ...
        code: VoraNodeErrorCode,
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: VoraNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: VoraCanvasSnapshotFormat,
        maxWidth: Int?,
