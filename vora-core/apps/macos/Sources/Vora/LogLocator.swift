...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["VORA_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/vora")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("vora-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("vora-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/vora/ (rolling or stdout), or nil if none exist.
