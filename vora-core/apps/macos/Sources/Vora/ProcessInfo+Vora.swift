...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.vora.mac`) even if the shipped
    ...
    {
        if environment["VORA_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "vora.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "vora.nixMode") { return true }
