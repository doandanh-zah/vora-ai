...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: VoraProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.vora", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: VoraKit.AnyCodable]()) {
                $0[$1.key] = VoraKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: VoraProtocol.AnyCodable?) -> [String: VoraProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: VoraProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: VoraKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: VoraProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("vora.control.heartbeat")
    static let controlAgentEvent = Notification.Name("vora.control.agent")
}
