...
final class CanvasA2UIActionMessageHandler: NSObject, WKScriptMessageHandler {
    static let messageName = "voraCanvasA2UIAction"
    ...
    func userContentController(_: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        guard let name = VoraCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId =
        ...
        let contextJSON = VoraCanvasA2UIAction.compactJSON(userAction["context"])
        ...
        let text = VoraCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        Task { [weak webView] in
            ...
            await MainActor.run {
                ...
                let js = VoraCanvasA2UIAction.jsDispatchA2UIActionStatus(
                    actionId: actionId,
                    ok: result.ok,
                    error: result.error)
    ...
    // Formatting helpers live in VoraKit (`VoraCanvasA2UIAction`).
}
