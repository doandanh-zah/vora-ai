...
private let webChatSwiftLogger = Logger(subsystem: "ai.vora", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "vora.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: VoraChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> VoraChatSessionsListResponse {
        ...
        let defaults = decoded.defaults.map {
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return VoraChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [VoraChatAttachmentPayload]) async throws -> VoraChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> VoraChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                VoraGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: VoraProtocol.ModelChoice) -> VoraChatModelChoice {
        VoraChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<VoraChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any VoraChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = VoraChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: VoraChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<VoraChatView>) -> NSViewController
    {
