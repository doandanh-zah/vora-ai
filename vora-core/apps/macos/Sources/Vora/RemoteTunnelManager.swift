...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.vora", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Vora run already has an SSH listener on the expected port (common after restarts),
