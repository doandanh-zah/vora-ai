...
enum CommandResolver {
    private static let projectRootDefaultsKey = "vora.gatewayProjectRootPath"
    private static let helperName = "vora"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let voraEntry = root.appendingPathComponent("vora.mjs").path
        if FileManager().isReadableFile(atPath: voraEntry) { return voraEntry }
        let binEntry = root.appendingPathComponent("bin/vora.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/vora")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let voraPaths = self.voraManagedPaths(home: home)
        if !voraPaths.isEmpty {
            extras.insert(contentsOf: voraPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + voraPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func voraManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".vora"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func voraExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectVoraExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("vora.mjs").path,
            root.appendingPathComponent("bin/vora.js").path,
        ]
    ...
    static func hasAnyVoraInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.voraExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let voraPath = self.projectVoraExecutable(projectRoot: root) {
            return [voraPath, subcommand] + extraArgs
        }
        if let voraPath = self.voraExecutable(searchPaths: searchPaths) {
            return [voraPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "vora", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            vora entrypoint missing (looked for dist/index.js or vora.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real vora CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/vora"
        ...
        \(cliSection)
        \(projectSection)
        if command -v vora >/dev/null 2>&1; then
          CLI="$(command -v vora)"
          vora \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/vora.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/vora.mjs"
            node "$PRJ/vora.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/vora.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/vora.js"
            node "$PRJ/bin/vora.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent vora"
          pnpm --silent vora \(quotedArgs);
        else
          echo "vora CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? VoraConfigFile.loadDict()
