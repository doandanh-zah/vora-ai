...
enum VoraLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = VoraOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = VoraFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        VoraLogging.bootstrapIfNeeded()
        let label = VoraLogging.makeLabel(subsystem: subsystem, category: category)
...
struct VoraOSLogHandler: AppLogLevelBackedHandler {
...
struct VoraFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = VoraLogging.parseLabel(self.label)
