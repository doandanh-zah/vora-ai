...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.vora/vora.json. Prefer VORA_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.vora/vora.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.vora/vora.json. Restart the gateway to apply."
        }
