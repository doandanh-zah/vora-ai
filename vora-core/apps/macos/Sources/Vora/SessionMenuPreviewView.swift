...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.vora", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> VoraSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: VoraSessionPreviewEntry,
    ...
    private static func cache(payload: VoraSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: VoraChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [VoraKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [VoraKit.AnyCodable]) -> [VoraChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(VoraChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: VoraChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: VoraChatMessage) -> Bool {
    ...
    private static func toolNames(for message: VoraChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: VoraChatMessage) -> String? {
