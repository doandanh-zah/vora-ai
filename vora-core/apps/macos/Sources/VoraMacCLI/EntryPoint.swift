...
struct VoraMacCLI {
    static func main() async {
        ...
        default:
            fputs("vora-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      vora-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      vora-mac discover [--timeout <ms>] [--json] [--include-local]
      vora-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      vora-mac connect
      vora-mac connect --url ws://127.0.0.1:18789 --json
      vora-mac discover --timeout 3000 --json
      vora-mac wizard --mode local
    """)
}
