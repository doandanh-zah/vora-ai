...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: VoraProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: VoraProtocol.AnyCodable] = [
            "presence": .init([VoraProtocol.AnyCodable(entry)]),
        ]
        let payload = VoraProtocol.AnyCodable(payloadMap)
