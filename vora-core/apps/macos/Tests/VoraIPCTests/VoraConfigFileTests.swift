...
@Suite(.serialized)
struct VoraConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("vora-config-\(UUID().uuidString)")
            .appendingPathComponent("vora.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["VORA_CONFIG_PATH": override]) {
            #expect(VoraConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["VORA_CONFIG_PATH": override]) {
            ...
            #expect(VoraConfigFile.remoteGatewayPort() == 19999)
            #expect(VoraConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(VoraConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(VoraConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["VORA_CONFIG_PATH": override]) {
            ...
            VoraConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = VoraConfigFile.loadDict()
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["VORA_CONFIG_PATH": override]) {
            ...
            VoraConfigFile.clearRemoteGatewayUrl()
            let root = VoraConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("vora-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "VORA_CONFIG_PATH": nil,
            "VORA_STATE_DIR": dir,
        ]) {
            #expect(VoraConfigFile.stateDirURL().path == dir)
            #expect(VoraConfigFile.url().path == "\(dir)/vora.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("vora-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("vora.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "VORA_STATE_DIR": stateDir.path,
            "VORA_CONFIG_PATH": configPath.path,
        ]) {
            VoraConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-vora-config-file")
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let loaded = VoraConfigFile.loadDict()
